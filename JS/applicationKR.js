...
var cardNoColl = "";
var privateCardDetails = "";
var publicCardDetails = "";
var privateCardType = "Private Corporate Card";
var publicCardType = "Public Corporate Card";
var cardAmtArray = [];
var CurrCardAddModalId = "";
var isCreditCardRow = false;
var SplitRequired = "Yes";
...
var alltblID = ["idTblMIS", "idTblENT", "idTblGIFT", "idTblTSP", "idTblACC", "idTblAPT", "idTblCOM", "idTblMIL", "idTblMED", "idTblVSA", "idTblATC", "idTblOTH"];
...
function setDefaultValues(modalID, tblID) {
    ...
    if (travelStage == "pre") {
        $("#" + modalID).find($("select[fldtitle='CardType']")).val("Select").attr("disabled", true);
        $("#" + modalID).find($("select[fldtitle='CardNo']")).val("Select").attr("disabled", true);
        $("#" + modalID).find($("select[fldtitle='TransactionID']")).val("Select").attr("disabled", true);
        $("#" + modalID).find($("select[fldtitle='Transaction']")).val("Select").attr("disabled", true);
        $("#" + modalID).find($("input[fldtitle='CardAmount']")).val("").attr("disabled", true);
    }
    ...
    if ($("input[fldTitle='hdnVendorBRNInvoiceApplicable']").val() == "No") {
        $("#" + modalID).find($("input[fldTitle='VendorNumber']")).prev().css("color", "#e4e4e4");
        $("#" + modalID).find($("input[fldTitle='InvoiceNumber']")).prev().css("color", "#e4e4e4");
        $("#" + modalID).find($("input[fldTitle='BRNNumber']")).prev().css("color", "#e4e4e4");
        $("#" + modalID).find($("input[fldTitle='VendorNumber']")).val("NA").attr("disabled", true);
        $("#" + modalID).find($("input[fldTitle='InvoiceNumber']")).val("NA").attr("disabled", true);
        $("#" + modalID).find($("input[fldTitle='BRNNumber']")).val("NA").attr("disabled", true);
    }
    if (tblID == "idTblTSH") { // For Travel Schedule
        ...
        //    }
        //});
        //$("#idTSHDestCntryEdit, #idTSHDepCntryEdit").multiselect("refresh");
...
function addNewItem(modalID, tblID) {
    ...
    if (tblID == "idTblOTH") {
        ...
        $('#' + modalID).find(".ModalExpDisp").text("Sales Promotion Cost Details");//PIDSKR Gift->Sales Promotion Cost
    }
...
function bindStaticControls() {
    ...
    $(".btnCreditCardAdd").on('click', function () {
        CurrCardAddModalId = $(this).parent(".center-btn").find(".sum").attr("data-target").split('#')[1];
        $("#CreditCardModal").find("select[fldTitle='CardType']").val("Select");
        $("#CreditCardModal").find("select[fldTitle='CardType']").change();
    });
...
function validateTravelFields(fldIDArr, fldNameArr,fldLanguageNameArr, fldTypeArr, fldModal) {
    ...
    for (var i = 0; i < fldIDArr.length; i++) {
        ...
        var fldName = langArray[fldLanguageNameArr[i]];

        if (fldType == "text") {
            if ($("#" + fldID).val().trim() == "") {
                //   $("#" + fldModal).find('.modalInfo').text(fldName + " cannot be blank");
                $("#" + fldModal).find('.modalInfo').text(fldName + " " + langalertArray["0"]);
                $("#" + fldID).focus();
                showError();
                return false;
            }
            ...
            if (($("#" + fldID + ' option:selected').index() == -1) || $("#" + fldID).val().toLowerCase() == "select") {
                ...
                $("#" + fldModal).find('.modalInfo').text(langalertArray["1"]+" " + fldName);
                $("#" + fldModal).val("Select");
                $("#" + fldID).focus();
                showError();
                return false;
            ...
            if (isNaN(outputNumber($("#" + fldID).val().trim()))) {
                ...
                $("#" + fldModal).find('.modalInfo').text(fldName + " " + langalertArray["2"]);
                $("#" + fldID).focus();
                showError();
                return false;
            }
            if (outputNumber($("#" + fldID).val().trim()) <= 0) {
                ...
                $("#" + fldModal).find('.modalInfo').text(fldName + " " + langalertArray["3"]);
                $("#" + fldID).focus();
                showError();
                return false;
            }
...
//Format Table Start
function formatTravelFields(tr, fldLabel, fldID, fldClass,fldLanguageLabel, prePost) {
    if (printFlg) {
    ...
    if (postDetails) {
        ...
        formatText += '<th></th><th>' + langArray.fldPre + '</th><th>' + langArray.fldPost + '</th><th></th><th>' + langArray.fldPre + '</th><th>' + langArray.fldPost + '</th><th></th><th>' + langArray.fldPre + '</th><th>' + langArray.fldPost + '</th></tr><tr>';
        fldWidth = "width=10%";
    }
    for (var i = 0; i < fldLabel.length; i++) {
        fldLabelName = langArray[fldLanguageLabel[i]];
        ...
        if (fldLabel[i] == "Description") {
            ...
            if (postDetails) {
                ...
                var lsDesc = "<u>" + langArray.fldPreTravel + ":</u> " + (tr.find('#' + fldID[i]).attr("prevalue") ? tr.find('#' + fldID[i]).attr("prevalue") : "") + "<br><br><u>"+ langArray.fldPostTravel+":</u> " + tr.find("#" + fldID[i]).text();
               // formatText += "<th nowrap " + fldWidth + ">" + fldLabel[i] + "</th><td " + [fldWidth, cls, colspan].join(" ") + ">" + lsDesc + "</td>" + closeRow;
                formatText += "<th nowrap " + fldWidth + ">" + fldLabelName+ "</th><td " + [fldWidth, cls, colspan].join(" ") + ">" + lsDesc + "</td>" + closeRow;
            }
            else {
                //formatText += "<th nowrap " + fldWidth + ">" + fldLabel[i] + "</th>" + preTD + postTD + closeRow;
                formatText += "<th nowrap " + fldWidth + ">"+ fldLabelName + "</th>" + preTD + postTD + closeRow;
            }
        }
        else {
            //formatText += "<th nowrap " + fldWidth + ">" + fldLabel[i] + "</th>" + preTD + postTD + closeRow;
...
function calculateTotalAmt(tblID) {
    ...
    if (tblID == "idTblADV") {
        ...
        //PIDSKR Actual Amount Payout
        //if (travelStage == "pre" && (docStatus == "New" || docStatus == "Draft" || docStatus == "RFI")) {
        //    $("input[fldTitle='AccAdvAmt']").val(outputMoney(TotLCAmt));
        //}
    }
    if (tblID == "idTblRef") {
...
function getActionBtns(modalID) {
    ...
    if (SplitRequired == "Yes") {
        splitRow = '<button class="splitrow" modalid="' + modalID + '"><i class="fa fa-hand-scissors-o text-green"></i></button>';
    }
...
function setMEDDataEditModal() {
    ...
    var argCardType = tr.find("#idMEDCardType").text();
    setCardTransaction(argCardType);
    var cardNos = tr.find("#idMEDCardNo").text();
    getTransID(cardNos, argCardType);
    setTransaction(argCardType);
    setTransIDDetails('transID');
    setTransIDDetails('transfldslct');
    var fldEditID = ["idMEDCardTypeEdit", "idMEDCardNoEdit", "idMEDTransIDEdit", "idMEDTransDetailsEdit", "idMEDCardAmountEdit", "idMEDIDEdit", "idMEDSNoEdit", "idMEDTypeEdit", "idMEDClaimCurrEdit", "idMEDClaimAmtEdit", "idMEDExRateEdit", "idMEDLCAmtEdit", "idMEDCostCenterEdit", "idMEDGSTCodeEdit", "idMEDTaxableAmtEdit", "idMEDGSTAmtEdit", "idMEDGLCodeEdit", "idMEDReceiptNoEdit", "idMEDChargeToFactoryEdit", "idMEDExpDescEdit", "idMEDCostCenterDisplayEdit", "idMEDGSTCodeDisplayEdit"]
    var fldID = ["idMEDCardType", "idMEDCardNo", "idMEDTransID", "idMEDTransDetails", "idMEDCardAmount", "idMEDID", "idMEDSNo", "idMEDType", "idMEDClaimCurr", "idMEDClaimAmt", "idMEDExRate", "idMEDLCAmt", "idMEDCostCenter", "idMEDGSTCode", "idMEDTaxableAmt", "idMEDGSTAmt", "idMEDGLCode", "idMEDReceiptNo", "idMEDChargeToFactory", "idMEDExpDesc", "idMEDCostCenterDisplay", "idMEDGSTCodeDisplay"]
    var fldAccEditable = ["disabled", "disabled", "disabled", "disabled", "readonly", "", "", "disabled", "disabled", "", "readonly", "", "", "", "", "", "", "", "disabled", "readonly", "", ""]
...
function generateMEDRow(strJSON, flgIntialLoading) {
    ...
    $.each(objJSON, function (i, item) {
        ...
        strNewRow += '<td class="displaynone"><span class="datacol" prekey="ExpCode" postkey="ExpCode">Medical</span><span class="datacol pkey" id="idMEDID" prekey="ID" postkey="ID">' + item.ID + '</span>' + item.ID + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol sno" prekey="SNo" postkey="SNo" id="idMEDSNo">' + item.SNo + '</span>' + item.SNo + '</td>';
        ...
        // Transaction Details
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardType" postkey="CardType" id="idMEDCardType">' + item.CardType + '</span>' + item.CardType + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardNo" postkey="CardNo" id="idMEDCardNo">' + item.CardNo + '</span>' + item.CardNo + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionID" postkey="TransactionID" id="idMEDTransID">' + item.TransactionID + '</span>' + item.TransactionID + '</td>';
        if (item.MerchantName != "" || item.TransactionDate != "")
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idMEDTransDetails">' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</span>' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</td>';
        else
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idMEDTransDetails">Select</span>Select</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="MerchantName" postkey="MerchantName" id="idMEDMerchantName">' + item.MerchantName + '</span>' + item.MerchantName + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionDate" postkey="TransactionDate" id="idMEDTransactionDate">' + item.TransactionDate + '</span>' + item.TransactionDate + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardAmount" postkey="CardAmount" id="idMEDCardAmount">' + outputMoney(item.CardAmount) + '</span>' + outputMoney(item.CardAmount) + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionStatus" postkey="TransactionStatus" id="idMEDTransactionStatus">' + item.TransactionStatus + '</span>' + item.TransactionStatus + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionTripType" postkey="TransactionTripType" id="idMEDTransactionTripType">' + item.TransactionTripType + '</span>' + item.TransactionTripType + '</td>';
...
function getMEDDataEditModal() {
    ...
    // Transaction Details
    rowJSON["CardType"] = $("#idMEDCardTypeEdit").val();
    rowJSON["CardNo"] = $("#idMEDCardNoEdit").val();
    rowJSON["TransactionID"] = $("#idMEDTransIDEdit").val();
    rowJSON["TransDetails"] = $("#idMEDTransDetailsEdit").val();
    var TransDetails = $("#idMEDTransDetailsEdit").val();
    if (TransDetails != null && TransDetails != undefined && TransDetails != "" && TransDetails != "Select") {
        TransDetails = TransDetails.split('/');
        rowJSON["MerchantName"] = TransDetails[TransDetails.length - 5].trim();
        rowJSON["TransactionDate"] = TransDetails[TransDetails.length - 4].trim();
        rowJSON["CardAmount"] = TransDetails[TransDetails.length - 3].trim();
        rowJSON["TransactionStatus"] = TransDetails[TransDetails.length - 2].trim();
        rowJSON["TransactionTripType"] = TransDetails[TransDetails.length - 1].trim();
    }
    else {
        rowJSON["MerchantName"] = "";
        rowJSON["TransactionDate"] = "";
        rowJSON["CardAmount"] = $("#idMEDCardAmountEdit").val();
        rowJSON["TransactionStatus"] = "";
        rowJSON["TransactionTripType"] = "";
    }
...
function validateMED() {
    if (isCreditCardRow || (NewRow && NewRowsCnt == 0)) {
        return true;
    }
    ...
    var fldName = ["Expense Type", "FC Amount", "Curr.", "GST Code", "Taxable Amount", "Cost Center", "GL Code", "Charge to Fact."];
    ...
    if (travelStage == "post") {
        if (!ValidateCreditCardsFields(fldModal)) {
            return false;
        }
    }
    ...
    var cardtype = $("#" + fldModal).find($("select[fldTitle='CardType']"));
    if ((cardtype.val() == privateCardType || cardtype.val() == publicCardType) && travelStage == "post") {
        var actualAmt = outputNumber($("#idMEDCardAmountEdit").val());
        var claimAmt = outputNumber($("#idMEDLCAmtEdit").val());
        if (actualAmt < claimAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed card amount");
            showError();
            return false;
        }
        if (IsApplicantLevel == "true" && checkTransExists($("#idMEDCardNoEdit").val(), $("#idMEDTransIDEdit").val(), $("#idMEDSNoEdit").val())) {
            $("#" + fldModal).find('.modalInfo').text("Selected transaction : " + $("#idMEDTransIDEdit").val()+" is already used.");
            showError();
            return false;
        }
        var utilizedAmt = getUtilizedAmount($("#idMEDCardNoEdit").val(), $("#idMEDTransIDEdit").val(), $("#idMEDSNoEdit").val());
        if ((utilizedAmt + claimAmt) > actualAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed remaining amount for the selected transaction : " + outputMoney(actualAmt - utilizedAmt));
            showError();
            return false;
        }
    }
...
function formatMED(tr) {
    var fldLabel = ["Card Type", "Card No", "Transaction ID", "Transaction", "Card Amount", "", "S.#", "LC Amt", "GL Code", "Expense Type", "GST Code", "Charge to Fact.", "FC Amt", "Taxable Amt", "Receipt #", "Curr.", "GST Amt", "", "Ex. Rate", "Cost Center", "", "Description"];
    var fldID = ["idMEDCardType", "idMEDCardNo", "idMEDTransID", "idMEDTransDetails", "idMEDCardAmount", "", "idMEDSNo", "idMEDLCAmt", "idMEDGLCode", "idMEDType", "idMEDGSTCodeDisplay", "idMEDChargeToFactory", "idMEDClaimAmt", "idMEDTaxableAmt", "idMEDReceiptNo", "idMEDClaimCurr", "idMEDGSTAmt", "", "idMEDExRate", "idMEDCostCenterDisplay", "", "idMEDExpDesc"];
    var fldClass = ["", "", "", "", "amount", "", "", "amount", "", "", "", "", "amount", "amount", "", "", "amount", "amount", "amount", "", "", "", ""];
...
function setMILDataEditModal() {
    //Set Row for editing
    var tr = objSelectedRow;
    var argCardType = tr.find("#idMILCardType").text();
    setCardTransaction(argCardType);
    var cardNos = tr.find("#idMILCardNo").text();
    getTransID(cardNos, argCardType);
    setTransaction(argCardType);
    setTransIDDetails('transID');
    setTransIDDetails('transfldslct');
    ...
    //var fldAccEditable = ["", "", "disabled", "disabled", "readonly", "readonly", "readonly", "readonly", "readonly", "readonly", "disabled", "disabled", "disabled", "disabled", "readonly", "readonly", "readonly", "disabled", "", "readonly", "", "", "", "", "", "", "readonly", "disabled", "", "", ""]

    var fldEditID = ["idMILCardTypeEdit", "idMILCardNoEdit", "idMILTransIDEdit", "idMILTransDetailsEdit", "idMILCardAmountEdit", "idMILIDEdit", "idMILSNoEdit", "idMILTypeEdit", "idMILTransportationEdit", "idMILDateEdit", "idMILFromEdit", "idMILToEdit", "idMILMileageStartEdit", "idMILMileageEndEdit", "idMILDistanceTravelledEdit", "idMILKmsMileageRateEdit", "idMILClaimCurrEdit", "idMILClaimAmtEdit", "idMILExRateEdit", "idMILLCAmtEdit", "idMILCostCenterEdit", "idMILGSTCodeEdit", "idMILTaxableAmtEdit", "idMILGSTAmtEdit", "idMILGLCodeEdit", "idMILExpDescEdit", "idMILChargeToFactoryEdit", "idMILReceiptNoEdit", "idMILCostCenterDisplayEdit", "idMILGSTCodeDisplayEdit"]
    var fldID = ["idMILCardType", "idMILCardNo", "idMILTransID", "idMILTransDetails", "idMILCardAmount", "idMILID", "idMILSNo", "idMILType", "idMILTransportation", "idMILDate", "idMILFrom", "idMILTo", "idMILMileageStart", "idMILMileageEnd", "idMILDistanceTravelled", "idMILKmsMileageRate", "idMILClaimCurr", "idMILClaimAmt", "idMILExRate", "idMILLCAmt", "idMILCostCenter", "idMILGSTCode", "idMILTaxableAmt", "idMILGSTAmt", "idMILGLCode", "idMILExpDesc", "idMILChargeToFactory", "idMILReceiptNo", "idMILCostCenterDisplay", "idMILGSTCodeDisplay"]
    var fldAccEditable = ["disabled", "disabled", "disabled", "disabled", "readonly", "", "", "disabled", "disabled", "readonly", "readonly", "readonly", "readonly", "readonly", "readonly", "readonly", "disabled", "", "readonly", "", "", "", "", "", "", "readonly", "disabled", "", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function generateMILRow(strJSON, flgIntialLoading) {
    ...
    $.each(objJSON, function (i, item) {
        ...
        var DescDisp = setDescDisplay(item.PostDesc);
        strNewRow = '<td class="details-control center vmiddle"><i class="fa fa-plus"></td>';
        strNewRow += '<td class="displaynone"><span class="datacol" prekey="ExpCode" postkey="ExpCode">Mileage</span><span class="datacol pkey" id="idMILID" prekey="ID" postkey="ID">' + item.ID + '</span>' + item.ID + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol sno" prekey="SNo" postkey="SNo" id="idMILSNo">' + item.SNo + '</span>' + item.SNo + '</td>';
        ...
        // Transaction Details
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardType" postkey="CardType" id="idMILCardType">' + item.CardType + '</span>' + item.CardType + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardNo" postkey="CardNo" id="idMILCardNo">' + item.CardNo + '</span>' + item.CardNo + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionID" postkey="TransactionID" id="idMILTransID">' + item.TransactionID + '</span>' + item.TransactionID + '</td>';
        if (item.MerchantName != "" || item.TransactionDate != "")
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idMILTransDetails">' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</span>' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</td>';
        else
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idMILTransDetails">Select</span>Select</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="MerchantName" postkey="MerchantName" id="idMILMerchantName">' + item.MerchantName + '</span>' + item.MerchantName + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionDate" postkey="TransactionDate" id="idMILTransactionDate">' + item.TransactionDate + '</span>' + item.TransactionDate + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardAmount" postkey="CardAmount" id="idMILCardAmount">' + outputMoney(item.CardAmount) + '</span>' + outputMoney(item.CardAmount) + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionStatus" postkey="TransactionStatus" id="idMILTransactionStatus">' + item.TransactionStatus + '</span>' + item.TransactionStatus + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionTripType" postkey="TransactionTripType" id="idMILTransactionTripType">' + item.TransactionTripType + '</span>' + item.TransactionTripType + '</td>';
...
function getMILDataEditModal() {
    ...
    // Transaction Details
    rowJSON["CardType"] = $("#idMILCardTypeEdit").val();
    rowJSON["CardNo"] = $("#idMILCardNoEdit").val();
    rowJSON["TransactionID"] = $("#idMILTransIDEdit").val();
    rowJSON["TransDetails"] = $("#idMILTransDetailsEdit").val();
    var TransDetails = $("#idMILTransDetailsEdit").val();
    if (TransDetails != null && TransDetails != undefined && TransDetails != "" && TransDetails != "Select") {
        TransDetails = TransDetails.split('/');
        rowJSON["MerchantName"] = TransDetails[TransDetails.length - 5].trim();
        rowJSON["TransactionDate"] = TransDetails[TransDetails.length - 4].trim();
        ...
        rowJSON["MerchantName"] = "";
        rowJSON["TransactionDate"] = "";
        rowJSON["CardAmount"] = $("#idMILCardAmountEdit").val();
        rowJSON["TransactionStatus"] = "";
        rowJSON["TransactionTripType"] = "";
    }
    objJSON.push(rowJSON);
...
function validateMIL() {
    ...
    var fldType = [];
    if (MILType == "By Zone") {
        //rowJSON["Zone"] = $("#idMILZoneEdit").val();//rowJSON["CustomerIndustrialZone"] = $("#idMILCustomerZoneEdit").val();//rowJSON["Province"] = $("#idMILProvinceEdit").val();
        //rowJSON["VisitingRoute"] = $("#idMILVisitingRouteEdit").val();//rowJSON["KMMaster"] = $("#idMILKmsMasterEdit").val();//rowJSON["KMActual"] = $("#idMILKmsActualEdit").val();
        //idMILZoneEdit, idMILCustomerZoneEdit, idMILProvinceEdit, idMILVisitingRouteEdit, idMILKmsMasterEdit, idMILKmsActualEdit
        ...
        var fldName = ["Expense Type", "Trans. Mode", "Date", "Place From", "Place To", "Mileage Start", "Zone", "Cust./Ind. Zone", "Province", "Visiting Route", "KMs(Actual)", "FC Amount", "Currency", "GST Code", "Taxable Amt", "Cost Center", "GL Code", "Charge to Fact."];
        ...
        //var fldID = ["idMILTypeEdit", "idMILTransportationEdit", "idMILDateEdit", "idMILFromEdit", "idMILToEdit", "idMILMileageStartEdit", "idMILMileageEndEdit", "idMILClaimAmtEdit", "idMILClaimCurrEdit", "idMILGSTCodeEdit", "idMILTaxableAmtEdit", "idMILCostCenterEdit", "idMILGLCodeEdit", "idMILChargeToFactoryEdit"];
        //var fldName = ["Expense Type", "Trans. Mode", "Date", "Place From", "Place To", "Mileage Start", "Mileage End", "Zone", "Cust./Ind. Zone", "Province", "Visiting Route", "FC Amount", "Currency", "GST Code", "Taxable Amt", "Cost Center", "GL Code", "Charge to Fact."];
        ...
        var fldID = ["idMILTypeEdit", "idMILTransportationEdit", "idMILDateEdit", "idMILFromEdit", "idMILToEdit", "idMILMileageStartEdit", "idMILMileageEndEdit", "idMILClaimAmtEdit", "idMILClaimCurrEdit", "idMILGSTCodeEdit", "idMILTaxableAmtEdit", "idMILCostCenterEdit", "idMILGLCodeEdit", "idMILChargeToFactoryEdit"];
        var fldName = ["Expense Type", "Trans. Mode", "Date", "Place From", "Place To", "Mileage Start", "Mileage End", "FC Amount", "Currency", "GST Code", "Taxable Amt", "Cost Center", "GL Code", "Charge to Fact."];
        var fldLanguageName = ["fldExpenseType", "fldTransMode", "fldDate", "fldPlaceFrom", "fldPlaceTo", "fldMileageStart", "fldMileageEnd", "fldFCAmt", "fldCurr", "fldGSTCode", "fldTaxableAmt", "fldCostCenter", "fldGLCode", "fldChargeToFact."];
        var fldType = ["select", "select", "text", "text", "text", "zeronumber", "number", "number", "select", "select", "number", "select", "text", "select"];
    }
    var isPreRow = isPreExpenseRow(objSelectedRow);
    if (isPreRow && travelStage == "post") { // To Allow user to submit with 0 LC Amount for Pre Detail Row
        fldType.forEach(function (type, idx, arr) {
    ...
    if (!validateTravelFields(fldID, fldName,fldLanguageName, fldType, fldModal)) {
        return false;
    }
    if (isPreRow) { // For Pre Rows confirmation to update 0.00 LC Amount
        if (!postTravelZeroLCConfirm(fldModal)) {
    ...
    var cardtype = $("#" + fldModal).find($("select[fldTitle='CardType']"));
    if ((cardtype.val() == privateCardType || cardtype.val() == publicCardType) && travelStage == "post") {
        var actualAmt = outputNumber($("#idMILCardAmountEdit").val());
        var claimAmt = outputNumber($("#idMILLCAmtEdit").val());
        if (actualAmt < claimAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed card amount");
            showError();
            return false;
        }
        if (IsApplicantLevel == "true" && checkTransExists($("#idMILCardNoEdit").val(), $("#idMILTransIDEdit").val(), $("#idMILSNoEdit").val())) {
            $("#" + fldModal).find('.modalInfo').text("Selected transaction : " + $("#idMILTransIDEdit").val() + " is already used.");
            showError();
            return false;
        }
        var utilizedAmt = getUtilizedAmount($("#idMILCardNoEdit").val(), $("#idMILTransIDEdit").val(), $("#idMILSNoEdit").val());
        if ((utilizedAmt + claimAmt) > actualAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed remaining amount for the selected transaction : " + outputMoney(actualAmt - utilizedAmt));
            showError();
            return false;
...
function formatMIL(tr) {
    ...
    //var fldLabel = ["S.#", "Zone", "Ex. Rate", "Expense Type", "Cust./Ind. Zone", "LC Amt", "Trans. Mode", "Province", "GST Code", "Date", "Visiting Route", "Taxable Amt", "Place From", "Kms(Master)", "GST Amt", "Place To", "KMs(Actual)", "Cost Center", "Mileage Start", "Mileage Rate", "GL Code", "Mileage End", "FC Amt", "Charge to Fact.", "Dist. Travelled", "Curr.", "Receipt #", "Description"]
    //var fldClass = ["", "", "amount", "", "", "amount", "", "", "", "", "", "amount", "", "amount", "amount", "", "amount", "", "amount", "amount", "", "amount", "amount", "", "amount", "", "", ""];
    var fldID = ["idMILCardType", "idMILCardNo", "idMILTransID", "idMILTransDetails", "idMILCardAmount", "", "idMILSNo", "idMILExRate", "idMILType", "idMILLCAmt", "idMILTransportation", "idMILGSTCodeDisplay", "idMILDate", "idMILTaxableAmt", "idMILFrom", "idMILGSTAmt", "idMILTo", "idMILCostCenterDisplay", "idMILMileageStart", "idMILKmsMileageRate", "idMILGLCode", "idMILMileageEnd", "idMILClaimAmt", "idMILChargeToFactory", "idMILDistanceTravelled", "idMILClaimCurr", "idMILReceiptNo", "idMILExpDesc"]
    var fldLabel = ["Card Type", "Card No", "Transaction ID", "Transaction", "Card Amount", "", "S.#", "Ex. Rate", "Expense Type", "LC Amt", "Trans. Mode", "GST Code", "Date", "Taxable Amt", "Place From", "GST Amt", "Place To", "Cost Center", "Mileage Start", "Mileage Rate", "GL Code", "Mileage End", "FC Amt", "Charge to Fact.", "Dist. Travelled", "Curr.", "Receipt #", "Description"]
    var fldClass = ["", "", "", "", "amount", "", "", "amount", "", "amount", "", "", "", "amount", "", "amount", "", "", "amount", "amount", "", "amount", "amount", "", "amount", "", "", ""];
    var fldLanguageLabel = ["fldCardType", "fldCardNo", "fldTransactionID", "fldTransaction", "fldCardAmount", "", "fldSNo", "fldExchangeRate", "fldExpenseType", "fldLCAmt", "fldTransMode", "fldGSTCode", "fldDate", "fldTaxableAmt", "fldPlaceFrom", "fldGSTAmt", "fldPlaceTo", "fldCostCenter", "fldMileageStart", "fldMileageRate", "fldGLCode", "fldMileageEnd", "fldFCAmt", "fldChargeToFact", "fldDistTravelled", "fldCurr", "fldReceiptNo", "fldDescription"]
    strRowData = formatTravelFields(tr, fldLabel, fldID, fldClass,fldLanguageLabel, false);
    return strRowData;
}
...
function setOTHDataEditModal() {
    //Set Row for editing
    var tr = objSelectedRow;
    var argCardType = tr.find("#idOTHCardType").text();
    ...
    setCardTransaction(argCardType);
    var cardNos = tr.find("#idOTHCardNo").text();
    getTransID(cardNos, argCardType);
    setTransaction(argCardType);
    setTransIDDetails('transID');
    setTransIDDetails('transfldslct');
    var fldEditID = ["idOTHCardTypeEdit", "idOTHCardNoEdit", "idOTHTransIDEdit", "idOTHTransDetailsEdit", "idOTHCardAmountEdit", "idOTHIDEdit", "idOTHSNoEdit", "idOTHTypeEdit", "idOTHDateEdit", "idOTHFromEdit", "idOTHToEdit", "idOTHClaimCurrEdit", "idOTHClaimAmtEdit", "idOTHExRateEdit", "idOTHLCAmtEdit", "idOTHCostCenterEdit", "idOTHGSTCodeEdit", "idOTHTaxableAmtEdit", "idOTHGSTAmtEdit", "idOTHGLCodeEdit", "idOTHExpDescEdit", "idOTHChargeToFactoryEdit", "idOTHReceiptNoEdit", "idOTHCostCenterDisplayEdit", "idOTHGSTCodeDisplayEdit"]
    var fldID = ["idOTHCardType", "idOTHCardNo", "idOTHTransID", "idOTHTransDetails", "idOTHCardAmount", "idOTHID", "idOTHSNo", "idOTHType", "idOTHDate", "idOTHFrom", "idOTHTo", "idOTHClaimCurr", "idOTHClaimAmt", "idOTHExRate", "idOTHLCAmt", "idOTHCostCenter", "idOTHGSTCode", "idOTHTaxableAmt", "idOTHGSTAmt", "idOTHGLCode", "idOTHExpDesc", "idOTHChargeToFactory", "idOTHReceiptNo", "idOTHCostCenterDisplay", "idOTHGSTCodeDisplay"]
    var fldAccEditable = ["", "", "", "", "", "", "", "disabled", "readonly", "readonly", "readonly", "disabled", "", "readonly", "", "", "", "", "", "", "readonly", "disabled", "", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function generateOTHRow(strJSON, flgIntialLoading) {
    ...
    $.each(objJSON, function (i, item) {
        ...
        var DescDisp = setDescDisplay(item.PostDesc);
        strNewRow = '<td class="details-control center vmiddle"><i class="fa fa-plus"></td>';
        strNewRow += '<td class="displaynone"><span class="datacol" prekey="ExpCode" postkey="ExpCode">Others</span><span class="datacol pkey" id="idOTHID" prekey="ID" postkey="ID">' + item.ID + '</span>' + item.ID + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol sno" prekey="SNo" postkey="SNo" id="idOTHSNo">' + item.SNo + '</span>' + item.SNo + '</td>';
        ...
        // Transaction Details
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardType" postkey="CardType" id="idOTHCardType">' + item.CardType + '</span>' + item.CardType + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardNo" postkey="CardNo" id="idOTHCardNo">' + item.CardNo + '</span>' + item.CardNo + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionID" postkey="TransactionID" id="idOTHTransID">' + item.TransactionID + '</span>' + item.TransactionID + '</td>';
        if (item.MerchantName != "" || item.TransactionDate != "")
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idOTHTransDetails">' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</span>' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</td>';
        else
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idOTHTransDetails">Select</span>Select</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="MerchantName" postkey="MerchantName" id="idOTHMerchantName">' + item.MerchantName + '</span>' + item.MerchantName + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionDate" postkey="TransactionDate" id="idOTHTransactionDate">' + item.TransactionDate + '</span>' + item.TransactionDate + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardAmount" postkey="CardAmount" id="idOTHCardAmount">' + outputMoney(item.CardAmount) + '</span>' + outputMoney(item.CardAmount) + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionStatus" postkey="TransactionStatus" id="idOTHTransactionStatus">' + item.TransactionStatus + '</span>' + item.TransactionStatus + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionTripType" postkey="TransactionTripType" id="idOTHTransactionTripType">' + item.TransactionTripType + '</span>' + item.TransactionTripType + '</td>';
...
function getOTHDataEditModal() {
    ...
    // Transaction Details
    rowJSON["CardType"] = $("#idOTHCardTypeEdit").val();
    rowJSON["CardNo"] = $("#idOTHCardNoEdit").val();
    rowJSON["TransactionID"] = $("#idOTHTransIDEdit").val();
    rowJSON["TransDetails"] = $("#idOTHTransDetailsEdit").val();
    var TransDetails = $("#idOTHTransDetailsEdit").val();
    if (TransDetails != null && TransDetails != undefined && TransDetails != "" && TransDetails != "Select") {
        TransDetails = TransDetails.split('/');
        rowJSON["MerchantName"] = TransDetails[TransDetails.length - 5].trim();
        rowJSON["TransactionDate"] = TransDetails[TransDetails.length - 4].trim();
        ...
        rowJSON["MerchantName"] = "";
        rowJSON["TransactionDate"] = "";
        rowJSON["CardAmount"] = $("#idOTHCardAmountEdit").val();
        rowJSON["TransactionStatus"] = "";
        rowJSON["TransactionTripType"] = "";
    }
    objJSON.push(rowJSON);
...
function validateOTH() {
    ...
    var fldID = ["idOTHTypeEdit", "idOTHDateEdit", "idOTHFromEdit", "idOTHToEdit", "idOTHClaimAmtEdit", "idOTHClaimCurrEdit", "idOTHGSTCodeEdit", "idOTHTaxableAmtEdit", "idOTHCostCenterEdit", "idOTHGLCodeEdit", "idOTHChargeToFactoryEdit"];
    var fldName = ["Expense Type", "Date", "Place From", "Place To", "FC Amount", "Curr.", "GST Code", "Taxable Amt", "Cost Center", "GL Code", "Charge to Fact."];
    var fldLanguageName = ["fldExpenseType", "fldDate", "fldPlaceFrom", "fldPlaceTo", "fldFCAmt", "fldCurr", "fldGSTCode", "fldTaxableAmt", "fldCostCenter", "fldGLCode", "fldChargeToFact"];
    var fldType = ["select", "text", "text", "text", "number", "select", "select", "number", "select", "text", "select"];
    var isPreRow = isPreExpenseRow(objSelectedRow);
    if (isPreRow && travelStage == "post") { // To Allow user to submit with 0 LC Amount for Pre Detail Row
        fldType.forEach(function (type, idx, arr) {
            if (type == "number") {
...
function formatOTH(tr) {
    var fldID = ["idOTHCardType", "idOTHCardNo", "idOTHTransID", "idOTHTransDetails", "idOTHCardAmount", "", "idOTHSNo", "idOTHClaimCurr", "idOTHCostCenterDisplay", "idOTHType", "idOTHExRate", "idOTHGLCode", "idOTHDate", "idOTHLCAmt", "idOTHChargeToFactory", "idOTHFrom", "idOTHGSTCodeDisplay", "idOTHReceiptNo", "idOTHTo", "idOTHTaxableAmt", "", "idOTHClaimAmt", "idOTHGSTAmt", "", "idOTHExpDesc"]
    var fldLabel = ["Card Type", "Card No", "Transaction ID", "Transaction", "Card Amount", "", "S.#", "Curr.", "Cost Center", "Expense Type", "Ex. Rate", "GL Code", "Date", "LC Amt", "Charge to Fact.", "Place From", "GST Code", "Receipt #", "Place To", "Taxable Amt", "", "FC Amt", "GST Amt", "", "Description"]
    var fldClass = ["", "", "", "", "amount", "", "", "", "", "", "amount", "", "", "amount", "", "", "", "", "", "amount", "amount", "amount", "amount", "amount", "text"];
    var fldLanguageLabel = ["fldCardType", "fldCardNo", "fldTransactionID", "fldTransaction", "fldCardAmount", "", "fldSNo", "fldCurr", "fldCostCenter", "fldExpenseType", "fldExchangeRate", "fldGLCode", "fldDate", "fldLCAmt", "fldChargeToFact", "fldPlaceFrom", "fldGSTCode", "fldReceiptNo", "fldPlaceTo", "fldTaxableAmt", "", "fldFCAmt", "fldGSTAmt", "", "fldDescription"]
    strRowData = formatTravelFields(tr, fldLabel, fldID, fldClass,fldLanguageLabel, false);
    return strRowData;
}
//OTHER Expense Ends
...
function validateWIN() {
    ...
    var fldName = ["LC Amount", "GST Code", "Taxable Amt", "Cost Center", "GL Code", "Charge to Fact."];
...
function formatWIN(tr) {
    var fldLabel = ["S.#", "LC Amt", "Cost Center", "Expense Type", "GST Code", "GL Code", "Last Claim Date", "Taxable Amt", "Charge To Fact.", "Last Claim Amount", "GST Amt", ""];
...
function setVSADataEditModal() {
    //Set Row for editing    
    var tr = objSelectedRow;
    var argCardType = tr.find("#idVSACardType").text();
    setCardTransaction(argCardType);
    var cardNos = tr.find("#idVSACardNo").text();
    getTransID(cardNos, argCardType);
    setTransaction(argCardType);
    setTransIDDetails('transID');
    setTransIDDetails('transfldslct');
    setDefaultCountryValues("idTblVSA");
    var fldEditID = ["idVSACardTypeEdit", "idVSACardNoEdit", "idVSATransIDEdit", "idVSATransDetailsEdit", "idVSACardAmountEdit", "idVSAIDEdit", "idVSASNoEdit", "idVSAExpTypEdit", "idVSACountryEdit", "idVSAVsaEdit", "idVSAFCAmtEdit", "idVSAExRateEdit", "idVSAPCEdit", "idVSATaxAmtEdit", "idVSAGSTAmtEdit", "idVSAGLCodeEdit", "idVSAGSTCodeEdit", "idSVSAChrgToFactEdit", "idVSARcptNoEdit", "idVSADescEdit", "idVSACurrencyEdit", "idVSALCAmtEdit", "idVSAArrgByEdit", "idVSAPCDisplayEdit", "idVSAGSTCodeDisplayEdit"]
    var fldID = ["idVSACardType", "idVSACardNo", "idVSATransID", "idVSATransDetails", "idVSACardAmount", "idVSAID", "idVSASNo", "idVSAExpTyp", "idVSACountry", "idVSAVsa", "idVSAFCAmt", "idVSAExRate", "idVSAPC", "idVSATaxAmt", "idVSAGSTAmt", "idVSAGLCode", "idVSAGSTCode", "idSVSAChrgToFact", "idVSARcptNo", "idVSADesc", "idVSACurrency", "idVSALCAmt", "idVSAArrgBy", "idVSAPCDisplay", "idVSAGSTCodeDisplay"]
    var fldAccEditable = ["disabled", "disabled", "disabled", "disabled", "readonly", "", "", "disabled", "disabled", "disabled", "", "readonly", "disabled", "", "", "", "", "disabled", "", "readonly", "disabled", "", "disabled", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function getVSADataEditModal() {
    ...
    if (travelStage == "pre") {
        ...
        rowJSON["CardType"] = "Select";
        rowJSON["CardNo"] = "Select";
        rowJSON["TransactionID"] = "Select";
        rowJSON["TransDetails"] = "Select";
        rowJSON["MerchantName"] = "";
        rowJSON["TransactionDate"] = "";
        rowJSON["CardAmount"] = "0.00";
    }
    else {
        ...
        // Transaction Details
        rowJSON["CardType"] = $("#idVSACardTypeEdit").val();
        rowJSON["CardNo"] = $("#idVSACardNoEdit").val();
        rowJSON["TransactionID"] = $("#idVSATransIDEdit").val();
        rowJSON["TransDetails"] = $("#idVSATransDetailsEdit").val();
        var TransDetails = $("#idVSATransDetailsEdit").val();
        if (TransDetails != null && TransDetails != undefined && TransDetails != "" && TransDetails != "Select") {
            TransDetails = TransDetails.split('/');
            rowJSON["MerchantName"] = TransDetails[TransDetails.length - 5].trim();
            rowJSON["TransactionDate"] = TransDetails[TransDetails.length - 4].trim();
            ...
            rowJSON["MerchantName"] = "";
            rowJSON["TransactionDate"] = "";
            rowJSON["CardAmount"] = $("#idVSACardAmountEdit").val();
            rowJSON["TransactionStatus"] = "";
            rowJSON["TransactionTripType"] = "";
        }
    }
...
function generateVSARow(strJSON, flgIntialLoading) {
    ...
    $.each(objJSON, function (i, item) {
        ...
        strNewRow = '<td class="details-control center vmiddle"><i class="fa fa-plus"></td>';
        strNewRow += '<td class="displaynone"><span class="datacol" prekey="ExpCode" postkey="ExpCode">Visa</span><span class="datacol pkey" id="idVSAID" prekey="ID" postkey="ID">' + item.ID + '</span>' + item.ID + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol sno" prekey="SNo" postkey="SNo" id="idVSASNo">' + item.SNo + '</span>' + item.SNo + '</td>';
        strNewRow += '<td class="predisplay vmiddle">' + item.PreExpTypes + '</td>';
        ...
        // Transaction Details
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardType" postkey="CardType" id="idVSACardType">' + item.CardType + '</span>' + item.CardType + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardNo" postkey="CardNo" id="idVSACardNo">' + item.CardNo + '</span>' + item.CardNo + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionID" postkey="TransactionID" id="idVSATransID">' + item.TransactionID + '</span>' + item.TransactionID + '</td>';
        if (item.MerchantName != "" || item.TransactionDate != "")
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idVSATransDetails">' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</span>' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</td>';
        else
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idVSATransDetails">Select</span>Select</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="MerchantName" postkey="MerchantName" id="idVSAMerchantName">' + item.MerchantName + '</span>' + item.MerchantName + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionDate" postkey="TransactionDate" id="idVSATransactionDate">' + item.TransactionDate + '</span>' + item.TransactionDate + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardAmount" postkey="CardAmount" id="idVSACardAmount">' + outputMoney(item.CardAmount) + '</span>' + outputMoney(item.CardAmount) + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionStatus" postkey="TransactionStatus" id="idVSATransactionStatus">' + item.TransactionStatus + '</span>' + item.TransactionStatus + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionTripType" postkey="TransactionTripType" id="idVSATransactionTripType">' + item.TransactionTripType + '</span>' + item.TransactionTripType + '</td>';
...
function validateVSA() {
    ...
    var fldName = ["Expense Types", "Country", "VISA", "FC Amount", "Visa Arranged By", "Currency", "Expense Rate", "LC Amount", "GST Code", "Taxable Amount", "Cost Center", "GL Code", "Charge To Factory"];
    ...
    var cardtype = $("#" + fldModal).find($("select[fldTitle='CardType']"));
    if ((cardtype.val() == privateCardType || cardtype.val() == publicCardType) && travelStage == "post") {
        var actualAmt = outputNumber($("#idVSACardAmountEdit").val());
        var claimAmt = outputNumber($("#idVSALCAmtEdit").val());
        if (actualAmt < claimAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed card amount");
            showError();
            return false;
        }
        if (IsApplicantLevel == "true" && checkTransExists($("#idVSACardNoEdit").val(), $("#idVSATransIDEdit").val(), $("#idVSASNoEdit").val())) {
            $("#" + fldModal).find('.modalInfo').text("Selected transaction : " + $("#idVSATransIDEdit").val() + " is already used.");
            showError();
            return false;
        }
        var utilizedAmt = getUtilizedAmount($("#idVSACardNoEdit").val(), $("#idVSATransIDEdit").val(), $("#idVSASNoEdit").val());
        if ((utilizedAmt + claimAmt) > actualAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed remaining amount for the selected transaction : " + outputMoney(actualAmt - utilizedAmt));
            showError();
            return false;
...
function formatVSA(tr) {
    var fldLabel = ["Card Type", "Card No", "Transaction ID", "Transaction", "Card Amount", "", "S.#", "Curr.", "GST Amt", "Expense Type", "Ex. Rate", "Cost Center", "Country/City", "LC Amt", "GL Code", "VISA #", "GST Code", "Charge To Fact.", "FC Amt", "Taxable Amt", "Receipt #", "Visa Arr. By", "NA", "NA", "Description"];
    var fldID = ["idVSACardType", "idVSACardNo", "idVSATransID", "idVSATransDetails", "idVSACardAmount", "", "idVSASNo", "idVSACurrency", "idVSAGSTAmt", "idVSAExpTyp", "idVSAExRate", "idVSAPCDisplay", "idVSACountry", "idVSALCAmt", "idVSAGLCode", "idVSAVsa", "idVSAGSTCodeDisplay", "idSVSAChrgToFact", "idVSAFCAmt", "idVSATaxAmt", "idVSARcptNo", "idVSAArrgBy", "NA", "NA", "idVSADesc"];
...
function setMISDataEditModal() {
    //Set Row for editing
    var tr = objSelectedRow;
    var argCardType = tr.find("#idMISCardType").text();
    setCardTransaction(argCardType);
    var cardNos = tr.find("#idMISCardNo").text();
    getTransID(cardNos, argCardType);
    setTransaction(argCardType);
    setTransIDDetails('transID');
    setTransIDDetails('transfldslct');
    var fldEditID = ["idMISCardTypeEdit", "idMISCardNoEdit", "idMISTransIDEdit", "idMISTransDetailsEdit", "idMISCardAmountEdit", "idMISIDEdit", "idMISSNoEdit", "idMISFCAmtEdit", "idMISExRateEdit", "idMISPCEdit", "idMISTaxAmtEdit", "idMISGSTAmtEdit", "idMISGLCodeEdit", "idMISGSTCodeEdit", "idSMISChargFacryEdit", "idMISRcptAmtEdit", "idMISDescEdit", "idMISCurncyEdit", "idMISLCAmtEdit", "idMISExpTypEdit", "idMISPCDisplayEdit", "idMISGSTCodeDisplayEdit"]
    var fldID = ["idMISCardType", "idMISCardNo", "idMISTransID", "idMISTransDetails", "idMISCardAmount", "idMISID", "idMISSNo", "idMISFCAmt", "idMISExRate", "idMISPC", "idMISTaxAmt", "idMISGSTAmt", "idMISGLCode", "idMISGSTCode", "idSMISChargFacry", "idMISRcptAmt", "idMISDesc", "idMISCurncy", "idMISLCAmt", "idMISExpTyp", "idMISPCEdit", "idMISGSTCodeDisplay"]
    var fldAccEditable = ["disabled", "disabled", "disabled", "disabled", "readonly", "", "", "", "readonly", "disabled", "", "", "", "", "disabled", "", "readonly", "disabled", "", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
}
function getMISDataEditModal() {
    ...
    // Transaction Details
    rowJSON["CardType"] = $("#idMISCardTypeEdit").val();
    rowJSON["CardNo"] = $("#idMISCardNoEdit").val();
    rowJSON["TransactionID"] = $("#idMISTransIDEdit").val();
    rowJSON["TransDetails"] = $("#idMISTransDetailsEdit").val();
    var TransDetails = $("#idMISTransDetailsEdit").val();
    if (TransDetails != null && TransDetails != undefined && TransDetails != "" && TransDetails != "Select") {
        TransDetails = TransDetails.split('/');
        rowJSON["MerchantName"] = TransDetails[TransDetails.length - 5].trim();
        rowJSON["TransactionDate"] = TransDetails[TransDetails.length - 4].trim();
        ...
        rowJSON["MerchantName"] = "";
        rowJSON["TransactionDate"] = "";
        rowJSON["CardAmount"] = $("#idMISCardAmountEdit").val();
        rowJSON["TransactionStatus"] = "";
        rowJSON["TransactionTripType"] = "";
    }
    objJSON.push(rowJSON);
...
function generateMISRow(strJSON, flgIntialLoading) {
    ...
    $.each(objJSON, function (i, item) {
        ...
        var DescDisp = setDescDisplay(item.PostDesc);
        strNewRow = '<td class="details-control center vmiddle"><i class="fa fa-plus"></td>';
        strNewRow += '<td class="displaynone"><span class="datacol" prekey="ExpCode" postkey="ExpCode">Miscellaneous</span><span class="datacol pkey" id="idMISID" prekey="ID" postkey="ID">' + item.ID + '</span>' + item.ID + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol sno" prekey="SNo" postkey="SNo" id="idMISSNo">' + item.SNo + '</span>' + item.SNo + '</td>';
        ...
        // Transaction Details
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardType" postkey="CardType" id="idMISCardType">' + item.CardType + '</span>' + item.CardType + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardNo" postkey="CardNo" id="idMISCardNo">' + item.CardNo + '</span>' + item.CardNo + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionID" postkey="TransactionID" id="idMISTransID">' + item.TransactionID + '</span>' + item.TransactionID + '</td>';
        if (item.MerchantName != "" || item.TransactionDate != "")
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idMISTransDetails">' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</span>' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</td>';
        else
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idMISTransDetails">Select</span>Select</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="MerchantName" postkey="MerchantName" id="idMISMerchantName">' + item.MerchantName + '</span>' + item.MerchantName + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionDate" postkey="TransactionDate" id="idMISTransactionDate">' + item.TransactionDate + '</span>' + item.TransactionDate + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardAmount" postkey="CardAmount" id="idMISCardAmount">' + outputMoney(item.CardAmount) + '</span>' + outputMoney(item.CardAmount) + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionStatus" postkey="TransactionStatus" id="idMISTransactionStatus">' + item.TransactionStatus + '</span>' + item.TransactionStatus + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionTripType" postkey="TransactionTripType" id="idMISTransactionTripType">' + item.TransactionTripType + '</span>' + item.TransactionTripType + '</td>';
        strNewRow += strActBtns;
        if (flgIntialLoading) {
            strNewRow = "<tr>" + strNewRow + "</tr>";
...
function validateMIS() {
    ...
    var fldName = ["Expense Type", "FC Amount", "Currency", "Expense Rate", "LC Amount", "GST Code", "Taxable Amount", "Cost Center", "GL Code", "Charge To Factory"];
    var fldLanguageName = ["fldExpenseType", "fldFCAmount", "fldCurrency", "fldExchangeRate", "fldLCAmount", "fldGSTCode", "fldTaxableAmt", "fldCostCenter", "fldGLCode", "fldChargeToFact"];
    ...
    var cardtype = $("#" + fldModal).find($("select[fldTitle='CardType']"));
    if ((cardtype.val() == privateCardType || cardtype.val() == publicCardType) && travelStage == "post") {
        var actualAmt = outputNumber($("#idMISCardAmountEdit").val());
        var claimAmt = outputNumber($("#idMISLCAmtEdit").val());
        if (actualAmt < claimAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed card amount");
            showError();
            return false;
        }
        if (IsApplicantLevel == "true" && checkTransExists($("#idMISCardNoEdit").val(), $("#idMISTransIDEdit").val(), $("#idMISSNoEdit").val())) {
            $("#" + fldModal).find('.modalInfo').text("Selected transaction : " + $("#idMISTransIDEdit").val() + " is already used.");
            showError();
            return false;
        }
        var utilizedAmt = getUtilizedAmount($("#idMISCardNoEdit").val(), $("#idMISTransIDEdit").val(), $("#idMISSNoEdit").val());
        if ((utilizedAmt + claimAmt) > actualAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed remaining amount for the selected transaction : " + outputMoney(actualAmt - utilizedAmt));
            showError();
            return false;
...
function formatMIS(tr) {
    var fldLabel = ["Card Type", "Card No", "Transaction ID", "Transaction", "Card Amount", "", "S.#", "LC Amt", "Cost Center", "Expense Type", "GST Code", "GL Code", "FC Amt", "Taxable Amt", "Charge To Fact.", "Curr.", "GST Amt", "Receipt #", "Ex. Rate", "NA", "", "Description"];
    var fldID = ["idMISCardType", "idMISCardNo", "idMISTransID", "idMISTransDetails", "idMISCardAmount", "", "idMISSNo", "idMISLCAmt", "idMISPCDisplay", "idMISExpTyp", "idMISGSTCodeDisplay", "idMISGLCode", "idMISFCAmt", "idMISTaxAmt", "idSMISChargFacry", "idMISCurncy", "idMISGSTAmt", "idMISRcptAmt", "idMISExRate", "NA", "", "idMISDesc"];
    var fldClass = ["", "", "", "", "amount", "", "", "amount", "", "", "", "", "amount", "amount", "", "", "amount", "amount", "amount", "", "", ""];
    //added for multilanguage
    var fldLanguageLabel = ["fldCardType", "fldCardNo", "fldTransactionID", "fldTransaction", "fldCardAmount", "", "fldSNo", "fldLCAmt", "fldCostCenter", "fldExpenseType", "fldGSTCode", "fldGLCode", "fldFCAmt", "fldTaxableAmt", "fldChargeToFact", "fldCurr", "fldGSTAmt", "fldReceiptNo", "fldExchangeRate", "NA", "", "fldDescription"];
    //added for multilanguage
    strRowData = formatTravelFields(tr, fldLabel, fldID, fldClass,fldLanguageLabel, false);
    return strRowData;
}
...
function setACCDataEditModal() {
    ...
    var argCardType = tr.find("#idACCCardType").text();
    setCardTransaction(argCardType);
    var cardNos = tr.find("#idACCCardNo").text();
    getTransID(cardNos, argCardType);
    setTransaction(argCardType);
    setTransIDDetails('transID');
    setTransIDDetails('transfldslct');
    var fldEditID = ["idACCCardTypeEdit", "idACCCardNoEdit", "idACCTransIDEdit", "idACCTransDetailsEdit", "idACCCardAmountEdit", "idACCIDEdit", "idACCSNoEdit", "idACCLCAmtEdit", "idACCReceiptEdit", "idACCCountryEdit", "idACCGSTCodeEdit", "idACCVendorEdit", "idACCHotelNameEdit", "idACCTaxAmtEdit", "idACCInvoiceEdit", "idACCPNShareEdit", "idACCGSTAmtEdit", "idACCBRNEdit", "idACCNoDaysEdit", "idACCCostCenterEdit", "idACCFCAmtEdit", "idACCGLCodeEdit", "idACCExRateEdit", "idACCChrgtoFactEdit", "idACCCurrencyEdit", "idACCDescEdit", "idACCCostCenterDisplayEdit", "idACCGSTCodeDisplayEdit", "idACCArgByEdit"]
    var fldID = ["idACCCardType", "idACCCardNo", "idACCTransID", "idACCTransDetails", "idACCCardAmount", "idACCID", "idACCSNo", "idACCLCAmt", "idACCReceipt", "idACCCountry", "idACCGSTCode", "idACCVendor", "idACCHotelName", "idACCTaxAmt", "idACCInvoice", "idACCPNShare", "idACCGSTAmt", "idACCBRN", "idACCNoDays", "idACCCostCenter", "idACCFCAmt", "idACCGLCode", "idACCExRate", "idACCChrgtoFact", "idACCCurrency", "idACCDesc", "idACCCostCenterDisplay", "idACCGSTCodeDisplay", "idACCArgBy"]
    var fldAccEditable = ["disabled", "disabled", "disabled", "disabled", "readonly", "", "", "", "", "disabled", "", "", "readonly", "", "", "readonly", "", "", "readonly", "", "", "", "readonly", "disabled", "disabled", "readonly", "", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function generateACCRow(strJSON, flgIntialLoading) {
    ...
    $.each(objJSON, function (i, item) {
        ...
        var DescDisp = setDescDisplay(item.PostDesc);
        strNewRow = '<td class="details-control center vmiddle"><i class="fa fa-plus"></td>';
        strNewRow += '<td class="displaynone"><span class="datacol" prekey="ExpCode" postkey="ExpCode">Accommodation</span><span class="datacol pkey" id="idACCID" prekey="ID" postkey="ID">' + item.ID + '</span>' + item.ID + '</td>';
        strNewRow += '<td class="vmiddle"><span class="datacol displaynone sno" prekey="SNo" postkey="SNo" id="idACCSNo">' + item.SNo + '</span>' + item.SNo + '</td>';
        ...
        strNewRow += '<td class="displaynone amount vmiddle"><span class="datacol displaynone" prekey="VendorNo" postkey="VendorNo" id="idACCVendor">' + item.VendorNo + '</span>' + item.VendorNo + '</td>';
        ...
        strNewRow += '<td class="displaynone vmiddle"><span class="datacol displaynone" prekey="InvoiceNo" postkey="InvoiceNo" id="idACCInvoice">' + item.InvoiceNo + '</span>' + item.InvoiceNo + '</td>';
        ...
        strNewRow += '<td class="displaynone vmiddle"><span class="datacol displaynone" prekey="BRNNo" postkey="BRNNo" id="idACCBRN">' + item.BRNNo + '</span>' + item.BRNNo + '</td>';
        ...
        //strNewRow += '<td class="displaynone vmiddle"><span class="datacol displaynone" prekey="MaxPerNightShare" postkey="MaxPerNightShare" id="idACCPNShareMaxValue">' + MaxPerNightShare + '</span>' + MaxPerNightShare + '</td>';
        // Transaction Details
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardType" postkey="CardType" id="idACCCardType">' + item.CardType + '</span>' + item.CardType + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardNo" postkey="CardNo" id="idACCCardNo">' + item.CardNo + '</span>' + item.CardNo + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionID" postkey="TransactionID" id="idACCTransID">' + item.TransactionID + '</span>' + item.TransactionID + '</td>';
        if (item.MerchantName !="" || item.TransactionDate !="" ) 
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idACCTransDetails">' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</span>' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</td>';
        else
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idACCTransDetails">Select</span>Select</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="MerchantName" postkey="MerchantName" id="idACCMerchantName">' + item.MerchantName + '</span>' + item.MerchantName + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionDate" postkey="TransactionDate" id="idACCTransactionDate">' + item.TransactionDate + '</span>' + item.TransactionDate + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardAmount" postkey="CardAmount" id="idACCCardAmount">' + outputMoney(item.CardAmount) + '</span>' + outputMoney(item.CardAmount) + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionStatus" postkey="TransactionStatus" id="idACCTransactionStatus">' + item.TransactionStatus + '</span>' + item.TransactionStatus + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionTripType" postkey="TransactionTripType" id="idACCTransactionTripType">' + item.TransactionTripType + '</span>' + item.TransactionTripType + '</td>';
...
function getACCDataEditModal() {
    ...
    // Transaction Details
    rowJSON["CardType"] = $("#idACCCardTypeEdit").val();
    rowJSON["CardNo"] = $("#idACCCardNoEdit").val();
    rowJSON["TransactionID"] = $("#idACCTransIDEdit").val();
    rowJSON["TransDetails"] = $("#idACCTransDetailsEdit").val();
    var TransDetails = $("#idACCTransDetailsEdit").val();
    if (TransDetails != null && TransDetails != undefined && TransDetails != "" && TransDetails != "Select") {
        TransDetails = TransDetails.split('/');
        rowJSON["MerchantName"] = TransDetails[TransDetails.length - 5].trim();
        rowJSON["TransactionDate"] = TransDetails[TransDetails.length - 4].trim();
        ...
        rowJSON["MerchantName"] = "";
        rowJSON["TransactionDate"] = "";
        rowJSON["CardAmount"] = $("#idACCCardAmountEdit").val();
        rowJSON["TransactionStatus"] = "";
        rowJSON["TransactionTripType"] = "";
    }
    //rowJSON["CardAmount"] = $("#idACCCardAmountEdit").val();
...
function validateACC() {
    ...
    var fldName = ["Country", "Hotel Name", "Acc Arranged By", "No of Days", "FC Amount", "Per Night Share", "Currency", "GST Code", "Taxable Amount", "Cost Center", "GL Code", "Charge To Factory"];
    var fldLanguageName = ["fldCountry", "fldHotelName", "fldAccArrangedBy", "fldNoofDays", "fldFCAmt", "fldPerNightShare", "fldCurr", "fldGSTCode", "fldTaxableAmount", "fldCostCenter", "fldGLCode", "fldChargeToFact"];
    ...
    if (CustomValidateFields(fldModal) == false) {
        return false;
    }
    if (isPreRow) { // For Pre Rows confirmation to update 0.00 LC Amount
        if (!postTravelZeroLCConfirm(fldModal)) {
    ...
    var cardtype = $("#" + fldModal).find($("select[fldTitle='CardType']"));
    if ((cardtype.val() == privateCardType || cardtype.val() == publicCardType) && travelStage == "post") {
        var actualAmt = outputNumber($("#idACCCardAmountEdit").val());
        var claimAmt = outputNumber($("#idACCLCAmtEdit").val());
        if (actualAmt < claimAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed card amount");
            showError();
            return false;
        }
        if (IsApplicantLevel == "true" && checkTransExists($("#idACCCardNoEdit").val(), $("#idACCTransIDEdit").val(), $("#idACCSNoEdit").val())) {
            $("#" + fldModal).find('.modalInfo').text("Selected transaction : " + $("#idACCTransIDEdit").val() + " is already used.");
            showError();
            return false;
        }
        var utilizedAmt = getUtilizedAmount($("#idACCCardNoEdit").val(), $("#idACCTransIDEdit").val(), $("#idACCSNoEdit").val());
        if ((utilizedAmt + claimAmt) > actualAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed remaining amount for the selected transaction : " + outputMoney(actualAmt - utilizedAmt));
            showError();
            return false;
...
function formatACC(tr) {
    var fldLabel = ["Card Type", "Card No", "Transaction ID", "Transaction", "Card Amount", "", "S.#", "Curr.", "GL Code", "Country/City", "Ex. Rate", "Charge To Fact.", "Hotel Name", "LC Amt", "Receipt #", "Accom By", "GST Code", "Vendor #", "FC Amt", "Taxable Amt", "Invoice #", "No. of Days", "GST Amt", "BRN #", "Per Night Share", "Cost Center", "", "Description"];
    var fldID = ["idACCCardType", "idACCCardNo", "idACCTransID", "idACCTransDetails", "idACCCardAmount", "", "idACCSNo", "idACCCurrency", "idACCGLCode", "idACCCountry", "idACCExRate", "idACCChrgtoFact", "idACCHotelName", "idACCLCAmt", "idACCReceipt", "idACCArgBy", "idACCGSTCodeDisplay", "idACCVendor", "idACCFCAmt", "idACCTaxAmt", "idACCInvoice", "idACCNoDays", "idACCGSTAmt", "idACCBRN", "idACCPNShare", "idACCCostCenterDisplay", "", "idACCDesc"];
    var fldClass = ["", "", "", "", "amount", "", "", "", "", "", "amount", "", "", "amount", "", "", "", "", "amount", "amount", "", "", "amount", "", "amount", "", "", ""];
    //added for multilanguage
    var fldLanguageLabel = ["fldCardType", "fldCardNo", "fldTransactionID", "fldTransaction", "fldCardAmount", "", "fldSNo", "fldCurr", "fldGLCode", "fldCountryCity", "fldExchangeRate", "fldChargetoFact", "fldHotelName", "fldLCAmt", "fldReceiptNo", "fldAccomBy", "fldGSTCode", "fldVendorNo", "fldFCAmt", "fldTaxableAmt", "fldInvoiceNo", "fldNoofDays", "fldGSTAmt", "fldBRNNo", "fldPerNightShare", "fldCostCenter", "", "fldDescription"];
    //added for multilanguage
    var strRowData = formatTravelFields(tr, fldLabel, fldID, fldClass,fldLanguageLabel, false);
    return strRowData;
}
...
function setATCDataEditModal() {
    //Set Row for editing
    var tr = objSelectedRow;
    var argCardType = tr.find("#idATCCardType").text();
    setCardTransaction(argCardType);
    var cardNos = tr.find("#idATCCardNo").text();
    getTransID(cardNos, argCardType);
    setTransaction(argCardType);
    setTransIDDetails('transID');
    setTransIDDetails('transfldslct');
    var fldEditID = ["idATCCardTypeEdit", "idATCCardNoEdit", "idATCTransIDEdit", "idATCTransDetailsEdit", "idATCCardAmountEdit", "idATCIDEdit", "idATCSNoEdit", "idATCGSTCodeEdit", "idATCVendorEdit", "idATCInvoiceNoEdit", "idATCTaxAmtEdit", "idATCInvoiceEdit", "idATCTktArgByEdit", "idATCGSTAmtEdit", "idATCBRNEdit", "idATCCurrencyEdit", "idATCCostCenterEdit", "idATCDescEdit", "idATCFCAmtEdit", "idATCGLCodeEdit", "idATCExRateEdit", "idATCChrgToFactEdit", "idATCLCAmtEdit", "idATCReceiptEdit", "idATCCostCenterDisplayEdit", "idATCGSTCodeDisplayEdit"];
    var fldID = ["idATCCardType", "idATCCardNo", "idATCTransID", "idATCTransDetails", "idATCCardAmount", "idATCID", "idATCSNo", "idATCGSTCode", "idATCVendor", "idATCInvoiceNo", "idATCTaxAmt", "idATCInvoice", "idATCTktArgBy", "idATCGSTAmt", "idATCBRN", "idATCCurrency", "idATCCostCenter", "idATCDesc", "idATCFCAmt", "idATCGLCode", "idATCExRate", "idATCChrgToFact", "idATCLCAmt", "idATCReceipt", "idATCCostCenterDisplay", "idATCGSTCodeDisplay"];
    var fldAccEditable = ["disabled", "disabled", "disabled", "disabled", "readonly", "", "", "", "", "", "", "", "disabled", "", "", "disabled", "", "readonly", "", "", "readonly", "disabled", "", "", "", ""];
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function generateATCRow(strJSON, flgIntialLoading) {
    ...
    $.each(objJSON, function (i, item) {
        ...
        PreDesc = setDescDisplay(item.PreDesc);
        strNewRow = '<td class="details-control center vmiddle"><i class="fa fa-plus"></td>';
        strNewRow += '<td class="displaynone"><span class="datacol" prekey="ExpCode" postkey="ExpCode">AirTicketCost</span><span class="datacol pkey" id="idATCID" prekey="ID" postkey="ID">' + item.ID + '</span>' + item.ID + '</td>';
        strNewRow += '<td class="vmiddle"><span class="datacol displaynone sno" prekey="SNo" postkey="SNo" id="idATCSNo">' + item.SNo + '</span>' + item.SNo + '</td>';
        ...
        strNewRow += '<td class="displaynone vmiddle"><span class="datacol displaynone" prekey="VendorNo" postkey="VendorNo" id="idATCVendor">' + item.VendorNo + '</span>' + item.VendorNo + '</td>';
        ...
        strNewRow += '<td class="displaynone vmiddle"><span class="datacol displaynone" prekey="InvoiceNo" postkey="InvoiceNo" id="idATCInvoice">' + item.InvoiceNo + '</span>' + item.InvoiceNo + '</td>';
        ...
        strNewRow += '<td class="displaynone vmiddle"><span class="datacol displaynone" prekey="BRNNo" postkey="BRNNo" id="idATCBRN">' + item.BRNNo + '</span>' + item.BRNNo + '</td>';
        ...
        // Transaction Details
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardType" postkey="CardType" id="idATCCardType">' + item.CardType + '</span>' + item.CardType + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardNo" postkey="CardNo" id="idATCCardNo">' + item.CardNo + '</span>' + item.CardNo + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionID" postkey="TransactionID" id="idATCTransID">' + item.TransactionID + '</span>' + item.TransactionID + '</td>';
        if (item.MerchantName != "" || item.TransactionDate != "")
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idATCTransDetails">' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</span>' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</td>';
        else
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idATCTransDetails">Select</span>Select</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="MerchantName" postkey="MerchantName" id="idATCMerchantName">' + item.MerchantName + '</span>' + item.MerchantName + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionDate" postkey="TransactionDate" id="idATCTransactionDate">' + item.TransactionDate + '</span>' + item.TransactionDate + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardAmount" postkey="CardAmount" id="idATCCardAmount">' + outputMoney(item.CardAmount) + '</span>' + outputMoney(item.CardAmount) + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionStatus" postkey="TransactionStatus" id="idATCTransactionStatus">' + item.TransactionStatus + '</span>' + item.TransactionStatus + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionTripType" postkey="TransactionTripType" id="idATCTransactionTripType">' + item.TransactionTripType + '</span>' + item.TransactionTripType + '</td>';
...
function getATCDataEditModal() {
    ...
    rowJSON["VendorNo"] = $("#idATCVendorEdit").val();
    rowJSON["TaxableAmt"] = $("#idATCTaxAmtEdit").val();
    rowJSON["InvoiceNo"] = $("#idATCInvoiceEdit").val();
    rowJSON["GSTAmt"] = $("#idATCGSTAmtEdit").val();
    rowJSON["BRNNo"] = $("#idATCBRNEdit").val();
    ...
    if (travelStage == "pre") {
        ...
        // Transaction Details
        rowJSON["CardType"] = $("#idATCCardTypeEdit").val();
        rowJSON["CardNo"] = $("#idATCCardNoEdit").val();
        rowJSON["TransactionID"] = $("#idATCTransIDEdit").val();
        rowJSON["TransDetails"] = $("#idATCTransDetailsEdit").val();
        var TransDetails = $("#idATCTransDetailsEdit").val();
        if (TransDetails != null && TransDetails != undefined && TransDetails != "" && TransDetails != "Select") {
            TransDetails = TransDetails.split('/');
            rowJSON["MerchantName"] = TransDetails[TransDetails.length - 5].trim();
            rowJSON["TransactionDate"] = TransDetails[TransDetails.length - 4].trim();
            ...
            rowJSON["MerchantName"] = "";
            rowJSON["TransactionDate"] = "";
            rowJSON["CardAmount"] = $("#idATCCardAmountEdit").val();
            rowJSON["TransactionStatus"] = "";
            rowJSON["TransactionTripType"] = "";
        }
    }
...
function validateATC() {
    ...
    var fldName = ["Ticket Arr.By", "FC Amount", "Currency", "GST Code", "Taxable Amount", "Cost Center", "GL Code", "Charge To Factory"];
    var fldLanguageName = ["fldTicketArrBy", "fldFCAmt", "fldCurr", "fldGSTCode", "fldTaxableAmt", "fldCostCenter", "fldGLCode", "fldChargeToFact"];
    var fldType = ["select", "number", "select", "select", "number", "select", "text", "select"];
    var isPreRow = isPreExpenseRow(objSelectedRow);
    if (isPreRow && travelStage == "post") { // To Allow user to submit with 0 LC Amount for Pre Detail Row
        fldType.forEach(function (type, idx, arr) {
    ...
    var cardtype = $("#" + fldModal).find($("select[fldTitle='CardType']"));
    if ((cardtype.val() == privateCardType || cardtype.val() == publicCardType) && travelStage == "post") {
        var actualAmt = outputNumber($("#idATCCardAmountEdit").val());
        var claimAmt = outputNumber($("#idATCLCAmtEdit").val());
        if (actualAmt < claimAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed card amount");
            showError();
            return false;
        }
        if (IsApplicantLevel == "true" && checkTransExists($("#idATCCardNoEdit").val(), $("#idATCTransIDEdit").val(), $("#idATCSNoEdit").val())) {
            $("#" + fldModal).find('.modalInfo').text("Selected transaction : " + $("#idATCTransIDEdit").val() + " is already used.");
            showError();
            return false;
        }
        var utilizedAmt = getUtilizedAmount($("#idATCCardNoEdit").val(), $("#idATCTransIDEdit").val(), $("#idATCSNoEdit").val());
        if ((utilizedAmt + claimAmt) > actualAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed remaining amount for the selected transaction : " + outputMoney(actualAmt - utilizedAmt));
            showError();
            return false;
...
function formatATC(tr) {
    var fldLabel = ["Card Type", "Card No", "Transaction ID", "Transaction", "Card Amount", "", "S.#", "LC Amt", "Charge To Fact.", "Invoice Number", "GST Code", "Receipt #", "Tkt Arranged By", "Taxable Amt", "Vendor #", "FC Amt", "GST Amt", "Invoice #", "Curr.", "Cost Center", "BRN #", "Ex. Rate", "GL Code", "", "Description"];
    var fldID = ["idATCCardType", "idATCCardNo", "idATCTransID", "idATCTransDetails", "idATCCardAmount", "", "idATCSNo", "idATCLCAmt", "idATCChrgToFact", "idATCInvoiceNo", "idATCGSTCodeDisplay", "idATCReceipt", "idATCTktArgBy", "idATCTaxAmt", "idATCVendor", "idATCFCAmt", "idATCGSTAmt", "idATCInvoice", "idATCCurrency", "idATCCostCenterDisplay", "idATCBRN", "idATCExRate", "idATCGLCode", "", "idATCDesc"];
    var fldClass = ["", "", "", "", "amount", "", "", "amount", "", "", "", "", "", "amount", "", "amount", "amount", "", "", "", "", "amount", "", "", ""];
    //added for multilanguage
    var fldLanguageLabel = ["fldCardType", "fldCardNo", "fldTransactionID", "fldTransaction", "fldCardAmount", "", "fldSNo", "fldLCAmt", "fldChargetoFact", "fldInvoiceNo", "fldGSTCode", "fldReceiptNo", "fldTicketArrBy", "fldTaxableAmt", "fldVendorNo", "fldFCAmt", "fldGSTAmt", "fldInvoiceNo", "fldCurr.", "fldCostCenter", "fldBRNNo", "fldExchangeRate", "fldGLCode", "", "fldDescription"];
    //added for multilanguage
    var strRowData = formatTravelFields(tr, fldLabel, fldID, fldClass,fldLanguageLabel, true);
    return strRowData;
}
//Air Ticket Cost End
...
function setCOMDataEditModal() {
    //Set Row for editing
    var tr = objSelectedRow;
    var argCardType = tr.find("#idCOMCardType").text();
    setCardTransaction(argCardType);
    var cardNos = tr.find("#idCOMCardNo").text();
    getTransID(cardNos, argCardType);
    setTransaction(argCardType);
    setTransIDDetails('transID');
    setTransIDDetails('transfldslct');
    var fldEditID = ["idCOMCardTypeEdit", "idCOMCardNoEdit", "idCOMTransIDEdit", "idCOMTransDetailsEdit", "idCOMCardAmountEdit", "idCOMIDEdit", "idCOMSNoEdit", "idCOMExpTypeEdit", "idCOMTaxAmtEdit", "idCOMGSTAmtEdit", "idCOMCurrencyEdit", "idCOMCostCenterEdit", "idCOMFCAmtEdit", "idCOMGLCodeEdit", "idCOMExRateEdit", "idCOMChrgToFactEdit", "idCOMLCAmtEdit", "idCOMReceiptEdit", "idCOMGSTCodeEdit", "idCOMDescEdit", "idCOMCostCenterDisplayEdit", "idCOMGSTCodeDisplayEdit"];
    var fldID = ["idCOMCardType", "idCOMCardNo", "idCOMTransID", "idCOMTransDetails", "idCOMCardAmount", "idCOMID", "idCOMSNo", "idCOMExpType", "idCOMTaxAmt", "idCOMGSTAmt", "idCOMCurrency", "idCOMCostCenter", "idCOMFCAmt", "idCOMGLCode", "idCOMExRate", "idCOMChrgToFact", "idCOMLCAmt", "idCOMReceipt", "idCOMGSTCode", "idCOMDesc", "idCOMCostCenterDisplay", "idCOMGSTCodeDisplay"];
    var fldAccEditable = ["disabled", "disabled", "disabled", "disabled", "readonly", "", "", "disabled", "", "", "disabled", "", "", "", "readonly", "disabled", "", "", "", "readonly", "", ""];
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function generateCOMRow(strJSON, flgIntialLoading) {
    ...
    $.each(objJSON, function (i, item) {
        ...
        var DescDisp = setDescDisplay(item.PostDesc);
        strNewRow = '<td class="details-control center vmiddle"><i class="fa fa-plus"></td>';
        strNewRow += '<td class="displaynone"><span class="datacol" prekey="ExpCode" postkey="ExpCode">Communication</span><span class="datacol pkey" id="idCOMID" prekey="ID" postkey="ID">' + item.ID + '</span>' + item.ID + '</td>';
        strNewRow += '<td class="vmiddle"><span class="datacol displaynone sno" prekey="SNo" postkey="SNo" id="idCOMSNo">' + item.SNo + '</span>' + item.SNo + '</td>';
        ...
        // Transaction Details
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardType" postkey="CardType" id="idCOMCardType">' + item.CardType + '</span>' + item.CardType + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardNo" postkey="CardNo" id="idCOMCardNo">' + item.CardNo + '</span>' + item.CardNo + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionID" postkey="TransactionID" id="idCOMTransID">' + item.TransactionID + '</span>' + item.TransactionID + '</td>';
        if (item.MerchantName != "" || item.TransactionDate != "")
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idCOMTransDetails">' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</span>' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</td>';
        else
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idCOMTransDetails">Select</span>Select</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="MerchantName" postkey="MerchantName" id="idCOMMerchantName">' + item.MerchantName + '</span>' + item.MerchantName + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionDate" postkey="TransactionDate" id="idCOMTransactionDate">' + item.TransactionDate + '</span>' + item.TransactionDate + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardAmount" postkey="CardAmount" id="idCOMCardAmount">' + outputMoney(item.CardAmount) + '</span>' + outputMoney(item.CardAmount) + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionStatus" postkey="TransactionStatus" id="idCOMTransactionStatus">' + item.TransactionStatus + '</span>' + item.TransactionStatus + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionTripType" postkey="TransactionTripType" id="idCOMTransactionTripType">' + item.TransactionTripType + '</span>' + item.TransactionTripType + '</td>';
...
function getCOMDataEditModal() {
    ...
    // Transaction Details
    rowJSON["CardType"] = $("#idCOMCardTypeEdit").val();
    rowJSON["CardNo"] = $("#idCOMCardNoEdit").val();
    rowJSON["TransactionID"] = $("#idCOMTransIDEdit").val();
    rowJSON["TransDetails"] = $("#idCOMTransDetailsEdit").val();
    var TransDetails = $("#idCOMTransDetailsEdit").val();
    if (TransDetails != null && TransDetails != undefined && TransDetails != "" && TransDetails != "Select") {
        TransDetails = TransDetails.split('/');
        rowJSON["MerchantName"] = TransDetails[TransDetails.length - 5].trim();
        rowJSON["TransactionDate"] = TransDetails[TransDetails.length - 4].trim();
        ...
        rowJSON["MerchantName"] = "";
        rowJSON["TransactionDate"] = "";
        rowJSON["CardAmount"] = $("#idCOMCardAmountEdit").val();
        rowJSON["TransactionStatus"] = "";
        rowJSON["TransactionTripType"] = "";
    }
    objJSON.push(rowJSON);
...
function validateCOM() {
    ...
    var fldName = ["Expense Type", "FC Amount", "Currency", "GST Code", "Taxable Amount", "Cost Center", "GL Code", "Charge To Factory"];
    var fldType = ["select", "number", "select", "select", "number", "select", "text", "select"];
    var isPreRow = isPreExpenseRow(objSelectedRow);
    if (isPreRow && travelStage == "post") { // To Allow user to submit with 0 LC Amount for Pre Detail Row
        fldType.forEach(function (type, idx, arr) {
    ...
    var cardtype = $("#" + fldModal).find($("select[fldTitle='CardType']"));
    if ((cardtype.val() == privateCardType || cardtype.val() == publicCardType) && travelStage == "post") {
        var actualAmt = outputNumber($("#idCOMCardAmountEdit").val());
        var claimAmt = outputNumber($("#idCOMLCAmtEdit").val());
        if (actualAmt < claimAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed card amount");
            showError();
            return false;
        }
        if (IsApplicantLevel == "true" && checkTransExists($("#idCOMCardNoEdit").val(), $("#idCOMTransIDEdit").val(), $("#idCOMSNoEdit").val())) {
            $("#" + fldModal).find('.modalInfo').text("Selected transaction : " + $("#idCOMTransIDEdit").val() + " is already used.");
            showError();
            return false;
        }
        var utilizedAmt = getUtilizedAmount($("#idCOMCardNoEdit").val(), $("#idCOMTransIDEdit").val(), $("#idCOMSNoEdit").val());
        if ((utilizedAmt + claimAmt) > actualAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed remaining amount for the selected transaction : " + outputMoney(actualAmt - utilizedAmt));
            showError();
            return false;
...
function formatCOM(tr) {
    var fldLabel = ["Card Type", "Card No", "Transaction ID", "Transaction", "Card Amount", "", "S.#", "LC Amt", "GL Code", "Expense Type", "GST Code", "Charge to Fact.", "FC Amt", "Taxable Amt", "Receipt #", "Curr.", "GST Amt", "", "Ex. Rate", "Cost Center", "", "Description"];
    var fldID = ["idCOMCardType", "idCOMCardNo", "idCOMTransID", "idCOMTransDetails", "idCOMCardAmount", "", "idCOMSNo", "idCOMLCAmt", "idCOMGLCode", "idCOMExpType", "idCOMGSTCodeDisplay", "idCOMChrgToFact", "idCOMFCAmt", "idCOMTaxAmt", "idCOMReceipt", "idCOMCurrency", "idCOMGSTAmt", "", "idCOMExRate", "idCOMCostCenterDisplay", "", "idCOMDesc"];
    var fldClass = ["", "", "", "", "amount", "", "", "amount", "", "", "", "", "amount", "amount", "", "", "amount", "", "amount", "", "", ""];
...
function setDADataEditModal() {
    //Set Row for editing
    var tr = objSelectedRow;
    var fldEditID = ["idDAIDEdit", "idDASNoEdit", "idDACountryEdit", "idDACityEdit", "idDAAllowTypeEdit", "idDADailyRateEdit", "idDANoofDaysEdit", "idDAActualNoofDaysEdit", "idDACBrkFastEdit", "idDACLnchEdit", "idDACDinnerEdit", "idDAFCAmtEdit", "idDACurrencyEdit", "idDAExrateEdit", "idDALCAmtEdit", "idDAGSTCodeEdit", "idDATaxAmtEdit", "idDAGSTAmtEdit", "idDACostCenterEdit", "idDAGLCodeEdit", "idDAChrgToFactEdit", "idDAReceiptNoEdit", "idDADescriptionEdit", "idDAStartDateEdit", "idDAEndDateEdit", "idDACostCenterDisplayEdit", "idDAGSTCodeDisplayEdit"]
    var fldID = ["idDAID", "idDASNo", "idDACountry", "idDACity", "idDAAllowType", "idDADailyRate", "idDANoofDays", "idDAActualNoofDays", "idDACBrkFast", "idDACLnch", "idDACDinner", "idDAFCAmt", "idDACurrency", "idDAExrate", "idDALCAmt", "idDAGSTCode", "idDATaxAmt", "idDAGSTAmt", "idDACostCenter", "idDAGLCode", "idDAChrgToFact", "idDAReceiptNo", "idDADescription", "idDAStartDate", "idDAEndDate", "idDACostCenterDisplay", "idDAGSTCodeDisplay"]
    var fldAccEditable = ["", "", "disabled", "disabled", "disabled", "readonly", "readonly", "readonly", "readonly", "readonly", "readonly", "", "disabled", "", "", "", "", "", "", "", "disabled", "", "readonly", "readonly", "readonly", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function generateDARow(strJSON, flgIntialLoading) {
    ...
    $.each(objJSON, function (i, item) {
        var Bfast, Lunch, Dinner, FCAmt, Currency, ExRate, LCAmt, CostCenter, CostCenterDisplay, Desc, Country, City, Days, AllowType, DARate, ActualDays, StartDate, EndDate, DescDisp, PreDesc;
        Bfast = Lunch = FCAmt = Dinner = Currency = ExRate = LCAmt = CostCenter = CostCenterDisplay = Desc = City = Days = Country = AllowType = DARate = ActualDays = StartDate = EndDate = DescDisp = PreDesc = "";
        var TaxableAmt = outputMoney(item.TaxableAmt);
        var GSTAmt = outputMoney(item.GSTAmt);
        if (travelStage == "pre") {
            Bfast = item.PreBfast;
...
function validateDA() {
    ...
    var fldName = ["Country", "City", "Allowance Type", "Comp. B'Fast", "Comp. Lunch", "Comp. Dinner", "No. of Days", "Actual No. Of Days", "DA Rate", "FC Amt", "Currency", "GST Code", "Taxable Amt", "Cost Center", "GL Code", "Charge To Fact"];
    var fldLanguageName = ["fldCountry", "fldCity", "fldAllowanceType", "fldCompBFast", "fldCompLunch", "fldCompDinner", "fldNoofDays", "fldActualNoOfDays", "fldDARate", "fldFCAmt", "fldCurr", "fldGSTCode", "fldTaxableAmt", "fldCostCenter", "fldGLCode", "fldChargeToFact"];
...
function formatDA(tr) {
    var fldLabel = ["S.#", "No.of Days", "GST Code", "Country", "Actual No. Of Days", "Taxable Amt", "City", "DA Rate", "GST Amt", "Allowance Type", "FC Amt", "Cost Center", "Comp. B'Fast", "Curr.", "GL Code", "Comp. Lunch", "Ex. Rate", "Charge To Fact.", "Comp. Dinner", "LC Amt", "Receipt #", "Description"];
    ...
    var fldClass = ["", "amount", "", "", "amount", "amount", "", "amount", "amount", "", "amount", "", "amount", "", "", "amount", "amount", "", "amount", "amount", "", ""];
    //added for multilanguage
    var fldLanguageLabel = ["fldSNo", "fldNoofDays", "fldGSTCode", "fldCountry", "fldActualNoOfDays", "fldTaxableAmt", "fldCity", "fldDARate", "fldGSTAmt", "fldAllowanceType", "fldFCAmt", "fldCostCenter", "fldCompBFast", "fldCurr", "fldGLCode", "fldCompLunch", "fldExchangeRate", "fldChargeToFact", "fldCompDinner", "fldLCAmt", "fldReceiptNo", "fldDescription"];
    //added for multilanguage
    strRowData = formatTravelFields(tr, fldLabel, fldID, fldClass,fldLanguageLabel, true);
    return strRowData;
}
...
function generateENTRow(strJSON, flgIntialLoading) {
    ...
    $.each(objJSON, function (i, item) {
        ...
        PreDesc = setDescDisplay(item.PreDesc);
        strNewRow = '<td class="details-control center vmiddle"><i class="fa fa-plus"></td>';
        strNewRow += '<td class="displaynone"><span class="datacol" prekey="ExpCode" postkey="ExpCode">Entertainment</span><span class="datacol pkey" prekey="ID" postkey="ID" id="idENTID">' + item.ID + '</span>' + item.ID + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol sno" prekey="SNo" postkey="SNo" id="idENTSNo">' + item.SNo + '</span>' + item.SNo + '</td>';
        ...
        strNewRow += '<td class="vmiddle displaynone"><span class="displaynone datacol" prekey="VendorNo" postkey="VendorNo" id="idENTVendor">' + item.VendorNo + '</span>' + item.VendorNo + '</td>';
        strNewRow += '<td class="vmiddle displaynone"><span class="displaynone datacol" prekey="InvoiceNo" postkey="InvoiceNo" id="idENTInvoice">' + item.InvoiceNo + '</span>' + item.InvoiceNo + '</td>';
        strNewRow += '<td class="vmiddle displaynone"><span class="displaynone datacol" prekey="BRNNo" postkey="BRNNo" id="idENTBRN">' + item.BRNNo + '</span>' + item.BRNNo + '</td>';
        // Transaction Details
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardType" postkey="CardType" id="idENTCardType">' + item.CardType + '</span>' + item.CardType + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardNo" postkey="CardNo" id="idENTCardNo">' + item.CardNo + '</span>' + item.CardNo + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionID" postkey="TransactionID" id="idENTTransID">' + item.TransactionID + '</span>' + item.TransactionID + '</td>';
        if (item.MerchantName != "" || item.TransactionDate != "")
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idENTTransDetails">' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</span>' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</td>';
        else
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idENTTransDetails">Select</span>Select</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="MerchantName" postkey="MerchantName" id="idENTMerchantName">' + item.MerchantName + '</span>' + item.MerchantName + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionDate" postkey="TransactionDate" id="idENTTransactionDate">' + item.TransactionDate + '</span>' + item.TransactionDate + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardAmount" postkey="CardAmount" id="idENTCardAmount">' + outputMoney(item.CardAmount) + '</span>' + outputMoney(item.CardAmount) + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionStatus" postkey="TransactionStatus" id="idENTTransactionStatus">' + item.TransactionStatus + '</span>' + item.TransactionStatus + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionTripType" postkey="TransactionTripType" id="idENTTransactionTripType">' + item.TransactionTripType + '</span>' + item.TransactionTripType + '</td>';
        strNewRow += strActBtns;
        if (flgIntialLoading) {
            strNewRow = "<tr>" + strNewRow + "</tr>";
...
function setENTDataEditModal() {
    //Set Row for editing
    var tr = objSelectedRow;
    var argCardType = tr.find("#idENTCardType").text();
    setCardTransaction(argCardType);
    var cardNos = tr.find("#idENTCardNo").text();
    getTransID(cardNos, argCardType);
    setTransaction(argCardType);
    setTransIDDetails('transID');
    setTransIDDetails('transfldslct');
    var fldEditID = ["idENTCardTypeEdit", "idENTCardNoEdit", "idENTTransIDEdit", "idENTTransDetailsEdit", "idENTCardAmountEdit", "idENTIDEdit", "idENTSNoEdit", "idENTCategoryEdit", "idENTPersonCountEdit", "idENTDateEdit", "idENTPlaceEdit", "idENTFCAmtEdit", "idENTCurrencyEdit", "idENTExRateEdit", "idENTLCAmtEdit", "idENTGSTCodeEdit", "idENTTaxableAmtEdit", "idENTPerHeadAmtEdit", "idENTGSTAmtEdit", "idENTCostCenterEdit", "idENTGLCodeEdit", "idENTChargeToFactEdit", "idENTReceiptEdit", "idENTVendorEdit", "idENTInvoiceEdit", "idENTBRNEdit", "idENTDescEdit", "idENTCostCenterDisplayEdit", "idENTGSTCodeDisplayEdit"]
    var fldID = ["idENTCardType", "idENTCardNo", "idENTTransID", "idENTTransDetails", "idENTCardAmount", "idENTID", "idENTSNo", "idENTCategory", "idENTPersonCount", "idENTDate", "idENTPlace", "idENTFCAmt", "idENTCurrency", "idENTExRate", "idENTLCAmt", "idENTGSTCode", "idENTTaxableAmt", "idENTPerHeadAmt", "idENTGSTAmt", "idENTCostCenter", "idENTGLCode", "idENTChargeToFact", "idENTReceipt", "idENTVendor", "idENTInvoice", "idENTBRN", "idENTDesc", "idENTCostCenterDisplay", "idENTGSTCodeDisplay"]
    var fldAccEditable = ["disabled", "disabled", "disabled", "disabled", "readonly", "", "", "disabled", "readonly", "readonly", "readonly", "", "disabled", "readonly", "", "", "", "readonly", "", "", "", "disabled", "", "", "", "", "readonly", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function getENTDataEditModal() {
    ...
    rowJSON["VendorNo"] = $("#idENTVendorEdit").val();
    rowJSON["InvoiceNo"] = $("#idENTInvoiceEdit").val();
    rowJSON["BRNNo"] = $("#idENTBRNEdit").val();
    ...
    if (travelStage == "pre") {
        ...
        // Transaction Details
        rowJSON["CardType"] = $("#idENTCardTypeEdit").val();
        rowJSON["CardNo"] = $("#idENTCardNoEdit").val();
        rowJSON["TransactionID"] = $("#idENTTransIDEdit").val();
        rowJSON["TransDetails"] = $("#idENTTransDetailsEdit").val();
        var TransDetails = $("#idENTTransDetailsEdit").val();
        if (TransDetails != null && TransDetails != undefined && TransDetails != "" && TransDetails != "Select") {
            TransDetails = TransDetails.split('/');
            rowJSON["MerchantName"] = TransDetails[TransDetails.length - 5].trim();
            rowJSON["TransactionDate"] = TransDetails[TransDetails.length - 4].trim();
            ...
            rowJSON["MerchantName"] = "";
            rowJSON["TransactionDate"] = "";
            rowJSON["CardAmount"] = $("#idENTCardAmountEdit").val();
            rowJSON["TransactionStatus"] = "";
            rowJSON["TransactionTripType"] = "";
        }
    }
...
function validateENT() {
    ...
    var fldName = ["Category", "Person Count", "Date", "Place", "FC Amt", "Currency", "GST Code", "Taxable Amount", "Per Head Amt", "Cost Center", "GL Code", "Charge To Factory"];
    var fldLanguageName = ["fldCategory", "fldPersonCount", "fldDate", "fldPlace", "fldFCAmt", "fldCurr", "fldGSTCode", "fldTaxableAmt", "fldPerHeadAmt", "fldCostCenter", "fldGLCode", "fldChargeToFact"];
    ...
    var cardtype = $("#" + fldModal).find($("select[fldTitle='CardType']"));
    if ((cardtype.val() == privateCardType || cardtype.val() == publicCardType) && travelStage == "post") {
        var actualAmt = outputNumber($("#idENTCardAmountEdit").val());
        var claimAmt = outputNumber($("#idENTLCAmtEdit").val());
        if (actualAmt < claimAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed card amount");
            showError();
            return false;
        }
        if (IsApplicantLevel == "true" && checkTransExists($("#idENTCardNoEdit").val(), $("#idENTTransIDEdit").val(), $("#idENTSNoEdit").val())) {
            $("#" + fldModal).find('.modalInfo').text("Selected transaction : " + $("#idENTTransIDEdit").val() + " is already used.");
            showError();
            return false;
        }
        var utilizedAmt = getUtilizedAmount($("#idENTCardNoEdit").val(), $("#idENTTransIDEdit").val(), $("#idENTSNoEdit").val());
        if ((utilizedAmt + claimAmt) > actualAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed remaining amount for the selected transaction : " + outputMoney(actualAmt - utilizedAmt));
            showError();
            return false;
...
function formatENT(tr) {
    var fldLabel = ["Card Type", "Card No", "Transaction ID", "Transaction", "Card Amount", "", "S.#", "Ex. Rate", "GL Code", "Category", "LC Amt", "Charge To Fact.", "Person Count", "GST Code", "Receipt #", "Date", "Taxable Amt", "Vendor #", "Place", "Per Head Amt", "Invoice #", "FC Amt", "GST Amt", "BRN #", "Curr.", "Cost Center", "", "Description"];
    var fldID = ["idENTCardType", "idENTCardNo", "idENTTransID", "idENTTransDetails", "idENTCardAmount", "", "idENTSNo", "idENTExRate", "idENTGLCode", "idENTCategory", "idENTLCAmt", "idENTChargeToFact", "idENTPersonCount", "idENTGSTCodeDisplay", "idENTReceipt", "idENTDate", "idENTTaxableAmt", "idENTVendor", "idENTPlace", "idENTPerHeadAmt", "idENTInvoice", "idENTFCAmt", "idENTGSTAmt", "idENTBRN", "idENTCurrency", "idENTCostCenterDisplay", "", "idENTDesc"];
    //added for multilanguage
    var fldLanguageLabel = ["fldCardType", "fldCardNo", "fldTransactionID", "fldTransaction", "fldCardAmount", "", "fldSNo", "fldExchangeRate", "fldGLCode", "fldCategory", "fldLCAmt", "fldChargeToFact", "fldPersonCount", "fldGSTCode", "fldReceiptNo", "fldDate", "fldTaxableAmt", "fldVendorNo", "fldPlace", "fldPerHeadAmt", "fldInvoiceNo", "fldFCAmt", "fldGSTAmt", "fldBRNNo", "fldCurr", "fldCostCenter", "", "fldDescription"];
    //added for multilanguage
    var fldClass = ["", "", "", "", "amount", "", "", "amount", "", "", "amount", "", "", "", "", "", "amount", "", "", "amount", "", "amount", "amount", "", "", "", "", ""];
    strRowData = formatTravelFields(tr, fldLabel, fldID, fldClass,fldLanguageLabel, true);
    return strRowData;
...
function generateENTDRow(strJSON, flgIntialLoading) {
    ...
    var strActBtns = "";
    $.each(objJSON, function (i, item) {
        strNewRow = '<td class="displaynone"><span class="datacol" prekey="ExpCode" postkey="ExpCode">EntertainmentDetails</span><span class="datacol pkey" id="idENTDID" prekey="ID" postkey="ID">' + item.ID + '</span>' + item.ID + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol sno" prekey="SNo" postkey="SNo" id="idENTDSNo">' + item.SNo + '</span>' + item.SNo + '</td>';
...
function formatENTD(tr) {
    ...
    var fldClass = ["", "", "", "", "", "", ""];
    var fldLanguageLabel = ["fldSNo", "fldCompName", "fldCustomerName", "fldAttendant", "fldTitle", "", "fldDescription"];
    strRowData = formatTravelFields(tr, fldLabel, fldID, fldClass,fldLanguageLabel, true);
    return strRowData;
}
function validateENTD() {
    ...
    var fldLanguageName = ["fldAttendant", "fldCompName", "fldTitle", "fldCustomerName"];
...
function validatePAS() {
    ...
    var fldName = ["Expense Types", "FC Amt", "Currency", "GST Code", "Taxable Amt", "Cost Center", "GL Code", "Charge To Fact"];
    var fldType = ["select", "number", "select", "select", "number", "select", "text", "select"];
    var isPreRow = isPreExpenseRow(objSelectedRow);
    if (isPreRow && travelStage == "post") { // To Allow user to submit with 0 LC Amount for Pre Detail Row
        fldType.forEach(function (type, idx, arr) {
...
function setPASDataEditModal() {
    ...
    var fldAccEditable = ["", "", "disabled", "", "disabled", "readonly", "", "", "", "", "", "", "disabled", "", "readonly", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
    $("#idPASCurrencyEdit").change();
}
function formatPAS(tr) {
    var fldLabel = ["S.#", "LC Amt", "GL Code", "Expense Type", "GST Code", "Charge To Fact.", "FC Amt", "Taxable Amt", "Receipt #", "Curr.", "GST Amt", "", "Ex. Rate", "Cost Center", "", "Description"];
...
function setPRPDataEditModal() {
    ...
    var fldAccEditable = ["", "", "disabled", "readonly", "", "", "disabled", "readonly", "", "", "", "", "", "", "readonly", "", "readonly", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
    $("#idPRPCurrencyEdit").change();
}
function formatPRP(tr) {
    var fldLabel = ["S.#", "Curr.", "GST Amt", "Expense Type", "Ex. Rate", "Cost Center", "Last Claim Date", "LC Amt", "GL Code", "Last Claim Amt", "GST Code", "Charge To Fact.", "FC Amt", "Taxable Amt", "Receipt #", "Description"];
...
function setTSPDataEditModal() {
    //Set Row for editing
    var tr = objSelectedRow;
    var argCardType = tr.find("#idTSPCardType").text();
    setCardTransaction(argCardType);
    var cardNos = tr.find("#idTSPCardNo").text();
    getTransID(cardNos, argCardType);
    setTransaction(argCardType);
    setTransIDDetails('transID');
    setTransIDDetails('transfldslct');
    //var fldEditID = ["idTSPIDEdit", "idTSPSnoEdit", "idTSPExpTypesEdit", "idTSPDateEdit", "idTSPModeEdit", "idTSPFromEdit", "idTSPToEdit", "idTSPFCAmtEdit", "idTSPCurrencyEdit", "idTSPExchRateEdit", "idTSPLCAmtEdit", "idTSPGSTCodeEdit", "idTSPTaxableAmtEdit", "idTSPGSTAmtEdit", "idTSPCostCenterEdit", "idTSPGlCodeEdit", "idTSPChargeToFactEdit", "idTSPReceiptcheckEdit", "idTSPReceiptDateEdit", "idTSPReceiptEdit", "idTSPVendorEdit", "idTSPInvoiceEdit", "idTSPBRNEdit", "idTSPInvoiceDateEdit", "idTSPDescriptionEdit", "idTSPCostCenterDisplayEdit", "idTSPGSTCodeDisplayEdit"]
    //var fldID = ["idTSPID", "idTSPSno", "idTSPExpenseTypes", "idTSPDate", "idTSPMode", "idTSPFrom", "idTSPTo", "idTSPFCAmt", "idTSPCurrency", "idTSPExchRate", "idTSPLCAmt", "idTSPGSTCode", "idTSPTaxableAmt", "idTSPGSTAmt", "idTSPCostCenter", "idTSPGlCode", "idTSPChargeToFactory", "idTSPReceiptcheck", "idTSPReceiptDate", "idTSPReceipt", "idTSPVendor", "idTSPInvoice", "idTSPBRN", "idTSPInvoiceDate", "idTSPDescription", "idTSPCostCenterDisplay", "idTSPGSTCodeDisplay"]
    var fldEditID = ["idTSPCardTypeEdit", "idTSPCardNoEdit", "idTSPTransIDEdit", "idTSPTransDetailsEdit", "idTSPCardAmountEdit", "idTSPIDEdit", "idTSPSnoEdit", "idTSPExpTypesEdit", "idTSPDateEdit", "idTSPModeEdit", "idTSPFromEdit", "idTSPToEdit", "idTSPFCAmtEdit", "idTSPCurrencyEdit", "idTSPExchRateEdit", "idTSPLCAmtEdit", "idTSPGSTCodeEdit", "idTSPTaxableAmtEdit", "idTSPGSTAmtEdit", "idTSPCostCenterEdit", "idTSPGlCodeEdit", "idTSPChargeToFactEdit", "idTSPReceiptcheckEdit", "idTSPReceiptEdit", "idTSPVendorEdit", "idTSPInvoiceEdit", "idTSPBRNEdit", "idTSPDescriptionEdit", "idTSPCostCenterDisplayEdit", "idTSPGSTCodeDisplayEdit"]
    var fldID = ["idTSPCardType", "idTSPCardNo", "idTSPTransID", "idTSPTransDetails", "idTSPCardAmount", "idTSPID", "idTSPSno", "idTSPExpenseTypes", "idTSPDate", "idTSPMode", "idTSPFrom", "idTSPTo", "idTSPFCAmt", "idTSPCurrency", "idTSPExchRate", "idTSPLCAmt", "idTSPGSTCode", "idTSPTaxableAmt", "idTSPGSTAmt", "idTSPCostCenter", "idTSPGlCode", "idTSPChargeToFactory", "idTSPReceiptcheck", "idTSPReceipt", "idTSPVendor", "idTSPInvoice", "idTSPBRN", "idTSPDescription", "idTSPCostCenterDisplay", "idTSPGSTCodeDisplay"]
    var fldAccEditable = ["disabled", "disabled", "disabled", "disabled", "readonly", "", "", "disabled", "readonly", "disabled", "readonly", "readonly", "", "disabled", "readonly", "", "", "", "", "", "", "disabled", "", "", "", "", "", "readonly", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function getTSPDataEditModal() {
    ...
    //rowJSON["InvoiceDate"] = $("#idTSPInvoiceDateEdit").val();
    ...
    // Transaction Details
    rowJSON["CardType"] = $("#idTSPCardTypeEdit").val();
    rowJSON["CardNo"] = $("#idTSPCardNoEdit").val();
    rowJSON["TransactionID"] = $("#idTSPTransIDEdit").val();
    rowJSON["TransDetails"] = $("#idTSPTransDetailsEdit").val();
    var TransDetails = $("#idTSPTransDetailsEdit").val();
    if (TransDetails != null && TransDetails != undefined && TransDetails != "" && TransDetails != "Select") {
        TransDetails = TransDetails.split('/');
        rowJSON["MerchantName"] = TransDetails[TransDetails.length - 5].trim();
        rowJSON["TransactionDate"] = TransDetails[TransDetails.length - 4].trim();
        ...
        rowJSON["MerchantName"] = "";
        rowJSON["TransactionDate"] = "";
        rowJSON["CardAmount"] = $("#idTSPCardAmountEdit").val();
        rowJSON["TransactionStatus"] = "";
        rowJSON["TransactionTripType"] = "";
    }
    objJSON.push(rowJSON);
...
function generateTSPRow(strJSON, flgIntialLoading) {
    ...
    $.each(objJSON, function (i, item) {
        ...
        var DescDisp = setDescDisplay(item.PostDesc);
        strNewRow = '<td class="details-control center vmiddle"><i class="fa fa-plus"></td>';
        strNewRow += '<td class="displaynone"><span class="datacol" prekey="ExpCode" postkey="ExpCode">Transportation</span><span class="datacol pkey" id="idTSPID" prekey="ID" postkey="ID">' + item.ID + '</span>' + item.ID + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol sno" id="idTSPSno" prekey="SNo" postkey="SNo">' + item.SNo + '</span>' + item.SNo + '</td>';
        ...
        strNewRow += '<td class="vmiddle displaynone"><span class="displaynone datacol" id="idTSPVendor" prekey="VendorNo" postkey="VendorNo">' + item.VendorNo + '</span>' + item.VendorNo + '</td>';
        strNewRow += '<td class="vmiddle displaynone"><span class="displaynone datacol" id="idTSPInvoice" prekey="InvoiceNo" postkey="InvoiceNo">' + item.InvoiceNo + '</span>' + item.InvoiceNo + '</td>';
        strNewRow += '<td class="vmiddle displaynone"><span class="displaynone datacol" id="idTSPBRN" prekey="BRNNo" postkey="BRNNo">' + item.BRNNo + '</span>' + item.BRNNo + '</td>';
        //strNewRow += '<td class="vmiddle displaynone"><span class="displaynone datacol" id="idTSPInvoiceDate" prekey="InvoiceDate" postkey="InvoiceDate">' + item.InvoiceDate + '</span>' + item.InvoiceDate + '</td>';
        // Transaction Details
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardType" postkey="CardType" id="idTSPCardType">' + item.CardType + '</span>' + item.CardType + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardNo" postkey="CardNo" id="idTSPCardNo">' + item.CardNo + '</span>' + item.CardNo + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionID" postkey="TransactionID" id="idTSPTransID">' + item.TransactionID + '</span>' + item.TransactionID + '</td>';
        if (item.MerchantName != "" || item.TransactionDate != "")
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idTSPTransDetails">' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</span>' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</td>';
        else
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idTSPTransDetails">Select</span>Select</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="MerchantName" postkey="MerchantName" id="idTSPMerchantName">' + item.MerchantName + '</span>' + item.MerchantName + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionDate" postkey="TransactionDate" id="idTSPTransactionDate">' + item.TransactionDate + '</span>' + item.TransactionDate + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardAmount" postkey="CardAmount" id="idTSPCardAmount">' + outputMoney(item.CardAmount) + '</span>' + outputMoney(item.CardAmount) + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionStatus" postkey="TransactionStatus" id="idTSPTransactionStatus">' + item.TransactionStatus + '</span>' + item.TransactionStatus + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionTripType" postkey="TransactionTripType" id="idTSPTransactionTripType">' + item.TransactionTripType + '</span>' + item.TransactionTripType + '</td>';
...
function validateTSP() {
    ...
    var fldName = ["Expense Type", "Date", "Mode", "From", "To", "FC Amt", "Currency", "Exch. Rate", "LC Amt", "GST Code", "Taxable Amt", "Cost Center", "GL Code", "Charge To Factory", "Receipt"];
    var fldLanguageName = ["fldExpenseType", "fldDate", "fldMode", "fldFrom", "fldTo", "fldFCAmt", "fldCurr", "fldExchangeRate", "fldLCAmt", "fldGSTCode", "fldTaxableAmt", "fldCost Center", "fldGLCode", "fldChargeToFact", "fldReceipt"];
    ...
    var cardtype = $("#" + fldModal).find($("select[fldTitle='CardType']"));
    if ((cardtype.val() == privateCardType || cardtype.val() == publicCardType) && travelStage == "post") {
        var actualAmt = outputNumber($("#idTSPCardAmountEdit").val());
        var claimAmt = outputNumber($("#idTSPLCAmtEdit").val());
        if (actualAmt < claimAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed card amount");
            showError();
            return false;
        }
        if (IsApplicantLevel == "true" && checkTransExists($("#idTSPCardNoEdit").val(), $("#idTSPTransIDEdit").val(), $("#idTSPSNoEdit").val())) {
            $("#" + fldModal).find('.modalInfo').text("Selected transaction : " + $("#idTSPTransIDEdit").val() + " is already used.");
            showError();
            return false;
        }
        var utilizedAmt = getUtilizedAmount($("#idTSPCardNoEdit").val(), $("#idTSPTransIDEdit").val(), $("#idTSPSNoEdit").val());
        if ((utilizedAmt + claimAmt) > actualAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed remaining amount for the selected transaction : " + outputMoney(actualAmt - utilizedAmt));
            showError();
            return false;
...
function formatTSP(tr) {
    //var fldLabel = ["S.#", "Exch. Rate", "Receipt Check?", "Expense Type", "LC Amt", "Receipt Date", "Date", "GST Code", "Receipt #", "Mode", "Taxable Amt", "Vendor #", "From", "GST Amt", "Invoice #", "To", "Cost Center", "BRN #", "FC Amt", "GL Code", "Invoice Date", "Curr.", "Charge To Fact.", "", "Description"];
    //var fldID = ["idTSPSno", "idTSPExchRate", "idTSPReceiptcheck", "idTSPExpenseTypes", "idTSPLCAmt", "idTSPReceiptDate", "idTSPDate", "idTSPGSTCodeDisplay", "idTSPReceipt", "idTSPMode", "idTSPTaxableAmt", "idTSPVendor", "idTSPFrom", "idTSPGSTAmt", "idTSPInvoice", "idTSPTo", "idTSPCostCenterDisplay", "idTSPBRN", "idTSPFCAmt", "idTSPGlCode", "idTSPInvoiceDate", "idTSPCurrency", "idTSPChargeToFactory", "", "idTSPDescription"];
    //var fldWidth = ["15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15"];
    //var fldClass = ["", "amount", "", "", "amount", "", "", "", "", "", "amount", "", "", "amount", "", "", "", "", "amount", "", "", "", "", "", ""];
    //var fldClass = ["", "amount", "", "", "amount", "", "", "", "amount", "", "amount", "", "", "amount", "", "", "", "", "amount", "", "", "", "", "", ""];
    var fldLabel = ["Card Type", "Card No", "Transaction ID", "Transaction", "Card Amount", "", "S.#", "Exch. Rate", "Receipt Check?", "Expense Type", "LC Amt", "Date", "GST Code", "Receipt #", "Mode", "Taxable Amt", "Vendor #", "From", "GST Amt", "Invoice #", "To", "Cost Center", "BRN #", "FC Amt", "GL Code", "Curr.", "Charge To Fact.", "", "Description"];
    var fldID = ["idTSPCardType", "idTSPCardNo", "idTSPTransID", "idTSPTransDetails", "idTSPCardAmount", "", "idTSPSno", "idTSPExchRate", "idTSPReceiptcheck", "idTSPExpenseTypes", "idTSPLCAmt", "idTSPDate", "idTSPGSTCodeDisplay", "idTSPReceipt", "idTSPMode", "idTSPTaxableAmt", "idTSPVendor", "idTSPFrom", "idTSPGSTAmt", "idTSPInvoice", "idTSPTo", "idTSPCostCenterDisplay", "idTSPBRN", "idTSPFCAmt", "idTSPGlCode", "idTSPCurrency", "idTSPChargeToFactory", "", "idTSPDescription"];
    var fldWidth = ["15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15", "15"];
    var fldClass = ["", "", "", "", "amount", "", "", "amount", "", "", "amount", "", "", "amount", "", "amount", "", "", "amount", "", "", "", "", "amount", "", "", "", "", ""];
    var fldLanguageLabel = ["fldCardType", "fldCardNo", "fldTransactionID", "fldTransaction", "fldCardAmount", "", "fldSNo", "fldExchangeRate", "fldReceiptCheck", "fldExpenseType", "fldLCAmt", "fldDate", "fldGSTCode", "fldReceiptNo", "fldMode", "fldTaxableAmt", "fldVendorNo", "fldFrom", "fldGSTAmt", "fldInvoiceNo", "fldTo", "fldCostCenter", "fldBRNNo", "fldFCAmt", "fldGLCode", "fldCurr", "fldChargeToFact", "", "fldDescription"];
    strRowData = formatTravelFields(tr, fldLabel, fldID, fldClass, fldLanguageLabel,false);
    return strRowData;
}
//Transportation End		  
...
function setCADataEditModal() {
    ...
    var fldAccEditable = ["", "", "readonly", "readonly", "readonly", "readonly", "readonly", "readonly", "readonly", "", "disabled", "readonly", "", "", "", "", "", "", "disabled", "", "readonly", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
    $("#idCACrrEdit").change();
}
...
function validateCA() {
    ...
    var fldName = ["Details", "From Date", "From Time", "To Date", "To Time", "Days", "Max. Allowable Allowance", "FC Amt", "Currency", "Exchange. Rate", "LC Amt", "GST Code", "Taxable Amt", "Cost Center", "GL Code", "Charge To Factory"];
...
function formatCA(tr) {
    var fldLabel = ["S.#", "Max. Allowable Allowance", "GST Amt", "Details", "FC Amt", "Cost Center", "From Date", "Curr.", "GL Code", "From Time", "Exch. Rate", "Charge To Fact.", "To Date", "LC Amt", "Receipt #", "To Time", "GST Code", "", "Days", "Taxable Amt", "", "Description"];
...
function setTRNCDataEditModal() {
    ...
    var fldAccEditable = ["", "", "readonly", "", "disabled", "readonly", "", "", "", "", "", "", "disabled", "", "readonly", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function validateTRNC() {
    ...
    var fldName = ["Details", "FC Amt", "Currency", "Exch. Rate", "LC Amt", "GST Code", "Taxable Amt", "Cost Center", "GL Code", "Charge To Factory"];
...
function formatTRNC(tr) {
    var fldLabel = ["S.#", "LC Amt", "GL Code", "Details", "GST Code", "Charge To Fact.", "FC Amt", "Taxable Amt", "Receipt #", "Curr.", "GST Amt", "", "Exch. Rate", "Cost Center", "", "Description"];
...
function setEXBDataEditModal() {
    ...
    var fldAccEditable = ["", "", "", "readonly", "readonly", "", "readonly", "disabled", "", "", "", "disabled", "", "readonly", "disabled", "", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function validateEXB() {
    ...
    var fldName = ["Flight No", "Excess Weight (Kg)", "FC Amount", "Currency", "Expense Rate", "LC Amount", "GST Code", "Taxable Amount", "Cost Center", "GL Code", "Charge To Factory"];
...
function formatEXB(tr) {
    var fldLabel = ["S.#", "Ex. Rate", "Cost Center", "Flight No", "LC Amt", "GL Code", "Excess Wt(Kg)", "GST Code", "Charge To Fact.", "FC Amt", "Taxable Amt", "Receipt #", "Curr.", "GST Amt", "", "Description"];
...
function setLANDataEditModal() {
    ...
    var fldAccEditable = ["", "", "readonly", "", "disabled", "readonly", "", "", "", "", "disabled", "", "disabled", "", "readonly", "", ""];
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function validateLAN() {
    ...
    var fldName = ["Date", "FC Amount", "Currency", "Expense Rate", "LC Amount", "GST Code", "Taxable Amount", "Cost Center", "GL Code", "Charge To Factory"];
    var fldType = ["text", "number", "select", "number", "number", "select", "number", "select", "text", "select"];
    var isPreRow = isPreExpenseRow(objSelectedRow);
    if (isPreRow && travelStage == "post") { // To Allow user to submit with 0 LC Amount for Pre Detail Row
        fldType.forEach(function (type, idx, arr) {
...
function formatLAN(tr) {
    var fldLabel = ["S.#", "LC Amt", "GL Code", "Date", "GST Code", "Charge To Fact.", "FC Amt", "Taxable Amt", "Receipt #", "Curr.", "GST Amt", "", "Ex. Rate", "Cost Center", "", "Description"];
...
function setAPTDataEditModal() {
    //Set Row for editing
    var tr = objSelectedRow;
    var argCardType = tr.find("#idAPTCardType").text();
    setCardTransaction(argCardType);
    var cardNos = tr.find("#idAPTCardNo").text();
    getTransID(cardNos, argCardType);
    setTransaction(argCardType);
    setTransIDDetails('transID');
    setTransIDDetails('transfldslct');
    var fldEditID = ["idAPTCardTypeEdit", "idAPTCardNoEdit", "idAPTTransIDEdit", "idAPTTransDetailsEdit", "idAPTCardAmountEdit", "idAPTIDEdit", "idAPTSNoEdit", "idAPTAirportNameEdit", "idAPTClaimCurrEdit", "idAPTClaimAmtEdit", "idAPTExRateEdit", "idAPTLCAmtEdit", "idAPTCostCenterEdit", "idAPTGSTCodeEdit", "idAPTTaxableAmtEdit", "idAPTGSTAmtEdit", "idAPTGLCodeEdit", "idAPTReceiptNoEdit", "idAPTChargeToFactoryEdit", "idAPTExpDescEdit", "idAPTVendorNoEdit", "idAPTInvoiceNoEdit", "idAPTBRNNoEdit", "idAPTTaxArgByEdit", "idAPTSubTypeEdit", "idAPTCostCenterDisplayEdit", "idAPTGSTCodeDisplayEdit"]
    var fldID = ["idAPTCardType", "idAPTCardNo", "idAPTTransID", "idAPTTransDetails", "idAPTCardAmount", "idAPTID", "idAPTSNo", "idAPTAirportName", "idAPTClaimCurr", "idAPTClaimAmt", "idAPTExRate", "idAPTLCAmt", "idAPTCostCenter", "idAPTGSTCode", "idAPTTaxableAmt", "idAPTGSTAmt", "idAPTGLCode", "idAPTReceiptNo", "idAPTChargeToFactory", "idAPTExpDesc", "idAPTVendorNo", "idAPTInvoiceNo", "idAPTBRNNo", "idAPTTaxArgBy", "idAPTSubType", "idAPTCostCenterDisplay", "idAPTGSTCodeDisplay"]
    var fldAccEditable = ["disabled", "disabled", "disabled", "disabled", "readonly", "", "", "readonly", "disabled", "", "readonly", "", "", "", "", "", "", "", "disabled", "readonly", "", "", "", "", "", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function generateAPTRow(strJSON, flgIntialLoading) {
    ...
    $.each(objJSON, function (i, item) {
        ...
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol" prekey="VendorNo" postkey="VendorNo"  id="idAPTVendorNo">' + item.VendorNo + '</span>' + item.VendorNo + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol" prekey="InvoiceNo" postkey="InvoiceNo"  id="idAPTInvoiceNo">' + item.InvoiceNo + '</span>' + item.InvoiceNo + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol" prekey="BRNNo" postkey="BRNNo"  id="idAPTBRNNo">' + item.BRNNo + '</span>' + item.BRNNo + '</td>';
        ...
        // Transaction Details
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardType" postkey="CardType" id="idAPTCardType">' + item.CardType + '</span>' + item.CardType + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardNo" postkey="CardNo" id="idAPTCardNo">' + item.CardNo + '</span>' + item.CardNo + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionID" postkey="TransactionID" id="idAPTTransID">' + item.TransactionID + '</span>' + item.TransactionID + '</td>';
        if (item.MerchantName != "" || item.TransactionDate != "")
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idAPTTransDetails">' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</span>' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</td>';
        else
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idAPTTransDetails">Select</span>Select</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="MerchantName" postkey="MerchantName" id="idAPTMerchantName">' + item.MerchantName + '</span>' + item.MerchantName + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionDate" postkey="TransactionDate" id="idAPTTransactionDate">' + item.TransactionDate + '</span>' + item.TransactionDate + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardAmount" postkey="CardAmount" id="idAPTCardAmount">' + outputMoney(item.CardAmount) + '</span>' + outputMoney(item.CardAmount) + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionStatus" postkey="TransactionStatus" id="idAPTTransactionStatus">' + item.TransactionStatus + '</span>' + item.TransactionStatus + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionTripType" postkey="TransactionTripType" id="idAPTTransactionTripType">' + item.TransactionTripType + '</span>' + item.TransactionTripType + '</td>';
...
function getAPTDataEditModal() {
    ...
    rowJSON["VendorNo"] = $("#idAPTVendorNoEdit").val();
    rowJSON["InvoiceNo"] = $("#idAPTInvoiceNoEdit").val();
    rowJSON["BRNNo"] = $("#idAPTBRNNoEdit").val();
    rowJSON["PostTktArrangedBy"] = $("#idAPTTaxArgByEdit").val();
    rowJSON["PostExpTypes"] = $("#idAPTSubTypeEdit").val();
    // Transaction Details
    rowJSON["CardType"] = $("#idAPTCardTypeEdit").val();
    rowJSON["CardNo"] = $("#idAPTCardNoEdit").val();
    rowJSON["TransactionID"] = $("#idAPTTransIDEdit").val();
    rowJSON["TransDetails"] = $("#idAPTTransDetailsEdit").val();
    var TransDetails = $("#idAPTTransDetailsEdit").val();
    if (TransDetails != null && TransDetails != undefined && TransDetails != "" && TransDetails != "Select") {
        TransDetails = TransDetails.split('/');
        rowJSON["MerchantName"] = TransDetails[TransDetails.length - 5].trim();
        rowJSON["TransactionDate"] = TransDetails[TransDetails.length - 4].trim();
        ...
        rowJSON["MerchantName"] = "";
        rowJSON["TransactionDate"] = "";
        rowJSON["CardAmount"] = $("#idAPTCardAmountEdit").val();
        rowJSON["TransactionStatus"] = "";
        rowJSON["TransactionTripType"] = "";
    }
    objJSON.push(rowJSON);
...
function validateAPT() {
    ...
    var fldName = ["Expense Type", "Airport Name", "FC Amount", "Currency", "GST Code", "Taxable Amount", "Cost Center", "GL Code", "Charge to Fact.", "Arranged By"];
    var fldLanguageName = ["fldExpenseType", "fldAirportName", "fldFCAmt", "fldCurr", "fldGSTCode", "fldTaxableAmt", "fldCostCenter", "fldGLCode", "fldChargeToFact", "fldArrangedBy"];
    ...
    var cardtype = $("#" + fldModal).find($("select[fldTitle='CardType']"));
    if ((cardtype.val() == privateCardType || cardtype.val() == publicCardType) && travelStage == "post") {
        var actualAmt = outputNumber($("#idAPTCardAmountEdit").val());
        var claimAmt = outputNumber($("#idAPTLCAmtEdit").val());
        if (actualAmt < claimAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed card amount");
            showError();
            return false;
        }
        if (IsApplicantLevel == "true" && checkTransExists($("#idAPTCardNoEdit").val(), $("#idAPTTransIDEdit").val(), $("#idAPTSNoEdit").val())) {
            $("#" + fldModal).find('.modalInfo').text("Selected transaction : " + $("#idAPTTransIDEdit").val() + " is already used.");
            showError();
            return false;
        }
        var utilizedAmt = getUtilizedAmount($("#idAPTCardNoEdit").val(), $("#idAPTTransIDEdit").val(), $("#idAPTSNoEdit").val());
        if ((utilizedAmt + claimAmt) > actualAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed remaining amount for the selected transaction : " + outputMoney(actualAmt - utilizedAmt));
            showError();
            return false;
...
function formatAPT(tr) {
    var fldLabel = ["Card Type", "Card No", "Transaction ID", "Transaction", "Card Amount", "", "S.#", "LC Amt", "Charge to Fact.", "Sub Type", "GST Code", "Arranged By", "Airport Name", "Taxable Amt", "Receipt #", "FC Amt", "GST Amt", "Vendor #", "Curr.", "Cost Center", "Invoice #", "Ex. Rate", "GL Code", "BRN #", "Description"];
    var fldID = ["idAPTCardType", "idAPTCardNo", "idAPTTransID", "idAPTTransDetails", "idAPTCardAmount", "", "idAPTSNo", "idAPTLCAmt", "idAPTChargeToFactory", "idAPTSubType", "idAPTGSTCodeDisplay", "idAPTTaxArgBy", "idAPTAirportName", "idAPTTaxableAmt", "idAPTReceiptNo", "idAPTClaimAmt", "idAPTGSTAmt", "idAPTVendorNo", "idAPTClaimCurr", "idAPTCostCenterDisplay", "idAPTInvoiceNo", "idAPTExRate", "idAPTGLCode", "idAPTBRNNo", "idAPTExpDesc"];
    var fldClass = ["", "", "", "", "amount", "", "", "amount", "", "", "", "", "", "amount", "", "amount", "amount", "", "", "", "", "amount", "", "", ""];
    //added for multilanguage
    var fldLanguageLabel = ["fldCardType", "fldCardNo", "fldTransactionID", "fldTransaction", "fldCardAmount", "", "fldSNo", "fldLCAmt", "fldChargeToFact", "fldExpenseType", "fldGSTCode", "fldArrangedBy", "fldAirportName", "fldTaxableAmt", "fldReceiptNo", "fldFCAmt", "fldGSTAmt", "fldVendorNo", "fldCurr", "fldCostCenter", "fldInvoiceNo", "fldExchangeRate", "fldGLCode", "fldBRNNo", "fldDescription"];
    //added for multilanguage
    strRowData = formatTravelFields(tr, fldLabel, fldID, fldClass,fldLanguageLabel, false);
    return strRowData;
}
...
function setGIFTDataEditModal() {
    //Set Row for editing
    var tr = objSelectedRow;
    var argCardType = tr.find("#idGIFTCardType").text();
    setCardTransaction(argCardType);
    var cardNos = tr.find("#idGIFTCardNo").text();
    getTransID(cardNos, argCardType);
    setTransaction(argCardType);
    setTransIDDetails('transID');
    setTransIDDetails('transfldslct');
    var fldEditID = ["idGIFTCardTypeEdit", "idGIFTCardNoEdit", "idGIFTTransIDEdit", "idGIFTTransDetailsEdit", "idGIFTCardAmountEdit", "idGIFTIDEdit", "idGIFTSNoEdit", "idGIFTDetailsEdit", "idGIFTTypeEdit", "idGIFTDateEdit", "idGIFTFCAmtEdit", "idGIFTCurrencyEdit", "idGIFTExRateEdit", "idGIFTLCAmtEdit", "idGIFTGSTCodeEdit", "idGIFTTaxAmtEdit", "idGIFTGSTAmtEdit", "idGIFTCostCenterEdit", "idGIFTGLCodeEdit", "idGIFTDescEdit", "idGIFTCostCenterDisplayEdit", "idGIFTGSTCodeDisplayEdit", "idGIFTChargeToFactoryEdit", "idGIFTeFormRefNoEdit"]
    var fldID = ["idGIFTCardType", "idGIFTCardNo", "idGIFTTransID", "idGIFTTransDetails", "idGIFTCardAmount", "idGIFTID", "idGIFTSNo", "idGIFTDetails", "idGIFTType", "idGIFTDate", "idGIFTFCAmt", "idGIFTCurrency", "idGIFTExRate", "idGIFTLCAmt", "idGIFTGSTCode", "idGIFTTaxAmt", "idGIFTGSTAmt", "idGIFTCostCenter", "idGIFTGLCode", "idGIFTDesc", "idGIFTCostCenterDisplay", "idGIFTGSTCodeDisplay", "idGIFTChargeToFactory", "idGIFTeFormRefNo"]
    var fldAccEditable = ["disabled", "disabled", "disabled", "disabled", "readonly", "", "", "readonly","", "readonly", "", "disabled", "readonly", "", "", "", "", "", "", "readonly", "", "", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function generateGIFTRow(strJSON, flgIntialLoading) {
    ...
    $.each(objJSON, function (i, item) {
        var Details, ExpDate, FCAmt, Currency, ExRate, LCAmt, CostCenter, Desc, TaxableAmt, GSTAmt, DescDisp, PreDesc, eFrmRefNo;
        Details = ExpDate = FCAmt = Currency = ExRate = LCAmt = CostCenter = Desc = TaxableAmt = GSTAmt = DescDisp = PreDesc = eFrmRefNo = "";
        if (travelStage == "pre") {
            Details = item.PreDetails;
            ExpType = item.PreCategory;
            ExpDate = item.PreExpDate;
            FCAmt = outputMoney(item.PreFCAmt);
            Currency = item.PreCurrency;
            ExRate = outputMoneyEx(item.PreExRate);
            ...
            CostCenterDisplay = item.PreCostCenterDisplay;
            Desc = item.PreDesc;
            eFrmRefNo = item.PreeFormRefNo;
        }
        else {
            Details = item.PostDetails;
            ExpType = item.PostCategory;
            ExpDate = item.PostExpDate;
            FCAmt = outputMoney(item.PostFCAmt);
            Currency = item.PostCurrency;
            ExRate = outputMoneyEx(item.PostExRate);
        ...
        strNewRow = '<td class="details-control center vmiddle"><i class="fa fa-plus"></td>';
        strNewRow += '<td class="displaynone"><span class="datacol" prekey="ExpCode" postkey="ExpCode">Gift</span><span class="datacol pkey" prekey="ID" postkey="ID" id="idGIFTID">' + item.ID + '</span>' + item.ID + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol sno" prekey="SNo" postkey="SNo" id="idGIFTSNo">' + item.SNo + '</span>' + item.SNo + '</td>';
        strNewRow += '<td class="predisplay vmiddle">' + item.PreDetails + '</td>';
        ...
        strNewRow += Details + '</td>';
        strNewRow += '<td class="predisplay vmiddle">' + item.PreCategory + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol" prekey="PreCategory" postkey="PostCategory" id="idGIFTType" prevalue="' + item.PreCategory + '">' + ExpType + '</span>';
        strNewRow += ExpType + '</td>';
        strNewRow += '<td class="predisplay vmiddle">' + item.PreExpDate + '</td>';
        ...
        strNewRow += '<td class="displaynone vmiddle">' + item.PreeFormRefNo + '</td>';
        strNewRow += '<td class="displaynone"><span class="displaynone datacol" prekey="PreeFormRefNo" postkey="PosteFormRefNo" id="idGIFTeFormRefNo" prevalue="' + item.PreeFormRefNo + '">' + eFrmRefNo + '</span>';
        strNewRow += eFrmRefNo + '</td>';
        // Transaction Details
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardType" postkey="CardType" id="idGIFTCardType">' + item.CardType + '</span>' + item.CardType + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardNo" postkey="CardNo" id="idGIFTCardNo">' + item.CardNo + '</span>' + item.CardNo + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionID" postkey="TransactionID" id="idGIFTTransID">' + item.TransactionID + '</span>' + item.TransactionID + '</td>';
        if (item.MerchantName != "" || item.TransactionDate != "")
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idGIFTTransDetails">' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</span>' + item.MerchantName + ' / ' + item.TransactionDate + ' / ' + outputMoney(item.CardAmount) + ' / ' + item.TransactionStatus + ' / ' + item.TransactionTripType + '</td>';
        else
            strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransDetails" postkey="TransDetails" id="idGIFTTransDetails">Select</span>Select</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="MerchantName" postkey="MerchantName" id="idGIFTMerchantName">' + item.MerchantName + '</span>' + item.MerchantName + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionDate" postkey="TransactionDate" id="idGIFTTransactionDate">' + item.TransactionDate + '</span>' + item.TransactionDate + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="CardAmount" postkey="CardAmount" id="idGIFTCardAmount">' + outputMoney(item.CardAmount) + '</span>' + outputMoney(item.CardAmount) + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionStatus" postkey="TransactionStatus" id="idGIFTTransactionStatus">' + item.TransactionStatus + '</span>' + item.TransactionStatus + '</td>';
        strNewRow += '<td class="displaynone vmiddle"><span class="displaynone datacol"  prekey="TransactionTripType" postkey="TransactionTripType" id="idGIFTTransactionTripType">' + item.TransactionTripType + '</span>' + item.TransactionTripType + '</td>';
...
function getGIFTDataEditModal() {
    ...
    if (travelStage == "pre") {
        ...
        rowJSON["PreCategory"] = $("#idGIFTTypeEdit").val();
        ...
        rowJSON["PreeFormRefNo"] = $("#idGIFTeFormRefNoEdit").val();
        rowJSON["CardType"] = "Select";
        rowJSON["CardNo"] = "Select";
        rowJSON["TransactionID"] = "Select";
        rowJSON["TransDetails"] = "Select";
        ...
        rowJSON["PreCategory"] = ($("#idGIFTTypeEdit").attr("prevalue")) == undefined || ($("#idGIFTTypeEdit").attr("prevalue")) == "" ? "NA" : $("#idGIFTTypeEdit").attr("prevalue");
        rowJSON["PostCategory"] = $("#idGIFTTypeEdit").val();
        ...
        rowJSON["PreeFormRefNo"] = ($("#idGIFTeFormRefNoEdit").attr("prevalue")) == undefined || ($("#idGIFTeFormRefNoEdit").attr("prevalue")) == "" ? "NA" : $("#idGIFTeFormRefNoEdit").attr("prevalue");
        rowJSON["PosteFormRefNo"] = $("#idGIFTeFormRefNoEdit").val();
        // Transaction Details
        rowJSON["CardType"] = $("#idGIFTCardTypeEdit").val();
        rowJSON["CardNo"] = $("#idGIFTCardNoEdit").val();
        rowJSON["TransactionID"] = $("#idGIFTTransIDEdit").val();
        rowJSON["TransDetails"] = $("#idGIFTTransDetailsEdit").val();
        var TransDetails = $("#idGIFTTransDetailsEdit").val();
        if (TransDetails != null && TransDetails != undefined && TransDetails != "" && TransDetails != "Select") {
            TransDetails = TransDetails.split('/');
            rowJSON["MerchantName"] = TransDetails[TransDetails.length - 5].trim();
            rowJSON["TransactionDate"] = TransDetails[TransDetails.length - 4].trim();
            ...
            rowJSON["MerchantName"] = "";
            rowJSON["TransactionDate"] = "";
            rowJSON["CardAmount"] = $("#idGIFTCardAmountEdit").val();
            rowJSON["TransactionStatus"] = "";
            rowJSON["TransactionTripType"] = "";
        }
    }
...
function validateGIFT() {
    ...
    var fldID = ["idGIFTDetailsEdit", "idGIFTTypeEdit", "idGIFTDateEdit", "idGIFTFCAmtEdit", "idGIFTCurrencyEdit", "idGIFTGSTCodeEdit", "idGIFTTaxAmtEdit", "idGIFTCostCenterEdit", "idGIFTGLCodeEdit", "idGIFTChargeToFactoryEdit"];
    var fldName = ["Details","Expense Type", "Date", "FC Amt", "Currency", "GST Code", "Taxable Amount", "Cost Center", "GL Code", "Charge To Factory"];
    var fldLanguageName = ["fldDetails","fldExpenseType", "fldDate", "fldFCAmt", "fldCurr", "fldGSTCode", "fldTaxableAmt", "fldCostCenter", "fldGLCode", "fldChargeToFact"];
    var fldType = ["text","select", "text", "number", "select", "select", "number", "select", "text", "select"];
    var isPreRow = isPreExpenseRow(objSelectedRow);
    if (isPreRow && travelStage == "post") { // To Allow user to submit with 0 LC Amount for Pre Detail Row
        fldType.forEach(function (type, idx, arr) {
            if (type == "number") {
    ...
    if (travelStage == "post") {
        var efrmRefNo = $("#" + fldModal).find($("input[fldTitle='eFormRefNo']"));
        if (efrmRefNo.val().trim() == "") {
            $("#" + fldModal).find('.modalInfo').text("Please enter eForm Ref No");
            efrmRefNo.focus();
            showError();
            return false;
        }
    ...
    var cardtype = $("#" + fldModal).find($("select[fldTitle='CardType']"));
    if ((cardtype.val() == privateCardType || cardtype.val() == publicCardType) && travelStage == "post") {
        var actualAmt = outputNumber($("#idGIFTCardAmountEdit").val());
        var claimAmt = outputNumber($("#idGIFTLCAmtEdit").val());
        if (actualAmt < claimAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed card amount");
            showError();
            return false;
        }
        if (IsApplicantLevel == "true" && checkTransExists($("#idGIFTCardNoEdit").val(), $("#idGIFTTransIDEdit").val(), $("#idGIFTSNoEdit").val())) {
            $("#" + fldModal).find('.modalInfo').text("Selected transaction : " + $("#idGIFTTransIDEdit").val() + " is already used.");
            showError();
            return false;
        }
        var utilizedAmt = getUtilizedAmount($("#idGIFTCardNoEdit").val(), $("#idGIFTTransIDEdit").val(), $("#idGIFTSNoEdit").val());
        if ((utilizedAmt + claimAmt) > actualAmt) {
            $("#" + fldModal).find('.modalInfo').text("LC Amt should not exceed remaining amount for the selected transaction : " + outputMoney(actualAmt - utilizedAmt));
            showError();
            return false;
...
function formatGIFT(tr) {
    var fldLabel = ["Card Type", "Card No", "Transaction ID", "Transaction", "Card Amount", "", "S.#", "Curr.", "GST Amt", "Details", "Ex. Rate", "Cost Center", "Expense Type", "LC Amt", "GL Code", "Date", "GST Code", "Charge To Fact.", "FC Amt", "Taxable Amt", "eForm Ref No", "Description"];
    var fldID = ["idGIFTCardType", "idGIFTCardNo", "idGIFTTransID", "idGIFTTransDetails", "idGIFTCardAmount", "", "idGIFTSNo", "idGIFTCurrency", "idGIFTGSTAmt", "idGIFTDetails", "idGIFTExRate", "idGIFTCostCenterDisplay", "idGIFTType", "idGIFTLCAmt", "idGIFTGLCode", "idGIFTDate", "idGIFTGSTCodeDisplay", "idGIFTChargeToFactory", "idGIFTFCAmt", "idGIFTTaxAmt", "idGIFTeFormRefNo", "idGIFTDesc"];
    var fldClass = ["", "", "", "", "amount", "", "", "", "amount", "", "amount", "", "", "amount", "", "", "", "", "amount", "amount", "", ""];
    //added for multilanguage
    var fldLanguageLabel = ["fldCardType", "fldCardNo", "fldTransactionID", "fldTransaction", "fldCardAmount", "", "fldSNo", "fldCurr", "fldGSTAmt", "fldDetails", "fldExchangeRate", "fldCostCenter", "fldExpenseType", "fldLCAmt", "fldGLCode", "fldDate", "fldGSTCode", "fldChargeToFact", "fldFCAmt", "fldTaxableAmt", "fldeFormRefNo", "fldDescription"];
    //added for multilanguage
    strRowData = formatTravelFields(tr, fldLabel, fldID, fldClass, fldLanguageLabel,true);
    return strRowData;
}
...
function generateGIFTDRow(strJSON, flgIntialLoading) {
    ...
    var strActBtns = "";
    $.each(objJSON, function (i, item) {
        strNewRow = '<td class="displaynone"><span class="datacol" prekey="ExpCode" postkey="ExpCode">GiftDetails</span><span class="datacol pkey" id="idGIFTDID" prekey="ID" postkey="ID">' + item.ID + '</span>' + item.ID + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol sno" prekey="SNo" postkey="SNo" id="idGIFTDSNo">' + item.SNo + '</span>' + item.SNo + '</td>';
...
function validateGIFTD() {
    ...
    var fldLanguageName = ["fldTitle", "fldCompanyAttendant", "fldCompName", "fldCustomerName"];
    var fldType = ["text", "text", "text", "text"];
...
function generateTSHRow(strJSON, flgIntialLoading) {
    ...
    $.each(objJSON, function (i, item) {
        ...
        //PIDSKR
        if (IsApplicantLevel == "true") {// && (travelStage != "post" || $("[fldtitle=UrgentTrip]").find("input[type=radio]:checked").val() == "Yes")
            ...
            if ((travelStage == "post" && $("[fldtitle=UrgentTrip]").find("input[type=radio]:checked").val() == "No")) {
                btncolor = "grey";
                btndisable = "disabled";
            }
...
function validateTSH() {
    ...
    var fldLanguageName = ["fldDepCountry", "fldDepCity", "fldDestCountry", "fldDestCity", "fldDepDate", "fldDepTime", "fldArrTime", "fldTransportMode", "fldTransitDep"];
...
function setTSHDataEditModal() {
    ...
    //Disable all fields except city for PIDSKR in post stage
    if (travelStage == "post" && $("[fldtitle=UrgentTrip]").find("input[type=radio]:checked").val() == "No") {
        $('#TSHModal').find("input,textarea").prop("disabled", true);
        $('#TSHModal').find("#idTSHDepCntryEdit,#idTSHDestCntryEdit,#idTSHTransModeEdit,#idTSHisTravelledEdit").prop("disabled", true);
    }
    //$("#idTSHDepCntryEdit").multiselect($("#idTSHDepCntryEdit")[0].disabled ? 'disable' : 'enable');
    //$("#idTSHDestCntryEdit").multiselect($("#idTSHDestCntryEdit")[0].disabled ? 'disable' : 'enable');
}
function formatTSH(tr) {
    ...
    var fldClass = ["", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""];
    //added for multilanguage
    var fldLanguageLabel = ["fldSNo", "fldDepDate", "fldTransportMode", "fldDepCountry", "fldDepTime", "fldFlightNo", "fldDepCity", "fldArrDate", "fldAdditionalDay", "fldArrCountry", "fldArrTime", "fldCancelTrip", "fldArrCity", "NA", "NA", "fldDescription"];
    //added for multilanguage
    strRowData = formatTravelFields(tr, fldLabel, fldID, fldClass, fldLanguageLabel, true);
    return strRowData;
}
...
function generateACCENTRow(strJSON, flgIntialLoading) {
    ...
    $.each(objJSON, function (i, item) {
        var PostLCAmt = outputMoney(item.PostLCAmt);
        strNewRow = '<td class="displaynone"><span class="datacol" prekey="ExpCode" postkey="ExpCode">AccountingEntry</span><span class="datacol pkey" id="idACCENTID" prekey="ID" postkey="ID">' + item.ID + '</span>' + item.ID + '</td>';
        strNewRow += '<td class="vmiddle"><span class="datacol displaynone sno" prekey="SNo" postkey="SNo" id="idACCENTSNo">' + item.SNo + '</span>' + item.SNo + '</td>';
...
function setADVDataEditModal() {
    //Set Row for editing
    var tr = objSelectedRow;
    var fldEditID = ["idADVIDEdit", "idADVSNoEdit", "idADVCurrEdit", "idADVModeEdit", "idADVPCEdit", "idADVDenominationEdit", "idADVFCAmtEdit", "idADVBaseCurrAmtEdit"];
    var fldID = ["idADVID", "idADVSNo", "idADVCurr", "idADVMode", "idADVPC", "idADVDenomination", "idADVFCAmt", "idADVBaseCurrAmt"];
    var fldAccEditable = ["readonly", "readonly", "disabled", "disabled", "readonly", "readonly", "readonly", "readonly"];
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function validateADV() {
    ...
    var fldID = ["idADVCurrEdit", "idADVModeEdit", "idADVPCEdit", "idADVDenominationEdit", "idADVFCAmtEdit"];
    var fldName = ["Currency", "Mode", "PC", "Denomination", "FC Amt"];
    var fldLanguageName = ["fldCurrency", "fldMode", "fldPC", "fldDenomination", "fldFCAmt"];
    var fldType = ["select", "select", "text", "text", "number"];
...
function validateRef() {
    ...
    var fldLanguageName = ["fldCurrency", "fldMode", "fldFCAmt"];
...
function showHideAdvTable(flgShow) {
    if (flgShow) {
        ...
        //$(".AccAdvAmtSection").show();
        $("input[fldTitle='hdnAdvanceApplied']").val("Yes");
    }
    else {
        $("#advanceDataTable").hide();
        $("#lnkPrintAdvance").hide();
        //$(".AccAdvAmtSection").hide();
...
function addExpenseType(argForDA) {
    ...
    EETotAmt = outputNumber($("input[fldTitle='hdnEETotAmt']").val());
    if (expType == "Entertainment" && travelStage == "post" && (EETotAmt == 0 || docStatus == "Draft")) {
        //alertModal("Cannot claim Entertainment without pre travel approval");
        alertModalML("8");
        return false;
    }
    ACCTotAmt = outputNumber($("input[fldTitle='hdnACCTotAmt']").val());
    if (expType == "Accommodation" && travelStage == "post" && (ACCTotAmt == 0 || docStatus == "Draft")) {
        alertModal("Cannot claim Accommodation without pre travel approval");
        //alertModalML("8");
        return false;
    }
    APTTotAmt = outputNumber($("input[fldTitle='hdnAPTTotAmt']").val());
    if (expType == "AirportTax" && travelStage == "post" && (APTTotAmt == 0 || docStatus == "Draft")) {
        alertModal("Cannot claim Airport Tax without pre travel approval");
        //alertModalML("8");
        return false;
    }
    ...
    if (GLCode_TaxCode == "NA" || GLCode_TaxCode == "~") {
        ...
        alertModalML("9", { "<<EXPENSETYPE>>": expTypeDisplay });
        return false
    }
...
function loadDataTables() {
    ...
    $('.btnCreditCardAdd').on('click', function (e) {
        e.preventDefault();
    });
    loadTravelEntries();
    traverseRow();
...
function getTravelEntriesJSON(listName, linkKey, fnCallBack) {
    ...
    var DateCols = ["PreDepDate", "PreArrDate", "PostDepDate", "PostArrDate", "LastClaimDate", "FromDate", "ToDate", "PreExpDate", "PostExpDate", "PreStartDate", "PostStartDate", "PreEndDate", "PostEndDate"];
    $.ajax
    ({
...
function CustomValidateFields(argModal) {
    var gstCode = $("#" + argModal).find($("select[fldTitle='GST/VATCode']")).val();
    var objVendorNo = $("#" + argModal).find($("input[fldTitle='VendorNumber']"));
    var objInvoiceNo = $("#" + argModal).find($("input[fldTitle='InvoiceNumber']"));
    var objBRNNo = $("#" + argModal).find($("input[fldTitle='BRNNumber']"));
    if ($("input[fldTitle='hdnVendorBRNInvoiceApplicable']").val() == "Yes" && (gstCode == "1B" || gstCode == "E6")) {
        if (objVendorNo.val() == "") {
            $("#" + argModal).find('.modalInfo').text("Vendor # Cannot be blank");
            objVendorNo.focus();
            showError();
            return false;
        }
        if (objInvoiceNo.val() == "") {
            $("#" + argModal).find('.modalInfo').text("Invoice # Cannot be blank");
            objInvoiceNo.focus();
            showError();
            return false;
        }
        if (objBRNNo.val() == "") {
            $("#" + argModal).find('.modalInfo').text("BRN # Cannot be blank");
            objBRNNo.focus();
            showError();
            return false;
        }
...
function getTravelType() {
    ...
    if (listData.length == 0) {
        ...
        alertModalML("13", "<<TRAVELLINGTO>>", TravellingTo);
        return false;
    }
...
function getGLCode(argTravelType, argCheckedVal) {
    ...
    if (TravellingTo != "" && argTravelType != "" && argCheckedVal == true) {
        ...
        if (listData.length == 0) {
            ...
            alertModalML("15", { "<<TRAVELLING TO>>": TravellingTo, "<<TRAVELTYPE>>": argTravelType });
            return false;
        }
...
function winterAllowance(callingFrom) {
    ...
    if (duration < 0) {
        ...
        alertModalML("23", { "<<EXPENSETYPE>>": winExp });
        return false;
    }
...
function alertModalML(argKey, argSearchVal) {
    captionTxt = ("WRN: #ETRAVEL" + argKey + "<br/>" + langalertArray[argKey]);
    captionTxt = !!argSearchVal ? (captionTxt.replace(/<<[^<>]+>>/ig, function (mp) { return !!argSearchVal[mp] ? argSearchVal[mp] : mp })) : captionTxt;
...
function refreshDA() {
    ...
    if (objDA == null) {
        urlForDA = currentSiteUrl + "/_api/web/lists/getByTitle('DailyAllowanceType')/Items?$select=" + regularAllowance.concat(oneDayAir, additionalDetails).join(",") + "&"
        filterURL = "$filter=Country/Description eq'" + Country + "' and CityName/Description eq null";
        filterURL += " and (Division/Title eq'" + division + "' or Division/Title eq'" + Alldivision + "') and TravellingTo eq'" + TravellingTo + "' and AllowanceType/DAType eq'" + aType + "' and StaffGrade/StaffGrade eq'" + staffGrade + "' and EffectiveStartDate le'" + daDepDate + "' and EffectiveEndDate ge'" + daDepDate + "'&$top=1";
        urlForDA = urlForDA + filterURL;
        $.ajax({
            url: urlForDA,
...
function setExpDetails() {
    ...
    var items = getData(currentSiteUrl + "/_api/web/lists/getByTitle('ExpenseType')/Items?$select=ExpenseType,ExpenseTypeDisplay,GLCodeRefersTo,PostToHRIQ");
    items = JSON.parse(items);
    items.forEach(function (item) {
        ExpenseDetails.expensesToList.push(item.ExpenseType);
        ExpenseDetails[item.ExpenseType] = {
            "displayName": item.ExpenseTypeDisplay,
...
function getTravelInfoBudget() {
    ...
    if (budgetType.toLowerCase() == "iaf budget") {
        ...
        getBudgetData = getData(rootSiteUrl + "/_api/web/lists/getByTitle('BudgetMaster')/Items?$filter=ProfitCenterID/ProfitCenterId eq'" + CostCenter + "' and GLCode eq'" + GLCode + "' and FinancialYear eq'" + finYear + "'");
    }
    if (travelStage == "pre") {
...
function getTravelExpBudget() {
    ...
    $.each(tempbudgetSumColl, function (i, item) {
        ...
        var getBudgetData = getData(rootSiteUrl + "/_api/web/lists/getByTitle('BudgetMaster')/Items?$filter=ProfitCenterID/ProfitCenterId eq'" + CostCenter + "' and GLCode eq'" + GLCode + "' and FinancialYear eq'" + finYear + "'");
...
function getFinancialCurrencyRate() {
    ...
    if (FinCurrencyRate.length == 0) {
        ...
        alertModalML("22",{"<<FYYEAR>>":finYear});
        return;
    }
...
function prepAllowance(callingFrom) {
    ...
    switch (PAType) {
        ...
        default:
            ...
            alertModalML("23",{"<<EXPENSETYPE>>":prepExp});
    }
    return false;
}
...
function prepByBaggage(callingFrom) {
    ...
    if (allowAmount <= 0) {
        //alertModal(prepExp + " cannot be claimed. Please contact administrator.");
        alertModal("23", { "<<EXPENSETYPE>>": prepExp });
        return false;
    }
    if (callingFrom == "modal") {
...
function setConditionalFlds() {
    ...
    drawDataTbl(tblID, false);
    //Accomodation Claim - Total Amount
    tblID = "idTblACC"
    drawDataTbl(tblID, true);
    TotAmt = 0;
    $(tableRowSelector(tblID)).each(function () {
    ...
    $("input[fldTitle='hdnACCTotAmt']").val(TotAmt);
    drawDataTbl(tblID, false);
    //Airport Tax Claim - Total Amount
    tblID = "idTblAPT"
    drawDataTbl(tblID, true);
    TotAmt = 0;
    $(tableRowSelector(tblID)).each(function () {
    ...
    $("input[fldTitle='hdnAPTTotAmt']").val(TotAmt);
    drawDataTbl(tblID, false);
    //DA Country
    tblID = "idTblDA"
    drawDataTbl(tblID, true);
...
function isProperSchedule() {
    ...
    for (var i = 0; i < dep.length; i++) {
        if (roundTrip) {
            ...
            alertModalML("24",{"<<STARTEDFROM>>":startedFrom,"<<TRIPS>>":trips[i]});
            return false;
        } else {
            ...
            //PIDSKR
            if (dep[i] == dest[i]) {
            alertModal((TravellingTo == "Domestic" ? "Local" : "Overseas") + " trips made at '" + dep[i] + "' (" + trips[i] + ") is not allowed in " + TravellingTo + " trip. Please correct the schedule before submission.");
            return false;
            }
        }
    }
    if (!roundTrip) {
        ...
        alertModalML("25",{"<<STARTEDFROM>>":startedFrom,"<<ARRIVALAT>>":startedFrom});
        return false;
    }
    return true;
}
...
function getPreviousTrips() {
    ...
    var listData = getData(currentSiteUrl + "/_api/web/lists/getByTitle('" + appLstName + "')/Items?$select=ID,DocRefNo,TravellingTo,DACountry,DACity,PostDeptDate,PostArrDate&$filter=TravelStage eq 'post' and DocumentStatus eq 'Approved' and ApplicantID eq '" + empID + "'&$OrderBy=Created desc&$top=10");
    listData = JSON.parse(listData);
    var strTripsHTML = '<a href="javascript:void(0)" class="closebtn" onclick="showMyTrips(false);">&times;</a>';
    strTripsHTML += '<span class="mytripheader">'+ langArray["fldMyPRVTrips"]+ '(' + listData.length + ')</span>';
    if (listData.length == 0) {
        strTripsHTML += '<div style="top: 15px; text-align: center; position: relative;">'+langArray["fldNoTripsavailable"]+'</div>';
...
function enableDisableExRateTab() {
    ...
    if (advApplied == "Yes") {
       ...
       // $(".AccAdvAmtSection").show();
        $("#lnkPrintAdvance").show();
       // if (travelStage == "pre" && isAccountsLevel == "Yes") {
       //     $("input[fldTitle='AccAdvAmt']").attr("disabled", false);
       // }
        if (travelStage == "post") {
...
function getLatestExchRateDate(exchangeRateType) {
    if (exchangeRateType == "House Rate") {
...
function displayExRateTable() {
    ...
    var strRow = "";
    if (travelStage == "pre") {
        strRow = "<tr><td>" + langArray["fldExchangeRateOn"] + "</td><td class='center'>" + strPreExDate + "</td></tr>";
    ...
    strRow = "<tr><td>"+langArray["fldExchangeRateOn"]+"</td><td class='center'>" + strPreExDate + "</td><td class='center'>" + strPostExDate + "</td></tr>";
...
function calculateGSTAmt(objGST, idLcAmt, idTaxAmt, idGSTAmt, modelId) {
    ...
    if (GSTCodes[GSTCode]) {
        ...
        $("#" + modelId).find('.modalInfo').text("GST Rate not found. Please Contact Administrator");
        showError();
        return;
    }
}
function reCalcGST(idLCAmt, idGetAmt, idSetAmt, modelId) {
    ...
    if (getAmt > LCAmt) {
        objLCAmt.change();
        $("#" + modelId).find('.modalInfo').text("Taxable Amount / GST Amount should be within LC Amount.Taxable Amount & GST Amount has been recalculated");
        showError();
        return;
    }
...
function getPrivateCardDetails() {
    var CurrDocNo = strDocRefNo == "0" ? null : "'" + strDocRefNo + "'";
    var cardNoDatacoll = getData(rootSiteUrl + "/_api/web/lists/getByTitle('StaffCardDetails')/Items?$select=CardNo/CardNo&$expand=CardNo/CardNo&$filter=EmployeeID/EmployeeID eq'" + empID + "'");
    cardNoDatacoll = JSON.parse(cardNoDatacoll);
    var cardNoColl = '';
    if (cardNoDatacoll.length <= 0) {
        return;
    }
    else {
        for (var i = 0; i < cardNoDatacoll.length; i++) {
            if (cardNoColl == '') {
                cardNoColl = "'" + cardNoDatacoll[i].CardNo.CardNo + "'";
            }
            else {
                cardNoColl += ' or CARD_NO eq ' + "'" + cardNoDatacoll[i].CardNo.CardNo + "'";
            }

        }
        var urlCC = rootSiteUrl + "/_api/web/lists/getByTitle('CreditCardDetails')/Items?$filter=(CardType eq'" + privateCardType + "' and (CARD_NO eq " + cardNoColl + ")and (ReferenceDoc eq " + CurrDocNo + " or ReferenceDoc eq null) and (Status eq 'Approved'))";
        var objJSON = [];
        var rowSno = $("#idCardDtlsRowNo").text();
        totalHeadCnt = 0;
        $.ajax({
            url: urlCC,
            type: "GET",
            ...
            headers: { "accept": "application/json;odata=verbose" },
            success: function (data) {
                privateCardDetails = data.d.results;
            },
            error: function (error) {
                console.log(JSON.stringify(error));
            }
...
function getPublicCardDetails() {
    var CurrDocNo = strDocRefNo == "0" ? null : "'" + strDocRefNo + "'";
    var urlCC = rootSiteUrl + "/_api/web/lists/getByTitle('CreditCardDetails')/Items?$filter=CardType eq'" + publicCardType + "' and EmployeeID/EmployeeID eq '" + empID + "'and (ReferenceDoc eq " + CurrDocNo + " or ReferenceDoc eq null) and (Status eq 'Approved')";
    $.ajax({
        url: urlCC,
        type: "GET",
        async: false,
        headers: { "accept": "application/json;odata=verbose" },
        success: function (data) {
            publicCardDetails = data.d.results;
        },
        error: function (error) {
            console.log(JSON.stringify(error));
        }
...
function generateCardDetails(argCardDetails) {
    var objJSON = [];
    $('#idTblCreditCard tbody tr').remove();
    var cardData = (argCardDetails == publicCardType) ? publicCardDetails : (argCardDetails == privateCardType) ? privateCardDetails : '';
    if (cardData == '') {
        return false;
    }
    if (cardData.length > 0) {
        $.each(cardData, function (key, value) {
            var rowJSON = {};
            rowJSON["CardNo"] = value.CARD_NO;
            rowJSON["TransactionID"] = value.APV_NO;
            rowJSON["MerchantName"] = value.MEST_NM == null ? "NA" : value.MEST_NM;
            rowJSON["TransactionDate"] = value.APV_DT;
            rowJSON["CardAmount"] = value.BUY_SUM;
            rowJSON["TransactionStatus"] = value.Status;
            rowJSON["TransactionTripType"] = value.TripType == null ? "NA" : value.TripType;
            objJSON.push(rowJSON);
        });
    }
    bindControlCardDetails(objJSON);
}

function bindControlCardDetails(objJSON) {
    var strNewRow = "";
    if (objJSON != undefined) {
        $.each(objJSON, function (i, item) {

            strNewRow = '<td class="chk_control center vmiddle"><input id="ccchkbox" type="checkbox" name="ccrowcheckbox" class="CCAllocchkCls"></td>';
            strNewRow += '<td class="vmiddle"><span  key="CardNo" id="idCardNo">' + item.CardNo + '</span></td>';
            strNewRow += '<td class="vmiddle"><span  key="TransactionID" id="idTransactionID">' + item.TransactionID + '</span></td>';
            strNewRow += '<td class="vmiddle"><span  key="MerchantName" id="idMerchantName">' + item.MerchantName + '</span> </td>';
            strNewRow += '<td class="vmiddle"><span  key="TransactionDate" id="idTransactionDate">' + item.TransactionDate + '</span> </td>';
            strNewRow += '<td class="amount vmiddle" id="idCardAmount"><span  key="CardAmount" id="idCardAmount">' + outputMoney(item.CardAmount) + '</span></td>';
            strNewRow += '<td class="vmiddle"><span  key="TransactionStatus" id="TransactionStatus">' + item.TransactionStatus + '</span> </td>';
            strNewRow += '<td class="vmiddle"><span  key="TransactionTripType" id="TransactionTripType">' + item.TransactionTripType + '</span> </td>';
            $('#idTblCreditCard tbody').append("<tr>" + strNewRow + "</tr>");
        });
    }

}
function addCreditCardEntries() {
    var creditCardDtlsArray = [];
    var cardType = $("#CreditCardModal").find("select[fldTitle='CardType']").val();
    var modalID = CurrCardAddModalId;
    var idPrefix = arrExpID[0][modalID];
    var tblID = arrTblID[0][modalID];
    var transDetails = "";
    if (cardType == "Select") 
    {
        $("#CreditCardModal").find('.modalInfo').text("Please select the Card Type");
        $("#CreditCardModal").find("select[fldTitle='CardType']").focus();
        showError();
        return false;
    }
    else {
        creditCardDtlsArray = getCardArray();
    }
    if (creditCardDtlsArray.length == 0) {
        $("#CreditCardModal").find('.modalInfo').text("Please select the Card Details");
        showError();
        return false;
    }
    if (creditCardDtlsArray.length > 0) {
        if (cardType == publicCardType) {
            //$('#' + modalID).find('#' + idPrefix + 'TransDetailsEdit').html($("#ddlPublicTransDetails").html());
            //$('#' + modalID).find('#' + idPrefix + 'TransIDEdit').html($("#ddlPublicTransID").html());
            $('#' + modalID).find('#' + idPrefix + 'CardNoEdit').html($("#ddlPublicCardNo").html());
            $.each(creditCardDtlsArray, function (i, item) {
                var cardNos = item.CardNo;
                getTransID(cardNos, cardType);
                setTransaction(cardType);
            });
        }
        else {
           // $('#' + modalID).find('#' + idPrefix + 'TransDetailsEdit').html($("#ddlPrivateTransDetails").html());
            //$('#' + modalID).find('#' + idPrefix + 'TransIDEdit').html($("#ddlPrivateTransID").html());
            $('#' + modalID).find('#' + idPrefix + 'CardNoEdit').html($("#ddlPrivateCardNo").html());
            $.each(creditCardDtlsArray, function (i, item) {
                var cardNos = item.CardNo;
                getTransID(cardNos, cardType);
                setTransaction(cardType);
    ...
    if (creditCardDtlsArray.length > 0) {
        $.each(creditCardDtlsArray, function (i, item) {
            transDetails = item.MerchantName + " / " + item.TransactionDate + " / " + item.CardAmount + " / " + item.TransactionStatus + " / " + item.TransactionTripType;
            isCreditCardRow = true;
            addNewItem(modalID, tblID)
            $('#' + idPrefix + 'CardTypeEdit').val(cardType);
            $('#' + idPrefix + 'CardNoEdit').val(item.CardNo);
            $('#' + idPrefix + 'TransIDEdit').val(item.TransactionID);
            $('#' + idPrefix + 'TransDetailsEdit').val(transDetails);
            $('#' + idPrefix + 'CardAmountEdit').val(item.CardAmount);
            updateRow(tblID, modalID);
            isCreditCardRow = false;
        });
    }
    $("#CreditCardModal").modal('hide');
}
function getCardArray() {
    return ($('#idTblCreditCard>tbody>tr').find("input[type=checkbox]:checked").map(function () {
        var trow = $(this).closest("tr");
        return {
            CardNo: $.trim(trow.find("span[key=CardNo]").text()),
            TransactionID: $.trim(trow.find("span[key=TransactionID]").text()),
            MerchantName: $.trim(trow.find("span[key=MerchantName]").text()),
            TransactionDate: $.trim(trow.find("span[key=TransactionDate]").text()),
            CardAmount: $.trim(trow.find("span[key=CardAmount]").text()),
            TransactionStatus: $.trim(trow.find("span[key=TransactionStatus]").text()),
            TransactionTripType: $.trim(trow.find("span[key=TransactionTripType]").text())
        }
    }).toArray());
}
function loadCardTransaction() {
    var cardNos = '';
    var existingcards = '';
    var cardNosOption = '<option value="Select" selected>Select</option>';
    if (publicCardDetails.length > 0) {
        $.each(publicCardDetails, function (key, value) {
            cardNos = value.CARD_NO;
            if (existingcards.indexOf(cardNos) == -1) {
                cardNosOption += '<option value="' + cardNos + '">' + cardNos + '</option>';
                existingcards += cardNos + ",";
            }
        });
    }
    $("select[fldTitle*='ddlPublicCardNo']").html(cardNosOption);
    existingcards = '';
    cardNosOption = '<option value="Select" selected>Select</option>';
    if (privateCardDetails.length > 0) {
        $.each(privateCardDetails, function (key, value) {
            cardNos = value.CARD_NO;
            if (existingcards.indexOf(cardNos) == -1) {
                cardNosOption += '<option value="' + cardNos + '">' + cardNos + '</option>';
                existingcards += cardNos + ",";
   ...
   // $("select[fldTitle*='ddlPrivateTransDetails']").html(transDetailsOption);
   // $("select[fldTitle*='ddlPrivateTransID']").html(transIDOption);
    $("select[fldTitle*='ddlPrivateCardNo']").html(cardNosOption);
}
function getTransID(cardNos, argCardType) {
    var transID = "";
    var transDetails = "";
    var transDetailsOption = '<option value="Select" selected>Select</option>';
    var transIDOption = '<option value="Select" selected>Select</option>';
    if (argCardType == "Public Corporate Card") {
        if (publicCardDetails.length > 0) {
            $.each(publicCardDetails, function (i, item) {
                if (cardNos == item.CARD_NO) {
                    transDetails = (item.MEST_NM == null ? "NA" : item.MEST_NM) + " / " + item.APV_DT + " / " + outputMoney(item.BUY_SUM) + " / " + item.Status + " / " + (item.TripType == null ? "NA" : item.TripType);
                    transID = item.APV_NO;
                    transDetailsOption += '<option value="' + transDetails + '">' + transDetails + '</option>';
                    transIDOption += '<option value="' + transID + '">' + transID + '</option>';
                }
            });
        }
        $("select[fldTitle*='ddlPublicTransDetails']").html(transDetailsOption);
        $("select[fldTitle*='ddlPublicTransID']").html(transIDOption);
    }
    if (argCardType == "Private Corporate Card") {
        if (privateCardDetails.length > 0) {
            $.each(privateCardDetails, function (key, item) {
                if (cardNos == item.CARD_NO) {
                    transDetails = (item.MEST_NM == null ? "NA" : item.MEST_NM) + " / " + item.APV_DT + " / " + outputMoney(item.BUY_SUM) + " / " + item.Status + " / " + (item.TripType == null ? "NA" : item.TripType);
                    transID = item.APV_NO;
                    transDetailsOption += '<option value="' + transDetails + '">' + transDetails + '</option>';
...
function setCardTransaction(argCardType) {
    var modalID = objSelectedRow.find('.editrow').attr('modalid');
    $('#' + modalID).find(".CardAmount").val("0.00");
    $('#' + modalID).find('.transfldslct').html('<option value="Select" selected="selected">Select</option>');
    $('#' + modalID).find('.transID').html('<option value="Select" selected="selected">Select</option>');
    $('#' + modalID).find($("select[fldtitle='CardNo'],select[fldtitle='TransactionID'],select[fldtitle='Transaction'],input[fldtitle='CardAmount']")).attr("disabled", false);
    if (argCardType == publicCardType) {
        $('#' + modalID).find('.cardno').html($("#ddlPublicCardNo").html());
        $('#' + modalID).find('select.currency').val(baseCurrency);
        $('#' + modalID).find($("select[title='Currency']")).attr("disabled", true);
    }
    else if (argCardType == privateCardType) {
        $('#' + modalID).find('.cardno').html($("#ddlPrivateCardNo").html());
        $('#' + modalID).find('select.currency').val(baseCurrency);
        $('#' + modalID).find($("select[title='Currency']")).attr("disabled", true);
    }
    else {
        $('#' + modalID).find('.cardno').html('<option value="Select" selected="selected">Select</option>');
        $('#' + modalID).find($("select[title='Currency']")).attr("disabled", false);
        $('#' + modalID).find($("select[fldtitle='CardNo'],select[fldtitle='TransactionID'],select[fldtitle='Transaction'],input[fldtitle='CardAmount']")).attr("disabled", true);
        $('#' + modalID).find($("input[title='Date']")).val("");
        $('#' + modalID).find($("input[title='Date']")).attr("disabled", false);
    }
}
function setTransaction(argCardType) {
    var modalID = objSelectedRow.find('.editrow').attr('modalid');
    if (argCardType == publicCardType) {
        $('#' + modalID).find('.transfldslct').html($("#ddlPublicTransDetails").html());
        $('#' + modalID).find('.transID').html($("#ddlPublicTransID").html());
    }
    else if (argCardType == privateCardType) {
        $('#' + modalID).find('.transfldslct').html($("#ddlPrivateTransDetails").html());
        $('#' + modalID).find('.transID').html($("#ddlPrivateTransID").html());
    }
    else {
        $('#' + modalID).find(".CardAmount").val("0.00");
        $('#' + modalID).find('.transfldslct').html('<option value="Select" selected="selected">Select</option>');
...
function ValidateCreditCardsFields(argModal) {
    var cardtype = $("#" + argModal).find($("select[fldTitle='CardType']"));
    var cardno = $("#" + argModal).find($("select[fldTitle='CardNo']"));
    var transactionid = $("#" + argModal).find($("select[fldTitle='TransactionID']"));
    var transaction = $("#" + argModal).find($("select[fldTitle='Transaction']"));
    var cardamt = $("#" + argModal).find($("input[fldTitle='CardAmount']"));
    if (cardtype.val() == "" || cardtype.val() == "Select") {
        $("#" + argModal).find('.modalInfo').text("Please select"+langArray["fldCardType"]);
        cardtype.focus();
        showError();
        return false;
    }
    if (cardtype.val() != privateCardType && cardtype.val() != publicCardType) {
        return true;
    }
    if (cardno.val() == "" || cardno.val() == "Select" || cardno.val() == null) {
        $("#" + argModal).find('.modalInfo').text("Please select"+langArray["fldCardNumber"]);
        cardno.focus();
        showError();
        return false;
    }
    if (transactionid.val() == "" || transactionid.val() == "Select" || cardno.val() == null) {
        $("#" + argModal).find('.modalInfo').text("Please select " + langArray["fldTransactionID"]);
        transactionid.focus();
        showError();
        return false;
    }
    if (transaction.val() == "" || transaction.val() == "Select" || cardno.val() == null) {
        $("#" + argModal).find('.modalInfo').text("Please select" + langArray["fldTransaction"]);
        transaction.focus();
        showError();
        return false;
    }
    if (cardamt.val() == "") {
        $("#" + argModal).find('.modalInfo').text(langArray["fldCardAmount"] + "Cannot be blank");
        cardamt.focus();
        showError();
        return false;
    }
...
function setTransIDDetails(argData) {
    var getClsID = argData;
    var setClsID = (argData == 'transID') ? 'transfldslct' : 'transID'
    var modalID = objSelectedRow.find('.editrow').attr('modalid');
    var objget = $('#' + modalID).find('.' + getClsID);
    var objset = $('#' + modalID).find('.' + setClsID);
    var idPrefix = arrExpID[0][modalID];
    $('#' + modalID).find('.' + getClsID);
    objset.prop('selectedIndex', objget.prop('selectedIndex'));
    var transDetails = ($('#' + modalID).find('.transfldslct').val()).split('/');
    if (transDetails != null && transDetails[0] != "Select") {
        $('#' + modalID).find('.CardAmount').val(outputMoney(transDetails[transDetails.length - 3]));
        $('#' + modalID).find($("input[title='FC Amount']")).val(outputMoney(transDetails[transDetails.length - 3]));
        $('#' + modalID).find($("input[title='FC Amount']")).blur();
        var transDate = transDetails[transDetails.length - 4].trim();
        //if (modalID == Transportation, gift, mil, oth, entertainment) {
        $('#' + modalID).find($("input[title='Date']")).val(transDate.slice(6, 9) + "/" + transDate.slice(4, 6) + "/" + transDate.slice(0, 4));
        $('#' + modalID).find($("input[title='Date']")).attr("disabled", true);
        //}
    }
    //$('#' + modalID).find('.CardAmount').val(outputMoney(transDetails.pop())); //pop() Removes last value and grap the last value
}
function getUtilizedAmount(cardNo, TransId, slNo) {
    var checkKey = cardNo + "-" + TransId;
    var utilizedAmt = 0;
    $.each(alltblID, function (i, tblID) {
        utilizedAmt += eval($(tableRowSelector(tblID)).map(function () {
            var rowSlNo = $.trim($(this).find("*[postkey='SNo']").text());
            var rowKey = $.trim($(this).find("*[postkey='CardNo']").text()) + "-" + $.trim($(this).find("*[postkey='TransactionID']").text());
            return rowKey == checkKey && slNo != rowSlNo ? outputNumber($.trim($(this).find("*[postkey='PostLCAmt']").text())) : null;
        }).toArray().concat(0).join("+"));
    });
    return utilizedAmt;
}
function getAllUtilizedAmount() {
    var returnObj = {
        cardTransactions: []
    };
    alltblID.forEach(function (tblID) {
        var expandedTable = $("#" + tblID).DataTable().page.len() == -1;
        if (!expandedTable) {
            drawDataTbl(tblID, true);
        }
        $(tableRowSelector(tblID)).each(function () {
            var CardType=$(this).find("*[postkey='CardType']").text();
            if (CardType == publicCardType || CardType == privateCardType) {
                var rowKey = $.trim($(this).find("*[postkey='CardNo']").text()) + "-" + $.trim($(this).find("*[postkey='TransactionID']").text());
                if (returnObj.cardTransactions.indexOf(rowKey) == -1) {
                    returnObj.cardTransactions.push(rowKey);
                    returnObj[rowKey] = 0;
                }
                returnObj[rowKey] += outputNumber($.trim($(this).find("*[postkey='PostLCAmt']").text()));
            }
        });
        if (!expandedTable) {
            drawDataTbl(tblID, false);
...
function submitCardLimitValidation() 
{
    var allUtilizedAmounts = getAllUtilizedAmount();
    var checkColl = publicCardDetails.concat(privateCardDetails);
    for (var i = 0; i < allUtilizedAmounts.cardTransactions.length; i++)
    {
        var checkKey = allUtilizedAmounts.cardTransactions[i];
        var cardJSON = {};
        cardJSON["CreditCardKey"] = checkKey;
        cardJSON["UtilizedAmount"] = allUtilizedAmounts[checkKey];
        cardAmtArray.push(cardJSON);
        var cardDetails = $(publicCardDetails.concat(privateCardDetails)).filter(function () { return this.CARD_NO + "-" + this.APV_NO == checkKey }).toArray();
        if (cardDetails.length > 0)
        {
            cardDetails = cardDetails[0];
            if (allUtilizedAmounts[checkKey] > cardDetails.BUY_SUM) 
            {
                //alertModal("Cannot submit. Amount for card " + checkKey.split("-")[0] + " transaction " + checkKey.split("-")[1] + " exceeding the limit.");
                alertModalML("28",{"<<AMOUNT>>":checkKey.split("-")[0],"<<LIMITAMOUNT>>":checkKey.split("-")[1]});
                return false;
            }
        } 
       else 
        {
            //alertModal("Cannot submit. Details are not maintained for card " + checkKey.split("-")[0] + " transaction " + checkKey.split("-")[1] + ".\nPlease update correct card information in entries/contact administrator.");
            alertModalML("29",{"<<CARDNO>>":checkKey.split("-")[0],"<<TRANSACTIONNO>>":checkKey.split("-")[1]});
                return false;
        }
    }
    $("input[fldTitle='hdnCreditCardKey']").val(JSON.stringify(cardAmtArray));
    return true;
}
...
$(document).on('click', '.splitrow', function (e) {
    e.preventDefault();
    splitRow($(this));
    return false;
});
function splitRow(obj) {
    ...
    drawDataTbl(tblID, true);
    var table = $('#' + tblID).DataTable();
    rowData = "<tr>" + tr.html() + "</tr>";
    table.row.add($(rowData)).draw();
    objSelectedRow = $('#' + tblID + ' > tbody > tr').last();
    objSelectedRow.find(".pkey").text("");
    objSNo = objSelectedRow.find("span.sno");
    var SNo = objSNo.text();
    SNo = SNo.replace(/[^a-z]/gi, '') + (objSelectedRow.index() + 1);
    objSelectedRow.find("span.sno").closest("td").html('<span class="displaynone datacol sno" postkey="SNo" id="' + objSNo.attr('id') + '">' + SNo + '</span>' + SNo);
    calculateTotalAmt(tblID);
    NewRowsCnt++;
    event.preventDefault ? event.preventDefault() : (event.returnValue = false);
    drawDataTbl(tblID, false);
}
function checkTransExists(cardNo, TransId, slNo) {
    var checkKey = cardNo + "-" + TransId;
    ...
    $.each(alltblID, function (i, tblID) {
        $(tableRowSelector(tblID)).each(function () {
            var rowSlNo = $.trim($(this).find("*[postkey='SNo']").text());
            var rowKey = $.trim($(this).find("*[postkey='CardNo']").text()) + "-" + $.trim($(this).find("*[postkey='TransactionID']").text());
            if (rowKey == checkKey && slNo != rowSlNo) {
                cardExists = true;
            }
        });
    });
