...
function getApplicantSectionLessDetails() {
    ...
    var expat = $("select[fldTitle='Expatriate']").val() == undefined || $("select[fldTitle='Expatriate']").val() == "Select" || $("select[fldTitle='Expatriate']").val() == "" ? "" : $("select[fldTitle='Expatriate']").val();
    ...
    applicantDetails +="<tr><td width='25%'>Name in Passport</td><td width='75%' colspan='3' style='text-transform:uppercase;'>" + $.trim($("#idlblPassportName").text()) + "</td></tr>";
    applicantDetails += "<tr><td width='25%'>Name of the Applicant</td><td width='75%' colspan='3'>" + $.trim($("#idlblApplicantName").text()) + "</td></tr>";
    applicantDetails += "<tr><td width='25%'>Division</td><td width='25%'>" + $.trim($("#idlblDivName").text()) + "</td><td>Staff Status</td><td>" + staffStatus + "</td></tr>";
    applicantDetails += "<tr><td width='25%'>Department/Section</td><td width='25%'>" + $.trim($("#idlblDeptName").text()) + " / " + ChrgToPC + "</td><td>Staff Grade</td><td>" + staffGrade + "</td></tr>";
    applicantDetails += "<tr><td width='25%'>Charge To PC</td><td width='25%'>" + ChrgToPCDisp + "</td><td width='25%'>Expatriate</td><td width='25%'>" + expat + "</td></tr>";
    if ($("input[fldTitle='hdnNonAPACRegionReq']").val() == "Yes") {
        ...
        applicantDetails += "<tr><td width='25%'>Non-APAC Region<small>(Europe, China, Hong Kong,Taiwan, Korea, USA, Japan & Latin America.)</small></td><td colspan='3' width='25%'>" + NonAPACReg + "</td></tr>";
    }
...
function getBasicTravelLessInfo() {
    ...
    PreNoDays = !!PreNoDays ? PreNoDays : "";
    var preDetails = preDeptDate + " to " + preArrDate + " (" + PreNoDays + " days)"
...
function getTravelReason() {
    ...
    if (!!ReasonForLateTravel) {
        returnStr += "<tr><td class='LabelText'>Reason For Late Submission : </td></tr><tr><td>" + ReasonForLateTravel + "</td></tr>"
    }
...
function getTravelScheduleLessDetails() {
    ...
    //TravelSchedule += framePanel("Travel Schedule");
    //TravelSchedule += "<table><thead><th>Travel Details :</th></thead></table>";
...
function getContactInformation() {
    ...
    contactInfo += "<tr><td class='LabelText'>Home Telephone No</td><td class='LabelText'>Mobile Phone No</td><td class='LabelText'>Emergency Contact</td><td class='LabelText'>Hotel Name</td><td class='LabelText'>Hotel Address</td><td class='LabelText'>Overseas Contact No</td></tr></thead>";
    contactInfo += "<tbody><tr><td>" + HomeTelNo + "</td><td>" + MobileNo + "</td><td>" + EmergencyContact + "</td><td>" + HotelName + "</td><td>" + HotelAddress + "</td><td>" + OversContNo + "</td></tr>";
...
function eTravelPrint(PrintTill, isPopup) {
    ...
    PostTravelFinalApprovalDate = PostTravelFinalApprovalDate.split("/");
    PostTravelFinalApprovalDate = PostTravelFinalApprovalDate[1] + "/" + PostTravelFinalApprovalDate[0] + PostTravelFinalApprovalDate[2];
...
function getExpenseTable() {
    ...
    allExpenses = allExpenses.sort(function (e1, e2) {
        if (objExpSort.expSortVal(e1.split("~")[0]) < objExpSort.expSortVal(e2.split("~")[0])) { return -1; }
        if (objExpSort.expSortVal(e1.split("~")[0]) > objExpSort.expSortVal(e2.split("~")[0])) { return 1; }
        return 0;
    })
...
function printPaymentSummary() {
    ...
    paySum += "<table border='0' width='100%' style='border-collapse:collapse;margin-top:20px' class='tabledisplay'><thead><tr height='30'><td colspan='5' class='LabelText'>Payment Summary</td></tr>";
        //framePanel("Payment Summary");
    //paySum += "<table>";
    //paySum += "<thead>";
    //paySum += "<tr height='30'><td colspan='5' class='LabelText'>Payment Summary</td></tr>";
    paySum += "<tr><td class='LabelText'>Items</td>";
    paySum += "<td class='LabelText' style='display:none'>Pre Amount (SGD)</td>";
    paySum += "<td class='LabelText'>Post Amount (SGD)</td>";
    paySum += "<td class='LabelText' style='display:none'>Diff. Against Post</td>";
    paySum += "<td class='LabelText'>GL Code</td>";
    paySum += "</tr>";
    paySum += "</thead>";
    var alignRight = "";
    var dispBold = "";
    ...
    $("#tblPaymentSummary").find("tbody>tr").each(function () {
        ...
        if (lblVal.indexOf("By Company")!=-1) {
        ...
        paySum += "<tr>";
        paySum += "<td " + dispBold + ">" + lblVal + "</td><td style='text-align:right;" + (dispBold == "" ? "" : "font-weight:bold;") + "'>" + PostAmt + "</td><td " + dispBold + ">" + GlCode + "</td></tr>";
    });
    paySum = "<tbody>" + paySum
    paySum += "</tbody></table>"
    if ($("input[fldTitle='hdnBudgetRequired']").val() == "Yes") {
...
function PrintPage() {
    ...
    eTravelPrintLessDetails('PrintAll');
...
function eTravelPrintLessDetails(PrintTill) {
    ...
    // Print Remarks 
    var tmpRemarks = $("#divRemarks").clone();
    tmpRemarks.find("textarea").after(tmpRemarks.find("textarea").val().trim().replace(/\n/g, "<br/>") );
    tmpRemarks.find("textarea").remove();
    tmpRemarks.find("label").remove();
    
    var lsRemarks = tmpRemarks.html();
    tmpRemarks = null;
    //For Internal Use    
    ...
    var PostActNoDays = $("input[fldTitle='hdnpostNoDays']").val();
    ...
    PostTrvDet += "</table>";
    //AccountUse += "<table>";
    //AccountUse += "<tr><td><b>Pre-Travel Authorised by</b></td><td>" + PreTravelFinalApprover + "</td></tr>";
    //AccountUse += "<tr><td><b>Date :</b>" + PreTravelFinalApprovalDate + "</td></tr>"
    //AccountUse += "<tr><td><b>Post-Travel Authorised by</b></td><td>" + PostTravelFinalApprover + "</td></tr>";
    //AccountUse += "<tr><td><b>Date : </b>" + PostTravelFinalApprovalDate + "</td></tr>";
    //AccountUse += "<tr><td><b>Accounts Use</b></td><td><b>Net Payable to Staff/Company (" + baseCurrency + " $)</b></br>" + NetPayToStaff + "</td></tr>";
    //AccountUse += "<tr><td><b>Pay through Payroll</b></td><td>" + PayPayroll + "</td></tr>";
        ...
        if (rowCnt > 0) {
            ...
            for (var i = 0; i < allExpenses.length; i++) {
                ...
                if (tableID != "idTblATC") {
                    ...
                    else if (tableID == "idTblMIL") {
                        expDetailFullValue += formatPrintDisplay(tableID);
                        expDetailFullValue += formatPrintDisplay("idTblOTH");
                    }
                    else
                        expDetailFullValue += formatPrintDisplay(tableID);
                }
...
function printTravelDetails(tblID, fldLabel, fldID, fldClass, flgTot, totColSpan) {
    ...
    if (expenseTableCount > 0) {
        ...
        else if (tblID == "idTblOTH") {
            formatText += "<table border='0' width='100%' style='border-collapse:collapse;margin-top:20px' class='tabledisplay'><tbody><tr height='30'><td width='20%' class='LabelText' style='border-bottom:0px !important'>Mileage(Others) Expenses </td></tr></tbody></table>"
        }
        else {
        ...
        if (flgTot) {
            var exptempName = expTableName;
            if (tblID == "idTblOTH")
                exptempName = "Mileage(Others)"; //For Other's Footer label
            if (flagarranageby)
                formatText += "<tfoot><tr><td style='text-align:right;font-weight: bold;' colspan='" + totColSpan + "'>" + exptempName + " Sub Total</td><td style='text-align:right'>" + outputMoney(EmployeeTotal) + "</td>";
            else
                formatText += "<tfoot><tr><td style='text-align:right;font-weight: bold;' colspan='" + totColSpan + "'>" + exptempName + " Sub Total</td><td style='text-align:right'>" + $("#" + tblID + ">tfoot>tr").find(".totalamt").text() + "</td>";
...
function printTravelMILDetails(tblID, fldLabel, fldID, fldClass, flgTot, totColSpan) {
    var formatText = "";
    var cls = "";
    var EmployeeTotal = 0;
    var flagarranageby = false;
    ...
    if (expenseTableCount > 0) {
        ...
        $(tableRowSelector(tblID)).each(function () {
            ...
            if (tr.index() == 0) {
                formatText += "<thead><tr>";
                formatText += "<th style='width:37px !important;font-weight: bold;' rowspan='2'>S.#</th>";
                formatText += "<th style='font-weight: bold;' rowspan='2'>Date</th>";
                formatText += "<th style='font-weight: bold;' colspan='2'>Mileage</th>";
                formatText += "<th style='font-weight: bold;' colspan='2'>Place</th>";
                formatText += "<th style='font-weight: bold;' rowspan='2'>Distance Travelled</th>";
                formatText += "<th style='font-weight: bold;' rowspan='2'>Mileage Claim Rate</th>";
                formatText += "<th style='font-weight: bold;' rowspan='2'>Amount in SGD</th>";
                formatText += "</tr><tr>";
                formatText += "<th style='font-weight: bold;'>Start</th>";
                formatText += "<th style='font-weight: bold;'>End</th>";
                formatText += "<th style='font-weight: bold;'>From</th>";
                formatText += "<th style='font-weight: bold;'>To</th>";
                formatText += "</tr></thead>";
                formatText += "<tbody>";
            }
            formatText += "<tr>";
            for (var i = 0; i < fldID.length; i++) {
                cls = fldClass[i] == "" ? "" : "class=" + fldClass[i];
                formatText += "<td style='word-wrap:break-word' " + cls + ">" + tr.find("#" + fldID[i]).text() + "</td>";
                if (fldID[i] == "idMILDistanceTravelled") {
                    distTravelledTot += outputNumber(tr.find("#" + fldID[i]).text());
                }
            }
            formatText += "</tr>";
        });
        if (flgTot) {
            formatText += "<tfoot><tr><td style='text-align:right;font-weight: bold;' colspan='6'>" + expTableName + " Sub Total</td>";
            formatText += "<td style='text-align:right'>" + outputMoney(distTravelledTot) + "</td>";
            formatText += "<td></td>";
            formatText += "<td style='text-align:right'>" + $("#" + tblID + ">tfoot>tr").find(".totalamt").text() + "</td>";
            formatText += "</tr>";
            val = arrExpCodes[0][tblID];
            var expRow = $("tr[expcode='" + val + "']");
            var remark = $.trim(expRow.find("#idExpRemarksText").val());
...
//}
function formatPrintMIL(tblID) {
    var fldID = ["idMILSNo", "idMILDate", "idMILMileageStart", "idMILMileageEnd", "idMILFrom", "idMILTo", "idMILDistanceTravelled", "idMILKmsMileageRate", "idMILLCAmt"]
    var fldLabel = ["S.#", "Date", "Mileage Start", "Mileage End", "Place From", "Place To", "Dist. Travelled", "Mileage Rate", "Amount in SGD"]
    var fldClass = ["", "", "amount", "amount", "", "", "amount", "amount", "amount"];
    var strPrintData = printTravelMILDetails(tblID, fldLabel, fldID, fldClass, true, 8);
    return strPrintData;
}
