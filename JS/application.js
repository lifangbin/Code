...
var NoCollapseExpand = ["idTblEXCO", "idTblEXCOTECH", "idTblADV", "idTblRef", "idTblWIN", "idTblENTD", "idTblGIFTD", "idTblACCENT", "idTblPAD", "idTblMYEXCH"];
var printFlg = false;
var printFlgLessDetails = false;
var DASplitRequired = $("input[fldTitle='hdnDASplitRequired']").val();
...
var objExpSort = {
    ...
    "Gift",
    "WinterAllowance"],
    expSortVal: function (expCode) { return this.mainExp.indexOf(expCode) },
    subExpDesc: ["Visa", "Medical"],
    isSubDescending: function (expCode) { return this.subExpDesc.indexOf(expCode) != -1; }
};
var expDefaultSub = {
    "Miscellaneous": "Conference",
    "Airport Tax": "Airport Tax",
    "Communication": "Communication",
    "Medical": "Medical(Local)",
    "Transportation": "Transportation Public",
    "Visa": "Visa",
    "Miscellaneous": "Conference/Meeting"
};
...
function calculateTotalAmt(tblID) {
    ...
    var TotMilLCAmt = 0;
    var objSubExp = null;
    var objSubExpEmp = null;
    var objSubExpComp = null;
    ...
    if (tblID == "idTblOTH" || tblID == "idTblMIL") {
        ...
        $(tableRowSelector(tblID)).each(function () {
            ...
            if (!isNaN(LCAmt) && LCAmt.length != 0) {
                TotLCAmt += LCAmt;
                var subExpSpan = $(this).find("*[postkey=PostExpTypes]");
                if (subExpSpan.length > 0) { // Sub Expense Object
                    if (!objSubExp) {
                        objSubExp = { names: [] };
                    }
                    if (objSubExp.names.indexOf($.trim(subExpSpan.text())) == -1) {
                        objSubExp.names.push($.trim(subExpSpan.text()));
                        objSubExp[$.trim(subExpSpan.text())] = {
                            "dispName": $.trim(subExpSpan.text()),
                            "postLCAmt": (travelStage != "pre" ? LCAmt : 0),
                            "preLCAmt": 0
                        };
                    } else {
                        objSubExp[$.trim(subExpSpan.text())].postLCAmt += (travelStage != "pre" ? LCAmt : 0);
                    }
                }
                if ($(this).find(".ticketArrBy").length > 0) {
                    if ($(this).find(".ticketArrBy").text().toLowerCase() == "employee") {
                        ...
                        if (subExpSpan.length > 0) { // Sub Expense Object
                            ...
                            if (objSubExpEmp.names.indexOf($.trim(subExpSpan.text())) == -1) {
                                objSubExpEmp.names.push($.trim(subExpSpan.text()));
                                objSubExpEmp[$.trim(subExpSpan.text())] = {
                                    "dispName": $.trim(subExpSpan.text()) + " By Employee",
                                    "postLCAmt": (travelStage != "pre" ? LCAmt : 0),
                                    "preLCAmt": 0
                                };
                                objSubExpComp.names.push($.trim(subExpSpan.text()));
                                objSubExpComp[$.trim(subExpSpan.text())] = {
                                    "dispName": $.trim(subExpSpan.text()) + " By Company",
                                    "postLCAmt": 0,
                                    "preLCAmt": 0
                                };
                            } else {
                                objSubExpEmp[$.trim(subExpSpan.text())].postLCAmt += (travelStage != "pre" ? LCAmt : 0);
                            }
                        }
                        ...
                        if (subExpSpan.length > 0) { // Sub Expense Object
                            ...
                            if (objSubExpComp.names.indexOf($.trim(subExpSpan.text())) == -1) {
                                objSubExpEmp.names.push($.trim(subExpSpan.text()));
                                objSubExpEmp[$.trim(subExpSpan.text())] = {
                                    "dispName": $.trim(subExpSpan.text()) + " By Employee",
                                    "postLCAmt": 0,
                                ...
                                objSubExpComp[$.trim(subExpSpan.text())].postLCAmt += (travelStage != "pre" ? LCAmt : 0);
                            }
                        }
                    }
                }
        ...
        if (!isNaN(LCAmt) && LCAmt.length != 0) {
            ...
            if (subExpSpan.length > 0) { // Sub Expense Object
                ...
                if (objSubExp.names.indexOf($.trim(subExpSpan.text())) == -1) {
                    ...
                    objSubExp[$.trim(subExpSpan.text())].preLCAmt += LCAmt;
                }
            }
            if ($(this).find(".ticketArrBy").length > 0) {
                if ($(this).find(".ticketArrBy").attr("prevalue").toLowerCase() == "employee") {
                    ...
                    if (subExpSpan.length > 0) { // Sub Expense Object
                        ...
                        if (objSubExpEmp.names.indexOf($.trim(subExpSpan.text())) == -1) {
                            ...
                            objSubExpEmp[$.trim(subExpSpan.text())].preLCAmt += LCAmt;
                        }
                    }
                    ...
                    if (subExpSpan.length > 0) { // Sub Expense Object
                        ...
                        if (objSubExpComp.names.indexOf($.trim(subExpSpan.text())) == -1) {
                            ...
                            objSubExpComp[$.trim(subExpSpan.text())].preLCAmt += LCAmt;
                        }
                    }
                }
            }
    ...
    if (flgPrePostExp) {
        ...
        if (expenseTypeObj[ExpCode + "Company"]) {
            ...
            if (!!objSubExpComp) {
                objSubExpComp.names = objSubExpComp.names.sort();
                objSubExpComp.names.forEach(function (sExp) {
                    objSubExpComp[sExp].preLCAmt = outputMoney(objSubExpComp[sExp].preLCAmt);
                    objSubExpComp[sExp].postLCAmt = outputMoney(objSubExpComp[sExp].postLCAmt);
                });
            }
            expenseTypeObj[ExpCode + "Employee"]["preLCAmt"] = outputMoney(EmployeeTotal);
            if (!!objSubExpEmp) {
                objSubExpEmp.names = objSubExpEmp.names.sort();
                objSubExpEmp.names.forEach(function (sExp) {
                    objSubExpEmp[sExp].preLCAmt = outputMoney(objSubExpEmp[sExp].preLCAmt);
                    objSubExpEmp[sExp].postLCAmt = outputMoney(objSubExpEmp[sExp].postLCAmt);
                });
            }
            expenseTypeObj[ExpCode + "Company"]["subExp"] = objSubExpComp;
            expenseTypeObj[ExpCode + "Employee"]["subExp"] = objSubExpEmp;
        } else if (ExpCode != undefined) {
            expenseTypeObj[ExpCode]["preLCAmt"] = outputMoney(TotLCAmt);
            if (!!objSubExp) {
                objSubExp.names = objSubExp.names.sort();
                objSubExp.names.forEach(function (sExp) {
                    objSubExp[sExp].preLCAmt = outputMoney(objSubExp[sExp].preLCAmt);
                    objSubExp[sExp].postLCAmt = outputMoney(objSubExp[sExp].postLCAmt);
                });
            }
            expenseTypeObj[ExpCode]["subExp"] = objSubExp;
        }
    } else {
        if (OnlyPostExpTypes.indexOf(ExpCode) != -1 && TicketArrangedByExp.indexOf(ExpCode) != -1) {
            ...
            if (expenseTypeObj[ExpCode + "Company"]) {
                if (expArranged.byCompany(ExpCode)) {
                    ...
                    if (!!objSubExpComp) {
                        objSubExpComp.names = objSubExpComp.names.sort();
                        if (objSubExpComp.names.indexOf(expDefaultSub[ExpCode]) != -1) {
                            objSubExpComp[expDefaultSub[ExpCode]].preLCAmt = outputMoney(CompanyTotal);
                        } else {
                            objSubExpComp[objSubExpComp.names[0]].preLCAmt = outputMoney(CompanyTotal);
                        }
                        objSubExpComp.names.forEach(function (sExp) {
                            objSubExpComp[sExp].preLCAmt = outputMoney(objSubExpComp[sExp].preLCAmt);
                            objSubExpComp[sExp].postLCAmt = outputMoney(objSubExpComp[sExp].postLCAmt);
                    ...
                    if (!!objSubExpEmp) {
                        objSubExpEmp.names = objSubExpEmp.names.sort();
                        if (objSubExpEmp.names.indexOf(expDefaultSub[ExpCode]) != -1) {
                            objSubExpEmp[expDefaultSub[ExpCode]].preLCAmt = outputMoney(CompanyTotal);
                        } else {
                            objSubExpEmp[objSubExpEmp.names[0]].preLCAmt = outputMoney(CompanyTotal);
                        }
                        objSubExpEmp.names.forEach(function (sExp) {
                            objSubExpEmp[sExp].preLCAmt = outputMoney(objSubExpEmp[sExp].preLCAmt);
                            objSubExpEmp[sExp].postLCAmt = outputMoney(objSubExpEmp[sExp].postLCAmt);
            ...
            if (expRow.length > 0) {
                ...
                if (!!objSubExp) {
                    objSubExp.names = objSubExp.names.sort();
                    if (objSubExp.names.indexOf(expDefaultSub[ExpCode]) != -1) {
                        objSubExp[expDefaultSub[ExpCode]].preLCAmt = expenseTypeObj[ExpCode]["preLCAmt"];
                    } else {
                        objSubExp[objSubExp.names[0]].preLCAmt = expenseTypeObj[ExpCode]["preLCAmt"];
                    }
                    objSubExp.names.forEach(function (sExp) {
                        objSubExp[sExp].preLCAmt = outputMoney(objSubExp[sExp].preLCAmt);
                        objSubExp[sExp].postLCAmt = outputMoney(objSubExp[sExp].postLCAmt);
...
function setDataEditModal(tblID, modalID) {
    switch (tblID) {
        ...
        case 'idTblACCENT':   //Accounting Enteries
            ...
            break;
        case 'idTblEXCO':   //Export Control Hand Carried
            setEXCODataEditModal();
            break;
        case 'idTblEXCOTECH':   //Export Control Technical information
            setEXCOTECHDataEditModal();
            break;
...
function updateRow(tblID, modalID) {
    ...
    switch (tblID) {
        ...
        case 'idTblACCENT':  //Accounting Enteries
            ...
            break;
        case 'idTblEXCO':   //Export Control Hand Carried
            rowData = getEXCODataEditModal();
            break;
        case 'idTblEXCOTECH':  //Export Control Technical information
            rowData = getEXCOTECHDataEditModal();
            break;
        case 'idTblADV':  //Advance Enteries
...
function generateTSHSummary() {
    ...
    if (DASplitRequired == "Yes") {
        drawDataTbl("idTblDA", true);
        $(tableRowSelector('idTblDA')).each(function () {
            var DaCountry = $(this).find("#idDACountry").attr("prevalue");
            if (DaCountry == "" || DaCountry == "Select") {
                $(this).addClass("PreEmptyDA");
            }
        });
        var table = $('#idTblDA').DataTable();
        var rows = table.rows('.PreEmptyDA').remove().draw();
...
function getActionBtns(modalID) {
    ...
    var DAsplitRow = "";
    if (DASplitRequired == "Yes") {
        DAsplitRow = '<button class="DAsplitrow" modalid="' + modalID + '"><i class="fa fa-hand-scissors-o text-green"></i></button>';
    }
    if (isAccountsLevel == "Yes") {
        ...
        strActBtns += '<div class="center-btn">';
        strActBtns += '<button class="editrow" modalid="' + modalID + '"><i class="fa fa-pencil text-blue"></i></button>';
        if (modalID == "DAModal" && travelStage == "post") {
            strActBtns += DAsplitRow;
        }
        if (modalID != "DAModal") {
...
function setOTHDataEditModal() {
    ...
    var fldID = ["idOTHID", "idOTHSNo", "idOTHType", "idOTHDate", "idOTHFrom", "idOTHTo", "idOTHClaimCurr", "idOTHClaimAmt", "idOTHExRate", "idOTHLCAmt", "idOTHCostCenter", "idOTHGSTCode", "idOTHTaxableAmt", "idOTHGSTAmt", "idOTHGLCode", "idOTHExpDesc", "idOTHChargeToFactory", "idOTHReceiptNo", "idOTHCostCenterDisplay", "idOTHGSTCodeDisplay"]
...
function formatOTH(tr) {
    var fldID = ["idOTHSNo", "idOTHClaimCurr", "idOTHCostCenterDisplay", "idOTHType", "idOTHExRate", "idOTHGLCode", "idOTHDate", "idOTHLCAmt", "idOTHChargeToFactory", "idOTHFrom", "idOTHGSTCodeDisplay", "idOTHReceiptNo", "idOTHTo", "idOTHTaxableAmt", "", "idOTHClaimAmt", "idOTHGSTAmt", "", "idOTHExpDesc"]
    var fldLabel = ["S.#", "Curr.", "Cost Center", "Expense Type", "Ex. Rate", "GL Code", "Date", "LC Amt", "Charge to Fact.", "Place From", "GST Code", "Receipt #", "Place To", "Taxable Amt", "", "FC Amt", "GST Amt", "", "Description"]
    var fldClass = ["", "", "", "", "amount", "", "", "amount", "", "", "", "", "", "amount", "amount", "amount", "amount", "amount", "text"];
    strRowData = formatTravelFields(tr, fldLabel, fldID, fldClass, false);
    return strRowData;
}
//OTHER Expense Ends
...
function setVSADataEditModal() {
    ...
    //var fldEditID = ["idVSAIDEdit", "idVSASNoEdit", "idVSATaxAmtEdit", "idVSAExpTypEdit", "idVSACountryEdit", "idVSAVsaEdit", "idVSAFCAmtEdit", "idVSAExRateEdit", "idVSAPCEdit", "idVSATaxAmtEdit", "idVSAGSTAmtEdit", "idVSAGLCodeEdit", "idVSAGSTCodeEdit", "idSVSAChrgToFactEdit", "idVSARcptNoEdit", "idVSADescEdit", "idVSACurrencyEdit", "idVSALCAmtEdit", "idVSAArrgByEdit", "idVSAPCDisplayEdit", "idVSAGSTCodeDisplayEdit"]
    //var fldID = ["idVSAID", "idVSASNo", "idVSATaxAmt", "idVSAExpTyp", "idVSACountry", "idVSAVsa", "idVSAFCAmt", "idVSAExRate", "idVSAPC", "idVSATaxAmt", "idVSAGSTAmt", "idVSAGLCode", "idVSAGSTCode", "idSVSAChrgToFact", "idVSARcptNo", "idVSADesc", "idVSACurrency", "idVSALCAmt", "idVSAArrgBy", "idVSAPCDisplay", "idVSAGSTCodeDisplay"]
    //var fldAccEditable = ["", "", "", "disabled", "disabled", "disabled", "", "readonly", "disabled", "", "", "", "", "disabled", "", "readonly", "", "", "", ""]
    ...
    var fldAccEditable = ["", "", "disabled", "disabled", "disabled", "", "readonly", "disabled", "", "", "", "", "disabled", "readonly", "readonly", "disabled", "", "disabled", "", ""]
...
function formatVSA(tr) {
    var fldLabel = ["S.#", "Curr.", "GST Amt", "Expense Type", "Ex. Rate", "Cost Center", "Country/City", "LC Amt", "GL Code", "VISA #", "GST Code", "Charge To Fact.", "FC Amt", "Taxable Amt", "Receipt #", "Visa Arr. By", "NA", "NA", "Description"];
...
function setMISDataEditModal() {
    ...
    var fldID = ["idMISID", "idMISSNo", "idMISFCAmt", "idMISExRate", "idMISPC", "idMISTaxAmt", "idMISGSTAmt", "idMISGLCode", "idMISGSTCode", "idSMISChargFacry", "idMISRcptAmt", "idMISDesc", "idMISCurncy", "idMISLCAmt", "idMISExpTyp", "idMISPCEdit", "idMISGSTCodeDisplay"]
    var fldAccEditable = ["", "", "", "readonly", "disabled", "", "", "", "", "disabled", "", "readonly", "disabled", "", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
}
...
function formatMIS(tr) {
    var fldLabel = ["S.#", "LC Amt", "Cost Center", "Expense Type", "GST Code", "GL Code", "FC Amt", "Taxable Amt", "Charge To Fact.", "Curr.", "GST Amt", "Receipt #", "Ex. Rate", "NA", "", "Description"];
    var fldID = ["idMISSNo", "idMISLCAmt", "idMISPCDisplay", "idMISExpTyp", "idMISGSTCodeDisplay", "idMISGLCode", "idMISFCAmt", "idMISTaxAmt", "idSMISChargFacry", "idMISCurncy", "idMISGSTAmt", "idMISRcptAmt", "idMISExRate", "NA", "", "idMISDesc"];
    var fldClass = ["", "amount", "", "", "", "", "amount", "amount", "", "", "amount", "amount", "amount", "", "", ""];
    strRowData = formatTravelFields(tr, fldLabel, fldID, fldClass, false);
    return strRowData;
}
...
function setACCDataEditModal() {
    ...
    var fldEditID = ["idACCIDEdit", "idACCSNoEdit", "idACCLCAmtEdit", "idACCReceiptEdit", "idACCCountryEdit", "idACCGSTCodeEdit", "idACCVendorEdit", "idACCHotelNameEdit", "idACCTaxAmtEdit", "idACCInvoiceEdit", "idACCPNShareEdit", "idACCGSTAmtEdit", "idACCBRNEdit", "idACCNoDaysEdit", "idACCCostCenterEdit", "idACCFCAmtEdit", "idACCGLCodeEdit", "idACCExRateEdit", "idACCChrgtoFactEdit", "idACCCurrencyEdit", "idACCDescEdit", "idACCCostCenterDisplayEdit", "idACCGSTCodeDisplayEdit", "idACCArgByEdit"]
    var fldID = ["idACCID", "idACCSNo", "idACCLCAmt", "idACCReceipt", "idACCCountry", "idACCGSTCode", "idACCVendor", "idACCHotelName", "idACCTaxAmt", "idACCInvoice", "idACCPNShare", "idACCGSTAmt", "idACCBRN", "idACCNoDays", "idACCCostCenter", "idACCFCAmt", "idACCGLCode", "idACCExRate", "idACCChrgtoFact", "idACCCurrency", "idACCDesc", "idACCCostCenterDisplay", "idACCGSTCodeDisplay", "idACCArgBy"]
    var fldAccEditable = ["", "", "", "", "disabled", "", "", "readonly", "", "", "readonly", "", "", "readonly", "", "", "", "readonly", "disabled", "disabled", "readonly", "", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function formatACC(tr) {
    var fldLabel = ["S.#", "Curr.", "GL Code", "Country/City", "Ex. Rate", "Charge To Fact.", "Hotel Name", "LC Amt", "Receipt #", "Accom By", "GST Code", "Vendor #", "FC Amt", "Taxable Amt", "Invoice #", "No. of Days", "GST Amt", "BRN #", "Per Night Share", "Cost Center", "", "Description"];
    var fldID = ["idACCSNo", "idACCCurrency", "idACCGLCode", "idACCCountry", "idACCExRate", "idACCChrgtoFact", "idACCHotelName", "idACCLCAmt", "idACCReceipt", "idACCArgBy", "idACCGSTCodeDisplay", "idACCVendor", "idACCFCAmt", "idACCTaxAmt", "idACCInvoice", "idACCNoDays", "idACCGSTAmt", "idACCBRN", "idACCPNShare", "idACCCostCenterDisplay", "", "idACCDesc"];
    var fldClass = ["", "", "", "", "amount", "", "", "amount", "", "", "", "", "amount", "amount", "", "", "amount", "", "amount", "", "", ""];
...
function setATCDataEditModal() {
    //Set Row for editing
    var tr = objSelectedRow;
    var fldEditID = ["idATCIDEdit", "idATCSNoEdit", "idATCGSTCodeEdit", "idATCVendorEdit", "idATCInvoiceNoEdit", "idATCTaxAmtEdit", "idATCInvoiceEdit", "idATCTktArgByEdit", "idATCGSTAmtEdit", "idATCBRNEdit", "idATCCurrencyEdit", "idATCCostCenterEdit", "idATCDescEdit", "idATCFCAmtEdit", "idATCGLCodeEdit", "idATCExRateEdit", "idATCChrgToFactEdit", "idATCLCAmtEdit", "idATCReceiptEdit", "idATCCostCenterDisplayEdit", "idATCGSTCodeDisplayEdit"];
    var fldID = ["idATCID", "idATCSNo", "idATCGSTCode", "idATCVendor", "idATCInvoiceNo", "idATCTaxAmt", "idATCInvoice", "idATCTktArgBy", "idATCGSTAmt", "idATCBRN", "idATCCurrency", "idATCCostCenter", "idATCDesc", "idATCFCAmt", "idATCGLCode", "idATCExRate", "idATCChrgToFact", "idATCLCAmt", "idATCReceipt", "idATCCostCenterDisplay", "idATCGSTCodeDisplay"];
    var fldAccEditable = ["", "", "", "", "", "", "", "disabled", "", "", "disabled", "", "readonly", "", "", "readonly", "disabled", "", "", "", ""];
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function formatATC(tr) {
    var fldLabel = ["S.#", "LC Amt", "Charge To Fact.", "Invoice Number", "GST Code", "Receipt #", "Tkt Arranged By", "Taxable Amt", "Vendor #", "FC Amt", "GST Amt", "Invoice #", "Curr.", "Cost Center", "BRN #", "Ex. Rate", "GL Code", "", "Description"];
    var fldID = ["idATCSNo", "idATCLCAmt", "idATCChrgToFact", "idATCInvoiceNo", "idATCGSTCodeDisplay", "idATCReceipt", "idATCTktArgBy", "idATCTaxAmt", "idATCVendor", "idATCFCAmt", "idATCGSTAmt", "idATCInvoice", "idATCCurrency", "idATCCostCenterDisplay", "idATCBRN", "idATCExRate", "idATCGLCode", "", "idATCDesc"];
...
function setDADataEditModal() {
    ...
    if (TravellingTo == "Domestic") {
        $('#idDACountryEdit').attr("disabled", true);
    }
...
function generateDARow(strJSON, flgIntialLoading) {
    ...
    $.each(objJSON, function (i, item) {
        ...
        strNewRow += '<span class="predisplay">' + item.PreDARate + '</span>' + DARate + '</td>';
...
function formatENT(tr) {
    var fldLabel = ["S.#", "Ex. Rate", "GL Code", "Category", "LC Amt", "Charge To Fact.", "Person Count", "GST Code", "Receipt #", "Date", "Taxable Amt", "Vendor #", "Place", "Per Head Amt", "Invoice #", "FC Amt", "GST Amt", "BRN #", "Curr.", "Cost Center", "", "Description"];
    var fldID = ["idENTSNo", "idENTExRate", "idENTGLCode", "idENTCategory", "idENTLCAmt", "idENTChargeToFact", "idENTPersonCount", "idENTGSTCodeDisplay", "idENTReceipt", "idENTDate", "idENTTaxableAmt", "idENTVendor", "idENTPlace", "idENTPerHeadAmt", "idENTInvoice", "idENTFCAmt", "idENTGSTAmt", "idENTBRN", "idENTCurrency", "idENTCostCenterDisplay", "", "idENTDesc"];
    var fldClass = ["", "amount", "", "", "amount", "", "", "", "", "", "amount", "", "", "amount", "", "amount", "amount", "", "", "", "", ""];
    strRowData = formatTravelFields(tr, fldLabel, fldID, fldClass, true);
    return strRowData;
}
...
function setTSPDataEditModal() {
    ...
    var fldAccEditable = ["", "", "disabled", "readonly", "disabled", "readonly", "readonly", "", "disabled", "readonly", "", "", "", "", "", "", "disabled", "","", "", "", "", "", "", "readonly", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function formatAPT(tr) {
    var fldLabel = ["S.#", "LC Amt", "Charge to Fact.", "Sub Type", "GST Code", "Arranged By", "Airport Name", "Taxable Amt", "Receipt #", "FC Amt", "GST Amt", "Vendor #", "Curr.", "Cost Center", "Invoice #", "Ex. Rate", "GL Code", "BRN #", "Description"];
    var fldID = ["idAPTSNo", "idAPTLCAmt", "idAPTChargeToFactory", "idAPTSubType", "idAPTGSTCodeDisplay", "idAPTTaxArgBy", "idAPTAirportName", "idAPTTaxableAmt", "idAPTReceiptNo", "idAPTClaimAmt", "idAPTGSTAmt", "idAPTVendorNo", "idAPTClaimCurr", "idAPTCostCenterDisplay", "idAPTInvoiceNo", "idAPTExRate", "idAPTGLCode", "idAPTBRNNo", "idAPTExpDesc"];
    var fldClass = ["", "amount", "", "", "", "", "", "amount", "", "amount", "amount", "", "", "", "", "amount", "", "", ""];
    strRowData = formatTravelFields(tr, fldLabel, fldID, fldClass, false);
    return strRowData;
}
...
function setGIFTDataEditModal() {
    ...
    var fldAccEditable = ["", "", "readonly", "readonly", "", "disabled", "readonly", "", "", "", "", "", "", "readonly", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable);
...
function validateGIFT() {
    ...
    var fldName = ["Details", "Date", "FC Amt", "Currency", "GST Code", "Taxable Amount", "Cost Center", "GL Code", "Charge To Factory"];
...
function formatGIFT(tr) {
    var fldLabel = ["S.#", "Ex. Rate", "Cost Center", "Details", "LC Amt", "GL Code", "Date", "GST Code", "Charge To Fact.", "FC Amt", "Taxable Amt", "", "Curr.", "GST Amt", "", "Description"];
...
//Travel Schedule End
//Export Control Hand-Carried Start
function generateEXCORow(strJSON, flgIntialLoading) {
    var objJSON = JSON.parse(strJSON);
    var tblEXCO = null;
    if (flgIntialLoading) {
        tblEXCO = $('#idTblEXCO').DataTable();
        tblEXCO.clear()
        tblEXCO.draw();
    }
    var strNewRow = "";
    ...
    $.each(objJSON, function (i, item) {
        var Amount = "";
        Amount = outputMoney(item.Amount);
        strNewRow = '<td class="displaynone"><span class="datacol" prekey="ExpCode" postkey="ExpCode">ExportControlHandCarried</span><span class="datacol" id="idEXCOID" prekey="ID" postkey="ID">' + item.ID + '</span>' + item.ID + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol" prekey="SNo" postkey="SNo" id="idEXCOSNo">' + item.SNo + '</span>' + item.SNo + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol" prekey="NameOfGoods" postkey="NameOfGoods" id="idEXCONameOfGoods">' + item.NameOfGoods + '</span>' + item.NameOfGoods + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol" prekey="ControlledByLocalComp" postkey="ControlledByLocalComp" id="idEXCOContbyLocComp">' + item.ControlledByLocalComp + '</span>' + item.ControlledByLocalComp + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol" prekey="JapanNotified" postkey="JapanNotified" id="idEXCOJapanNotified">' + item.JapanNotified + '</span>' + item.JapanNotified + '</td>';
        strNewRow += '<td class="vmiddle amount"><span class="displaynone datacol" prekey="Quantity" postkey="Quantity" id="idEXCOQty">' + item.Quantity + '</span>' + item.Quantity + '</td>';
        strNewRow += '<td class="vmiddle amount"><span class="displaynone datacol lcamt" prekey="Amount" postkey="Amount" id="idEXCOAmt">' + outputMoney(item.Amount) + '</span>' + Amount + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol" prekey="BringingBack" postkey="BringingBack" id="idEXCOBringBack">' + item.BringingBack + '</span>' + item.BringingBack + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol" prekey="Desc" postkey="Desc" id="idEXCODesc">' + item.Desc + '</span>' + item.Desc + '</td>';
        if (IsApplicantLevel == "true") {
            strActBtns = '<td class="center padding0" nowrap>';
            strActBtns += '<div class="center-btn">';
            strActBtns += '<button class="editrow" modalid="EXCOModal"><i class="fa fa-pencil text-blue"></i></button>';
            strActBtns += '<button class="deleterow" modalid="EXCOModal"><i class="fa fa-trash text-red"></i></button>';
            strActBtns += '<button class="undodel" style="display:none;" modalid="EXCOModal"><i class="fa fa-undo text-orange"></i></button>';
            strActBtns += '</div></td>';
        }
        strNewRow += strActBtns;
        if (flgIntialLoading) {
            strNewRow = "<tr>" + strNewRow + "</tr>";
            tblEXCO.row.add($(strNewRow)).draw();
        }
    });
    if (!flgIntialLoading) {
        return strNewRow;
...
function getEXCODataEditModal() {
    if (!validateEXCO()) {
        return false;
    }
    var objJSON = [];
    var rowJSON = {};
    rowJSON["ID"] = $("#idEXCOIDEdit").val();
    rowJSON["SNo"] = $("#idEXCOSNoEdit").val();
    rowJSON["NameOfGoods"] = $("#idEXCONameOfGoodsEdit").val();
    rowJSON["ControlledByLocalComp"] = $("#idEXCOContbyLocCompEdit").val();
    rowJSON["JapanNotified"] = $("#idEXCOJapanNotifiedEdit").val();
    rowJSON["Quantity"] = $("#idEXCOQtyEdit").val();
    rowJSON["Amount"] = $("#idEXCOAmtEdit").val();
    rowJSON["BringingBack"] = $("#idEXCOBringBackEdit").val();
    rowJSON["Desc"] = $("#idEXCODescEdit").val();
    objJSON.push(rowJSON);
    var strJSON = JSON.stringify(objJSON);
    return generateEXCORow(strJSON, false);
}
function setEXCODataEditModal() {
    //Set Row for editing
    var tr = objSelectedRow;
    var fldEditID = ["idEXCOIDEdit", "idEXCOSNoEdit", "idEXCONameOfGoodsEdit", "idEXCOContbyLocCompEdit", "idEXCOJapanNotifiedEdit", "idEXCOQtyEdit", "idEXCOAmtEdit", "idEXCOBringBackEdit", "idEXCODescEdit"]
    var fldID = ["idEXCOID", "idEXCOSNo", "idEXCONameOfGoods", "idEXCOContbyLocComp", "idEXCOJapanNotified", "idEXCOQty", "idEXCOAmt", "idEXCOBringBack", "idEXCODesc"]
    var fldAccEditable = ["", "", "", "", "", "", "", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable)
}
function validateEXCO() {
    if (NewRow && NewRowsCnt == 0) {
        return true;
    }
    var fldID = ["idEXCONameOfGoodsEdit", "idEXCOContbyLocCompEdit", "idEXCOJapanNotifiedEdit", "idEXCOQtyEdit", "idEXCOAmtEdit", "idEXCOBringBackEdit"];
    var fldName = ["Name of goods/type no.", "if it is controlled by the law of local country.", "if Japan has been notified that it is a controlled item.", "Quantity", "Amount", "if the item will be bringing back to Singapore"];
    var fldType = ["text", "select", "select", "number", "number", "select"];
    var fldModal = "EXCOModal";
    if (!validateTravelFields(fldID, fldName, fldType, fldModal)) {
        return false;
    }
    return true;
}
//Export Control Hand-Carried End
//Export Control Technical Info Start
function generateEXCOTECHRow(strJSON, flgIntialLoading) {
    var objJSON = JSON.parse(strJSON);
    var tblEXCOTECH = null;
    if (flgIntialLoading) {
        tblEXCOTECH = $('#idTblEXCOTECH').DataTable();
        tblEXCOTECH.clear()
        tblEXCOTECH.draw();
    }
    var strNewRow = "";
    var strActBtns = "";
    $.each(objJSON, function (i, item) {
        strNewRow = '<td class="displaynone"><span class="datacol" prekey="ExpCode" postkey="ExpCode">ExportControlTechnicalInfo</span><span class="datacol" id="idEXCOTECHID" prekey="ID" postkey="ID">' + item.ID + '</span>' + item.ID + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol" prekey="SNo" postkey="SNo" id="idEXCOTECHSNo">' + item.SNo + '</span>' + item.SNo + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol" prekey="NameOfGoods" postkey="NameOfGoods" id="idEXCOTECHNameOfTechInfo">' + item.NameOfGoods + '</span>' + item.NameOfGoods + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol" prekey="ControlledByLocalComp" postkey="ControlledByLocalComp" id="idEXCOTECHContbyLocComp">' + item.ControlledByLocalComp + '</span>' + item.ControlledByLocalComp + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol" prekey="JapanNotified" postkey="JapanNotified" id="idEXCOTECHJapanNotified">' + item.JapanNotified + '</span>' + item.JapanNotified + '</td>';
        strNewRow += '<td class="vmiddle"><span class="displaynone datacol" prekey="Desc" postkey="Desc" id="idEXCOTECHDesc">' + item.Desc + '</span>' + item.Desc + '</td>';
        if (IsApplicantLevel == "true") {
            strActBtns = '<td class="center padding0" nowrap>';
            strActBtns += '<div class="center-btn">';
            strActBtns += '<button class="editrow" modalid="EXCOTECHModal"><i class="fa fa-pencil text-blue"></i></button>';
            strActBtns += '<button class="deleterow" modalid="EXCOTECHModal"><i class="fa fa-trash text-red"></i></button>';
            strActBtns += '<button class="undodel" style="display:none;" modalid="EXCOTECHModal"><i class="fa fa-undo text-orange"></i></button>';
            strActBtns += '</div></td>';
        }
        strNewRow += strActBtns;
        if (flgIntialLoading) {
            strNewRow = "<tr>" + strNewRow + "</tr>";
            tblEXCOTECH.row.add($(strNewRow)).draw();
        }
    });
    if (!flgIntialLoading) {
        return strNewRow;
...
function getEXCOTECHDataEditModal() {
    if (!validateEXCOTECH()) {
        return false;
    }
    var objJSON = [];
    var rowJSON = {};
    rowJSON["ID"] = $("#idEXCOTECHIDEdit").val();
    rowJSON["SNo"] = $("#idEXCOTECHSNoEdit").val();
    rowJSON["NameOfGoods"] = $("#idEXCOTECHNameOfTechInfoEdit").val();
    rowJSON["ControlledByLocalComp"] = $("#idEXCOTECHContbyLocCompEdit").val();
    rowJSON["JapanNotified"] = $("#idEXCOTECHJapanNotifiedEdit").val();
    rowJSON["Desc"] = $("#idEXCOTECHDescEdit").val();
    objJSON.push(rowJSON);
    var strJSON = JSON.stringify(objJSON);
    return generateEXCOTECHRow(strJSON, false);
}
function setEXCOTECHDataEditModal() {
    //Set Row for editing
    var tr = objSelectedRow;
    var fldEditID = ["idEXCOTECHIDEdit", "idEXCOTECHSNoEdit", "idEXCOTECHNameOfTechInfoEdit", "idEXCOTECHContbyLocCompEdit", "idEXCOTECHJapanNotifiedEdit", "idEXCOTECHDescEdit"]
    var fldID = ["idEXCOTECHID", "idEXCOTECHSNo", "idEXCOTECHNameOfTechInfo", "idEXCOTECHContbyLocComp", "idEXCOTECHJapanNotified", "idEXCOTECHDesc"]
    var fldAccEditable = ["", "", "", "", "", ""]
    setAllTravelDataEditModal(tr, fldEditID, fldID, fldAccEditable)
}
function validateEXCOTECH() {
    if (NewRow && NewRowsCnt == 0) {
        return true;
    }
    var fldID = ["idEXCOTECHNameOfTechInfoEdit", "idEXCOTECHContbyLocCompEdit", "idEXCOTECHJapanNotifiedEdit"];
    var fldName = ["technical information and data", "if it is controlled by the law of local country.", "if Japan has been notified that it is a controlled item."];
    var fldType = ["text", "select", "select"];
    var fldModal = "EXCOTECHModal";
    if (!validateTravelFields(fldID, fldName, fldType, fldModal)) {
        return false;
    }
    return true;
}
//Export Control Technical Info End
...
function createPaymentSummary() {
    //var keyArray = Object.keys(expenseTypeObj);
    setExpDetails();
    var keyArray = ExpenseDetails.expensesToList;
    keyArray = keyArray.sort(function (e1, e2) {
        if (objExpSort.expSortVal(e1) < objExpSort.expSortVal(e2)) { return -1; }
        if (objExpSort.expSortVal(e1) > objExpSort.expSortVal(e2)) { return 1; }
        return 0;
    });
    var actualSummeryCodes = [];
    keyArray.forEach(function (exp) {
        if (TicketArrangedByExp.indexOf(exp) != -1) {
            actualSummeryCodes.push(exp + "Employee");
            actualSummeryCodes.push(exp + "Company");
        } else {
            actualSummeryCodes.push(exp);
        }
    });
    keyArray = actualSummeryCodes;
    var tempObj = {};
    var strNewRow = "";
    var strNewRowGrand = "";
    var strNewRowHRIQ = "";
    ...
    for (var i = 0; i < keyArray.length; i++) {
        tempObj = expenseTypeObj[keyArray[i]];
        tempObj = !!tempObj ? JSON.parse(JSON.stringify(tempObj)) : tempObj;
        var tmpExpType = keyArray[i].replace(/(Company)|(Employee)/ig, "");
        if (!tempObj) {
            tempObj = {
                expType: keyArray[i],
                expTypeDisplay: ExpenseDetails[tmpExpType].displayName,
                preLCAmt: 0,
                postLCAmt: 0,
                isHRIQExp: ExpenseDetails[tmpExpType].isHRIQ,
                isPaidByCompany: false
            }
            if(TicketArrangedByExp.indexOf(tmpExpType) != -1){
                if (tempObj.expType.indexOf("Company") != -1) {
                    tempObj.expTypeDisplay += " By Company";
                    tempObj.isPaidByCompany = true;
                }
                if (tempObj.expType.indexOf("Employee") != -1) {
                    tempObj.expTypeDisplay += " By Employee";
                }
            }
        }
        if (ExpenseDetails[tmpExpType].expSubTypes.length > 0 && tmpExpType != "Mileage") {
            var subCreatedInPS = false;
            if (!tempObj.subExp) {
                subCreatedInPS = true;
                tempObj.subExp = { names: [] };
            }
            ExpenseDetails[tmpExpType].expSubTypes.forEach(function (sExp) {
                if (tempObj.subExp.names.indexOf(sExp) == -1) {
                    tempObj.subExp.names.push(sExp);
                    tempObj.subExp[sExp] = {
                        dispName: sExp,
                        postLCAmt: 0,
                        preLCAmt: 0
                    ...
                    if (TicketArrangedByExp.indexOf(tmpExpType) != -1) {
                        if (tempObj.expType.indexOf("Company") != -1) {
                            tempObj.subExp[sExp].dispName += " By Company";
                        }
                        if (tempObj.expType.indexOf("Employee") != -1) {
                            tempObj.subExp[sExp].dispName += " By Employee";
                        }
                    }
                }
            });
            tempObj.subExp.names = tempObj.subExp.names.sort();
            if (objExpSort.isSubDescending(tmpExpType)) {
                tempObj.subExp.names = tempObj.subExp.names.reverse();
            }
            if ((OnlyPostExpTypes.indexOf(tmpExpType)!=-1 && travelStage == "pre" && subCreatedInPS) || subCreatedInPS) {
                var defaultSub = expDefaultSub[tmpExpType];
                defaultSub = !!defaultSub ? defaultSub : tempObj.subExp.names[0];
                defaultSub = !!tempObj.subExp[defaultSub] ? defaultSub : tempObj.subExp.names[0];
                tempObj.subExp[defaultSub].preLCAmt = tempObj.preLCAmt;
            }
        }
        ...
        if (tempObj.expType != "AdvanceDetails" && tempObj.expType != "RefundDetails") {
            ...
            if (!tempObj.isPaidByCompany) { // Paid By Employee Rows
                ...
                if (!!tempObj.subExp) {
                    tempObj.subExp.names.forEach(function (sExp) {
                        //if (outputNumber(tempObj.subExp[sExp].preLCAmt) != 0 || outputNumber(tempObj.subExp[sExp].postLCAmt) != 0) {
                            drawDataTbl(getTableID(expType), true);
                            psGLCode = $(tableRowSelector(getTableID(expType))).map(function () {
                                return $.trim($(this).find("*[postkey='PostExpTypes']").text()) == sExp ? $.trim($(this).find("*[postkey='GLCode']").text()) : null;
                            }).toArray();
                            psGLCode = psGLCode.length > 0 ? psGLCode[0] : "";
                            if (psGLCode == "") {
                                if ((travelStage == "pre" && outputNumber(tempObj.subExp[sExp].preLCAmt) != 0) || outputNumber(tempObj.subExp[sExp].postLCAmt != 0)) {
                                    psGLCode = $.trim(expRow.find("#idExpGLCode").text());
                                }
                            }
                            drawDataTbl(getTableID(expType), false);
                            var subDiff = parseFloat(outputNumber(tempObj.subExp[sExp].preLCAmt)) - parseFloat(outputNumber(tempObj.subExp[sExp].postLCAmt));
                            strNewRow += '<tr id=' + row_id + '><td class="vmiddle">' + tempObj.subExp[sExp].dispName + '</td>';
                            strNewRow += '<td class="amount vmiddle" style="text-align: right;">' + outputMoney(tempObj.subExp[sExp].preLCAmt) + '</td>';
                            strNewRow += '<td class="amount vmiddle" style="text-align: right;">' + outputMoney(tempObj.subExp[sExp].postLCAmt) + '</td>';
                            strNewRow += '<td class="amount vmiddle" style="text-align: right;' + (subDiff < 0 ? "color:red" : "") + '">' + outputMoney(subDiff) + '</td>';
                            strNewRow += '<td style="display: flex; justify-content: center;align-items: center;">' + psGLCode + '</td></tr>';
                        //}
                    });
                } else {
                    //if (outputNumber(tempObj.preLCAmt) != 0 || outputNumber(tempObj.postLCAmt) != 0) {
                ...
                if (!!tempObj.subExp) {
                    tempObj.subExp.names.forEach(function (sExp) {
                        //if (outputNumber(tempObj.subExp[sExp].preLCAmt) != 0 || outputNumber(tempObj.subExp[sExp].postLCAmt) != 0) {
                            ...
                            drawDataTbl(getTableID(expType), false);
                            var subDiff = parseFloat(outputNumber(tempObj.subExp[sExp].preLCAmt)) - parseFloat(outputNumber(tempObj.subExp[sExp].postLCAmt));
                            strNewRowGrand += '<tr id=' + row_id + '><td class="vmiddle">' + tempObj.subExp[sExp].dispName + '</td>';
                            strNewRowGrand += '<td class="amount vmiddle" style="text-align: right;">' + outputMoney(tempObj.subExp[sExp].preLCAmt) + '</td>';
                            strNewRowGrand += '<td class="amount vmiddle" style="text-align: right;">' + outputMoney(tempObj.subExp[sExp].postLCAmt) + '</td>';
                            strNewRowGrand += '<td class="amount vmiddle" style="text-align: right;' + (subDiff < 0 ? "color:red" : "") + '">' + outputMoney(subDiff) + '</td>';
...
function loadDataTables() {
    ...
    //Accounting Enteries Ends
    //Export Control Hand-Carried Start
    var tblEXCO = $('#idTblEXCO').DataTable({
        "lengthChange": false,
        "bFilter": false,
        "pageLength": 10,
        "ordering": false
    });
    $('.btnEXCOAdd').on('click', function (e) {
        addNewItem('EXCOModal', 'idTblEXCO');
        e.preventDefault();
    });
    //Export Control Hand-Carried Ends 
    //Export Control Technical Information Start
    var tblEXCOTECH = $('#idTblEXCOTECH').DataTable({
        "lengthChange": false,
        "bFilter": false,
        "pageLength": 10,
        "ordering": false
    });
    $('.btnEXCOTECHAdd').on('click', function (e) {
        addNewItem('EXCOTECHModal', 'idTblEXCOTECH');
        e.preventDefault();
    });
    //Export Control Technical Information Ends 
    //My Exchange Rate Start
...
var strJSONExportControlDetails = "[]";
var objJSONExportControlDetails = [];
...
function loadTravelEntries() {
    ...
    //Export Control -Hand Carried
    getTravelEntriesJSON("ExportControl", linkKey, function (resStrJSON) {
        strJSONExportControlDetails = resStrJSON;
        objJSONExportControlDetails = JSON.parse(strJSONExportControlDetails);
        var strJSON = queryExportEntries("ExportControlHandCarried", false);
        generateEXCORow(strJSON, true);
        //Export Control -Technical Info
        strJSON = queryExportEntries("ExportControlTechnicalInfo", false);
        generateEXCOTECHRow(strJSON, true);
    });
    //EXPENSE SUMMARY
...
function queryExportEntries(argExpCode) {
    var resultJSON = [];
    resultJSON = objJSONExportControlDetails.filter(function (item) { return (item.ExpCode == argExpCode); });
    return JSON.stringify(resultJSON);
}
...
function setConditionalFlds() {
    ...
    var dacountry = $.unique(countryColl.split(';'));
    var dacity = $.unique(cityColl.split(';'));
    $("input[fldTitle='hdnDACountry']").val(dacountry);
    $("input[fldTitle='hdnDACity']").val(dacity);
    drawDataTbl(tblID, false);
...
function getExchangeRateFromMaster() {
    ...
    if (exchangeRateType == "House Rate") {
        ...
        urlForExchange = exchRateSiteUrl + "/_api/web/lists/getByTitle('ExchangeRateMaster')/Items?$select=Code/CurrencyCode,BaseCurrency/CurrencyCode,BaseCurrencyRate,Date,RateType,CurrencyDescription&$expand=Code/CurrencyCode,BaseCurrency/CurrencyCode&$filter=BaseCurrency/CurrencyCode eq '" + baseCurrency + "' and Date ge datetime'" + StartDate + "' and Date le datetime'" + EndDate + "' and RateType eq '" + exchangeRateType + "'&$orderby=Date desc";
    }
    else {
        exchangeRateType = "Spot";
        if (ExRateDate == "") {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (items.length == 0) {
                ...
                option = "";
                if (modalId == "DAModal") {
                    var cities = objSchedCountryCity(false).destCities;
                    for (var i = 0; i < items.length; i++) {
                        if (cities.indexOf(items[i].Description) != -1) {
...
function setDropDownVal() {
    ...
    var expat = $("input[fldTitle='hdnExpatriate']").val();
    if (chargToPC != "") {
    ...
    if (expat != "") {
        setSelectValue($('#Expatriate'), expat);
    }
}
...
function getDropDownVal() {
    ...
    $("input[fldTitle='hdnExpatriate']").val($('#Expatriate option:selected').val());
    getTType();
    return true;
}
...
$(document).on('click', '.DAsplitrow', function (e) {
    e.preventDefault();
    DAsplitRow($(this));
    return false;
});
function DAsplitRow(obj) {
    var ArrDate = "";
    var PostAddDay = "";
    var AdditionalDates = "";
    drawDataTbl("idTblTSH", true);
    $(tableRowSelector("idTblTSH")).each(function () {
        ArrDate = $(this).find("*[postkey='PostArrDate']").text();
        PostAddDay = $(this).find("*[postkey='PostAddOneDay']").text();
        if (PostAddDay.trim() == "Yes") {
            AdditionalDates += ArrDate + ",";
        }
    });
    drawDataTbl("idTblTSH", false);
    var tr = $(obj).closest('tr');
    var sqNo = tr.find('span[postkey="SNo"]').text();
    var DAdays = outputNumber(tr.find('span[postkey="PostDays"]').text());
    var DAFinalJSON;
    var objDAJSON = getAllJSONFromTbl("idTblDA", true);
    var finalJSON = [];
    for (var i = 0; i < objDAJSON.length; i++) {
        if (sqNo == objDAJSON[i].SNo) {
            var Days = outputNumber(objDAJSON[i].PostDays);
            var StartDate = dateObj(objDAJSON[i].PostStartDate);
            var EndDate = dateObj(objDAJSON[i].PostEndDate);
            if (StartDate.toDateString() == EndDate.toDateString()) {//if (DAdays < 1) {
                alertModal("Splitting not allowed when No Of Days less than 1 or entry has additional day");
                return false;
            }
            var preClear = false;
            var postDeptDate = dateObj($("input[fldTitle='hdnpostDeptDate']").val());
            var postDeptTime = $("input[fldTitle='hdnpostDeptTime']").val();
            ADDays = $("input[fldTitle='hdnADDays']").val();
            var tmpDays = 0;
            while (Days > 0) {
                var tmpJSON = JSON.parse(JSON.stringify(objDAJSON[i]));
                tmpDays = 0;
                tmpJSON.PostStartDate = dateString(StartDate);
                if ((postDeptDate.toDateString() == StartDate.toDateString()) && postDeptTime > WDDepCuttOffTimeConfig) { //if 1st day is half day
                    tmpDays = 0.5;
                }
                else {
                    tmpDays = Days > 1 ? 1 : Days;
                }
                if (AdditionalDates.indexOf(StartDate.format("dd/MM/yyyy")) != -1) {
                    tmpJSON.PostDays = (tmpDays + parseInt(ADDays)).toString();//(Days > 1 ? (1 + parseInt(ADDays)).toString() : (Days + parseInt(ADDays)).toString());
                    tmpJSON.PostActualDays = (tmpDays + parseInt(ADDays)).toString();//(Days > 1 ? (1 + parseInt(ADDays)).toString() : (Days + parseInt(ADDays)).toString());
                    Days -= parseInt(ADDays);
                }
                else {
                    tmpJSON.PostDays = tmpDays.toString();//(Days > 1 ? "1" : Days.toString());
                    tmpJSON.PostActualDays = tmpDays.toString();//(Days > 1 ? "1" : Days.toString());
                }
                Days -= tmpDays;
                tmpJSON.PostEndDate = dateString(StartDate);
                StartDate.setDate(StartDate.getDate() + 1);

                // Clear existing Data
                tmpJSON.PostBfast = "0";
                tmpJSON.PostLunch = "0";
                tmpJSON.PostDinner = "0";
                tmpJSON.PostDARate = "0.00";
                tmpJSON.PostFCAmt = "0.00";
                tmpJSON.PostLCAmt = "0.00";
                tmpJSON.PostAllowType = "Select";

                if (preClear) {
                    tmpJSON.PreStartDate = "";
                    tmpJSON.PreEndDate = "";
                    tmpJSON.PreCountry = "";
                    tmpJSON.PreAllowType = "";
                    tmpJSON.PreFCAmt = "0.00";
                    tmpJSON.PreCurrency = "";
                    tmpJSON.PreExRate = "0.000000";
                    tmpJSON.PreDesc = "";
                    tmpJSON.PreDays = "0";
                    tmpJSON.PreActualDays = "0";
                    tmpJSON.PreDARate = "0.00";
                    tmpJSON.PreLunch = "0";
                    tmpJSON.PreBfast = "0";
                    tmpJSON.PreDinner = "0";
                    tmpJSON.PreCity = "";
                ...
                finalJSON.push(tmpJSON);
            }
        }
        else {
            finalJSON.push(objDAJSON[i]);
        }
    }
    drawDataTbl("idTblDA", true);
    strDAJSON = JSON.stringify(finalJSON);
    getExpRowIDs('idTblDA');
    generateDARow(strDAJSON, true);
    generateSQNo('DAModal', 'idTblDA');
    collapseAll('idTblDA');
    $('#idTblDA').find('.predisplay').hide();
    drawDataTbl("idTblDA", false);
    calculateTotalAmt("idTblDA");
}
function setDACountryValue() {
    var destCountries = [];
    var lsCountry = $("#idDACountryEdit");
    destCountries = objSchedCountryCity(false).destCountries;
    lsCountry.find("option").remove();
    destCountries.forEach(function (country) {
        lsCountry.append('<option value="' + country + '">' + country + '</option>');
